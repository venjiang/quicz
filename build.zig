...
pub fn build(b: *std.Build) void {
    ...
    // Pure-Zig TLS 1.3 over UDP loopback executable
    const exe_tls13_udp_loopback = b.addExecutable(.{
        .name = "quicz-tls13-udp-loopback",
        .root_module = b.createModule(.{
            .root_source_file = b.path("examples/tls13_udp_loopback.zig"),
            .target = target,
            .optimize = optimize,
            .imports = &.{
                .{ .name = "quicz", .module = quicz_mod },
    ...
    // Pure-Zig TLS 1.3 path validation over UDP loopback executable
    const exe_tls13_path_validation_loopback = b.addExecutable(.{
        .name = "quicz-tls13-path-validation-loopback",
        .root_module = b.createModule(.{
            .root_source_file = b.path("examples/tls13_path_validation_loopback.zig"),
            .target = target,
            .optimize = optimize,
            .imports = &.{
                .{ .name = "quicz", .module = quicz_mod },
    ...
    // zig build run-tls13-path-validation-loopback
    const run_tls13_path_validation_loopback = b.step("run-tls13-path-validation-loopback", "Run pure-Zig TLS 1.3 path validation UDP loopback");
    ...
    const run_interop_event_loopback = b.step("run-interop-event-loopback", "Run event-driven interop loopback (TESTCASE: handshake/transfer/loss/congestion/persistent/key-update/path/stream-control/stream-limit)");
