...
- 2026-06-04: Extended `examples/tls_openssl_backend_adapter.zig` so the
  ...
  over loopback UDP, then verifies the adapter-installed client 1-RTT keys can
  protect a short packet that a peer connection decrypts and ACKs with matching
  transcript secrets. Full endpoint-owned live TLS handshake/socket loop
  remains pending.
