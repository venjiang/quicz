...
The matrix is organized for execution, not for feature cataloging. P0 means
"required before the first usable QUIC v1 transport claim." P1 and later are
parked unless they directly unblock a P0 exit test.

Mature QUIC stacks are used as prioritization signal only: their release work
keeps returning to TLS/crypto correctness, endpoint timers, MTU/path behavior,
stream and flow-control correctness, CID retirement, close/error propagation,
recovery/PTO edge cases, fuzzing, and interop automation. `quicz` should follow
that order: core vertical transport first; examples, API polish, and optional
extensions later.

### Task intake filter

A task can enter the active queue only when it satisfies all of these checks:

- It maps to RFC 8999, RFC 9000, RFC 9001, or RFC 9002 behavior needed by a
  QUIC v1 client/server transport.
- It produces or tightens Zig tests for the changed behavior.
- It improves one P0 exit criterion below.
- It does not edit examples, README polish, HTTP/3, DATAGRAM, qlog, PMTU/GSO,
  broad fuzzing, or second-stack interop unless that is the smallest unblocker
  for a P0 gate.

### P0 gates and active task queue

| Gate | Current state | Exit criterion | Active task queue |
| --- | --- | --- | --- |
| P0-A. Wire envelope and non-commit invariants | Partial | Malformed packet/frame/transport-parameter input is rejected before ACK, CID, token, stream, recovery, or route mutation; close-propagating paths emit the correct transport error. | Fill only gaps that block TLS, endpoint routing, close, recovery, or interop. Do not spend standalone loops on codec-only edge cases. |
| P0-B. TLS-owned packet protection | Partial | Local Zig client/server completes a certificate-verified UDP handshake and exchanges protected 1-RTT packets without mock keys or OpenSSL. Certificate, SNI, ALPN, transport-parameter, key-share, and transcript failures remain non-committing. | Prefer rollback holes and missing TLS/backend hooks that can break protected Initial, Handshake, or 1-RTT operation. Defer TLS directory reshaping until behavior stabilizes. |
| P0-C. Endpoint-owned UDP lifecycle | Partial | Bounded client/server endpoint loops complete handshake, STREAM transfer, due-deadline service, close, and route cleanup using endpoint APIs only. | Highest priority when a real socket loop still needs caller glue around receive/process/drain/timer/cleanup. New narrow lifecycle helpers are allowed only if they collapse into the vertical loop. |
| P0-D. STREAM and flow-control vertical | Partial | Protected UDP covers bidi and uni open/write/read/FIN, RESET_STREAM, STOP_SENDING, stream-count credit release, connection/stream flow-control unblock, and retransmission requeue. | Prefer end-to-end protected path failures over parser-only work. Every fix must prove state credit, pending control frames, and packet output stay coherent. |
| P0-E. ACK, loss, PTO, and NewReno baseline | Partial | Deterministic tests plus one socket-backed loss run prove ACK cleanup, RTT/loss/PTO progression, STREAM/CRYPTO retransmission, bytes-in-flight accounting, and no live recovery timer after terminal close/drain. | Pick timer/recovery bugs that can stall a real endpoint loop before tuning congestion behavior. |
| P0-F. Lifecycle security edges | Partial | CONNECTION_CLOSE, APPLICATION_CLOSE, Retry, address validation, path validation, migration route update, NEW/RETIRE_CONNECTION_ID, idle close, stateless reset receive, and inactive-CID reset emission are endpoint-visible and non-committing on preflight failure. | Work only the edges needed for robust client/server operation or P0-G interop. |
| P0-G. First external interop gate | Partial | One external stack repeatedly passes certificate verification, Retry, STREAM FIN echo, reset/stop, close, and one controlled loss/PTO case against `quicz` client/server. | Use `quic-go` as the first gate. A Rust or second-stack gate moves to P1 unless needed to disambiguate a `quic-go` result. |

### Next P0 work order

| Order | Task | Primary gate | Done when |
| --- | --- | --- | --- |
| 1 | Collapse endpoint socket-loop gaps | P0-C | Client and server each expose one bounded receive/process/drain/due-deadline step that a UDP loop can call without per-case glue or examples edits. |
| 2 | Prove protected stream and flow-control vertical | P0-D | A Zig endpoint loop test covers bidi + uni FIN, stream-count release, RESET_STREAM, STOP_SENDING, flow-control unblock, and route-bound protected output. |
| 3 | Harden recovery timer integration | P0-E | Endpoint tests prove PTO retransmits CRYPTO/STREAM on the committed route, ACKs clear outstanding state, and closed/drained connections leave no recovery deadline. |
| 4 | Close, CID, Retry, and reset route safety sweep | P0-F | Missing-route, zero-capacity, invalid token/CID, stateless reset, and close paths fail before mutation and clean route/record state after terminal drain. |
| 5 | TLS/backend production boundary cleanup | P0-B | The protected path no longer depends on OpenSSL or mock secrets for the local client/server vertical; remaining TLS failures have rollback coverage. |
| 6 | First repeatable external interop gate | P0-G | `quic-go` interop is repeatable from a clean checkout for handshake, Retry, FIN echo, reset/stop, close, and controlled PTO. |
...
Each development loop chooses the first unfinished task in "Next P0 work order"
...
`goal.md`, run the relevant Zig validation, commit, and push. Do not mark a
P0 gate done from mock-only tests when its exit criterion requires protected UDP
or external interop.
...
| --- | --- | --- |
| UDP client/server endpoint | Required for first usable milestone. One endpoint owner must drive accept/connect, packet receive/send, timers, route cleanup, and close. | Partial: `Tls13ServerEndpoint` owns dynamically allocated or bounded TLS server records, active connection-handle enumeration, active-connection count/limit/capacity admission signals, active-record installed-key output polling/draining with committed route paths, route-bound application stream send and cancellation polling, multi-record pending-work sweep with closed-record destruction and single-connection deadline-planning, single-connection output-poll/drain, cross-connection output-poll/drain, single/cross explicit output-poll/drain, and sweep key-discard reporting, next-deadline selection, route-bound output draining, and post-drain next-deadline selection, ingress classification, route-bound Version Negotiation/stateless-reset response pairing, lifecycle routing/timers, route-bound accepted/routed Initial and Handshake TLS backend output drains with accepted Initial poll/drain post-output deadline selection, capacity-aware accepted-Initial admission drops that preserve caller-owned records and avoid half-installed routes, endpoint-owned long-packet Initial/Handshake dispatch with route-bound output, routed datagram dispatch for long-header CRYPTO and installed-key short packets, combined classify/process and classify/process/bounded-drain dispatch for routed datagrams, one bounded receive/drain/pending-work step for server socket loops with same-step idle/close record cleanup, installed-key coalesced Initial/Handshake datagram dispatch plus unsupported coalesced long-datagram rejection, route-bound Retry follow-up Initial validation plus TLS output drains, atomic record retirement, route-owned installed-key short-packet receive, route-bound installed-key feed/output polling, route-bound due recovery output polling/draining with post-drain next-deadline selection, route-bound protected close emission, route-bound explicit close draining with post-drain close deadline selection and zero-capacity non-commit guards, close-on-frame-error emission through the committed route, due idle/close record plus route retirement, process-server datagram send selection from the committed route tuple, and process-server validated route migration; `Tls13ClientEndpoint` owns one client transport, CID route, caller-selected route migration, route-bound Initial begin output, Version Negotiation follow-up route registration and endpoint-owned restart Initial emission, route-bound receive-generated Initial/Handshake output, route-bound application output polling/draining plus stream send/cancellation polling, route-bound close-on-frame-error output polling/draining with post-drain next-deadline selection, one bounded receive/drain/due-deadline step for client socket loops with same-step idle, key-discard, terminal receive, and close cleanup, route-bound due recovery output polling/draining with post-drain next-deadline selection, recovery-timer mirror, route-bound protected close emission, route-bound explicit close draining with post-drain close deadline selection and zero-capacity non-commit guard, and due idle/close route plus timer retirement. Process client/server UDP receive capacity matches each configured local datagram limit. Callers retain UDP I/O and admission policy; production multi-connection event-loop policy is still missing. |
| TLS 1.3 integration | Required. Pure-Zig TLS 1.3 (`src/quic/tls13.zig` + `src/quic/tls13_backend.zig`) exposed through the `CryptoBackend` interface; the OpenSSL C adapter is the deprecated path. | Partial: client and server TLS transports own protected I/O plus recovery, idle, close, and key-discard deadline selection/service; each concurrent server record owns `Tls13ServerTransport` (Connection, TLS backend, and fixed local/peer/Original-DCID copies). Server Certificate messages preserve the configured leaf-first DER chain and gate NewSessionTicket emission on connected server state; a client with a configured bundle validates the received leaf-to-issuer chain before the trust anchor. Pure-Zig TLS client validates local SNI, ALPN, and PSK ClientHello output bounds, rejects unsolicited ServerHello and EncryptedExtensions extensions while rejecting duplicate extension types, rejects missing, unoffered, or unsolicited server ALPN selections, unsupported HelloRetryRequest sentinel input, malformed ServerHello base fields, supported_versions length, and invalid X25519 key shares, oversized local or peer transport-parameter extension bytes, trailing bytes after extension vectors, non-empty server Certificate request contexts, malformed CertificateVerify signature vectors, malformed NewSessionTicket length/vector/ticket boundaries, duplicate NewSessionTicket extension types including known `early_data` and unknown duplicates, zero-lifetime NewSessionTicket messages, excessive NewSessionTicket lifetimes, invalid NewSessionTicket `early_data` values, and unsupported post-handshake messages; resumed ClientHello and the `CryptoBackend` 0-RTT write-secret hook are gated on a stored ticket carrying QUIC's RFC 9001 `0xffffffff` sentinel, denied 0-RTT hook polls do not build ClientHello or mutate CRYPTO output, and PSK identities preserve the server-provided `ticket_age_add` in `obfuscated_ticket_age`. Configured pure-Zig TLS servers bound EncryptedExtensions output, emit CSPRNG-backed NewSessionTicket `ticket_age_add` plus the RFC 9001 `early_data` sentinel, and validate ClientHello SNI, reject duplicate ALPN protocol names, reject oversized SNI host_name, reject malformed ClientHello base fields and extension vector lengths, reject non-empty ClientHello legacy_session_id, reject trailing bytes after ClientHello extensions, reject duplicate ClientHello extension types including unknown duplicates, require supported_groups, reject duplicate supported_groups entries, reject empty key_share entries, duplicate key_share groups, key_share groups absent from supported_groups, key_share order that differs from supported_groups, and invalid X25519 key shares, reject oversized local or peer transport-parameter extension bytes, require empty early_data, require pre_shared_key to be last, require compatible PSK key-exchange modes, reject malformed pre_shared_key identity/binder vectors including undersized vectors, non-SHA-256 binder lengths, and mismatched binder verification, reject client Finished verify_data mismatches, emit and parse ServerHello `pre_shared_key` identity selection, skip Certificate/CertificateVerify on the selected-PSK path, and validate signature-algorithm support before server flight progress. The `quic-go` proof below completes a real TLS-owned handshake and two FIN-terminated 1-RTT STREAM echoes without mock keys or OpenSSL; a production endpoint-owned event loop remains missing. |
| QUIC v1 packet protection | Required. Initial, Handshake, 0-RTT when enabled, and 1-RTT packets must be produced and consumed by the TLS-owned path. | Partial: v1/v2 Initial, Retry integrity, protected long/short helpers, receive-side UDP datagram size rejection before protected packet decryption/state updates, and mock installed-key paths exist. |
| Streams | Required. Bidirectional and unidirectional stream open, read, write, FIN, reset, STOP_SENDING, and stream limits must work over protected UDP. | Partial: TLS-owned client/server transports expose bidirectional and unidirectional open/write/read/FIN plus RESET_STREAM and STOP_SENDING; `Tls13ClientEndpoint` and per-record `Tls13ServerEndpoint` APIs now expose endpoint-owned stream open/read/write/FIN plus RESET_STREAM/STOP_SENDING controls. TLS-backed endpoint UDP loopbacks prove RESET_STREAM and FIN-driven MAX_STREAMS_BIDI release; an independent `quic-go` client proves sequential stream-count credit release across streams 0-to-4-to-8-to-12, RESET_STREAM(41), STOP_SENDING(42) followed by RESET_STREAM(42), and client uni stream 2 / server uni stream 3 FIN exchange. Broader stream-limit interop remains incomplete. |
| Flow control | Required. Connection, stream, and stream-count flow control must work over protected UDP. | Partial: frame-payload and protected loopback coverage exists; frame-payload and protected 0/1-RTT STREAM writes re-split against their exact packet budget, including ACK space where applicable; a quic-go client completes a 12 KiB echo across the Zig server's 2 KiB stream and 8 KiB connection initial windows. |
| ACK/loss/PTO recovery | Required. ACK processing, packet/time-threshold loss, PTO, retransmission, and timer service must drive the endpoint loop. | Partial: deterministic recovery model and socket-backed loopbacks exist; a bounded Zig server also completes external 1-RTT and Initial-space PTO recovery, while production lifecycle integration is incomplete. |
...
| Connection IDs and stateless reset | Required. Routing, CID issuance/retirement, reset-token handling, close cleanup, and inactive-CID reset emission must work in the endpoint lifecycle. | Partial: endpoint router, connection-level reset receive-to-draining state, endpoint installed-key feed active-route reset handling with recovery timer disarm, client endpoint active reset receive-to-draining handling with receive-step close-deadline selection, server endpoint bounded receive/drain and direct receive-step active reset reporting with close-deadline record retirement, server endpoint inactive-reset response path pairing, short-header/fixed-bit validation for locally generated stateless reset datagrams, lifecycle helpers, and socket-backed loopbacks exist. TLS client/server transports select the newest active peer `NEW_CONNECTION_ID` for outgoing packets, falling back to the authenticated Initial SCID; zero-length CID `NEW_CONNECTION_ID` and `RETIRE_CONNECTION_ID` protocol-violation guards are enforced before CID or ACK mutation; servers now advertise a stateless reset token only after a non-empty Initial Source Connection ID is known. The concurrent TLS process server generates an independent 8-byte CSPRNG Initial SCID per accepted connection. Full TLS-owned lifecycle integration is still incomplete. |
| Retry and address validation | Required for server-side robustness and interop. | Partial: concurrent Retry uses fresh per-process token-secret and per-issuance nonce entropy; token policy, Retry validation, address-validation loopbacks, and TLS extension byte checks exist. Replay-filter snapshots now restore newest unique token fingerprints so merged persisted state does not waste bounded replay capacity; broader production storage/replay policy is still missing. |
...
| 0-RTT | Schedule after the first 1-RTT stream echo interop gate; not required for the current milestone. | Partial: explicit accept/reject, mock installed-key 0-RTT paths, server-only protected 0-RTT receive guards, NewSessionTicket RFC 9001 sentinel validation, ticket-gated ClientHello `early_data` emission, connection-policy-bound EncryptedExtensions acceptance, client rejection-driven key discard, server-side ticket-age policy, and bounded local PSK identity replay filtering with snapshot/restore exist; production/distributed replay policy is still missing. |
...
Pure-Zig TLS client 0-RTT state now records whether the current ClientHello
actually offered `early_data`; EncryptedExtensions `early_data` is rejected
unless PSK selection, ticket 0-RTT permission, and that same ClientHello offer
all match. The rejection path leaves early-data acceptance and peer
transport-parameter state uncommitted.

Pure-Zig TLS client `CryptoBackend` hooks now expose server transport
parameters and negotiated ALPN only after the TLS handshake completes. A
certificate-validation failure can leave parsed EncryptedExtensions in the TLS
state machine, but the connection-facing hooks continue to return no server
parameters, no ALPN, no 1-RTT secrets, and no handshake confirmation.
The Connection-level CRYPTO-frame drive now has the same regression coverage:
a bad server certificate after a valid ServerHello and EncryptedExtensions does
not apply the parsed server transport parameters, does not install 1-RTT keys,
and does not confirm the handshake.
...
transcript, or server state. Finished verify_data failures keep transcript,
completion state, and client-side application key installation uncommitted.
ClientHello construction now rejects
...
oversized configured certificate chains or encoded Certificate flights, plus
missing, short, or otherwise invalid configured private keys before committing
parsed peer state or queuing/emitting ServerHello. Server-side Certificate output now also prevalidates the
...
The close-propagating connection backend drive now maps peer CRYPTO input that
fails in the TLS backend to a QUIC `CRYPTO_ERROR` CONNECTION_CLOSE using the
CRYPTO frame type, so lifecycle poll/drain wrappers can emit close output
instead of leaving the TLS failure as a bare backend error.
...
Connection crypto-backend driving now discards Handshake packet-number-space
state after backend-confirmed Application-only ticks when no Handshake CRYPTO
remains queued, while preserving the space when late Handshake CRYPTO still
needs to be sent.
...
Server-side connected TLS backends now reject peer post-handshake CRYPTO before
buffering input or emitting NewSessionTicket side effects; client-side
Application CRYPTO still handles server NewSessionTicket messages.
Server NewSessionTicket emission is now gated to Application-space output pulls,
so late Initial or Handshake polling cannot enqueue post-handshake Application
CRYPTO as a cross-space side effect.
NewSessionTicket construction and client-side processing now also require
derived Application traffic secrets, preventing resumption PSK derivation from
an uninitialized master secret even if TLS state is incorrectly marked
connected.
...
STREAM and CRYPTO max-payload budgeting now also caps binary-search candidates
against QUIC's maximum end offset, preserving the last sendable bytes when an
...
agreement, matching RFC 8446's client key_share uniqueness rule.
...
the complete state.
NewSessionTicket parsing now rejects duplicate extension types, including the
known `early_data` extension and duplicate unknown extensions, before deriving
or storing resumption PSKs for a later ClientHello.
...
ticket age on the server side. NewSessionTicket now carries QUIC's 0-RTT
...
ServerHello `pre_shared_key` selection is also bound to the current ClientHello
offer state, so a stale or externally changed ticket store cannot make the
client accept PSK selection that the current ClientHello did not advertise.
...
Protected 0-RTT receive is now server-only in the connection receive path:
active clients reject both explicit-key and installed-key 0-RTT long packets
before packet-number or ACK state mutation.

Connection-ID handling now enforces the RFC 9000 zero-length CID boundary: a
connection sending a zero-length destination CID rejects peer
`NEW_CONNECTION_ID` before active-CID or ACK mutation, and a connection that
provided a zero-length Initial Source CID rejects local `NEW_CONNECTION_ID`
issuance plus peer `RETIRE_CONNECTION_ID` before local-CID retirement. The
close-propagating receive path maps authenticated violations to
`PROTOCOL_VIOLATION`.
...
bounded route-bound output drain; Retry follow-up and routed Handshake input use
the same record boundary. This is
incremental endpoint ownership rather than a complete production event loop.
Route-facing server due-deadline drains now return the next endpoint-visible
...
caller-owned slots after authenticated frame errors, returns the post-drain
next deadline, and preserves queued output on non-closing route mismatches.
...
timers.
`Tls13ClientEndpoint` now forwards unidirectional stream open, RESET_STREAM,
and STOP_SENDING through the endpoint-owned API; the focused test proves these
controls mutate the same owned connection queues instead of requiring callers
to bypass the endpoint boundary.
`Tls13ServerEndpoint` now forwards per-record stream open/read/write/FIN,
RESET_STREAM, and STOP_SENDING through the endpoint-owned API. The bounded
record test proves server-initiated stream IDs and cancellation queues stay on
the selected owned record, while unknown handles return `UnknownConnectionId`.
`Tls13ServerEndpoint.sendStreamWithRoutePath()` now resolves the committed route
before queuing application data, then returns the protected stream datagram with
that route. The focused test proves missing handles fail without bypassing the
endpoint boundary and stream output is paired to the selected record's route.
...
missing-route failures before STREAM data, pending RESET_STREAM, or
...
first ack-eliciting 1-RTT PING, delivers the following three PINGs, and drains
the server's sparse ACK after any handshake control output. The receiver proves
the packet-number gap (`next_peer_packet=4`), while the sender proves the
packet-threshold congestion response (`cwnd_before=12000`,
`cwnd_after=6000`).

`zig build run-interop-event-loopback -- persistent` uses the same TLS-owned
path to establish a real RTT sample, withhold three timed contiguous PINGs,
deliver a fourth, and process the lifecycle-routed server ACK. Its observed
persistent-congestion duration is 411ms; the withheld send span is 1090ms and
the resulting response reduces the congestion window from 12000 to the
1200-byte-datagram minimum of 2400.

`zig build run-interop-event-loopback -- key-update` drains residual control
output, then drives two TLS-owned 1-RTT key-phase transitions over real UDP.
...
expired previous generation (`stale_rejected=true`). Single- and
cross-connection key-discard due-deadline poll/bounded-drain paths are also
covered: they clear the retained key without consuming the queued ACK/output
that normal polling should still own.
The client endpoint now also has direct no-input due-deadline poll and
bounded-drain coverage for `.key_discard`: a socket loop can service the
retained-key expiry without first receiving a datagram, without emitting
recovery output, and without retiring the committed route.
The server endpoint has matching direct no-input due-deadline coverage for both
route-path and route-less bounded-drain entry points: the socket owner can
service a retained-key expiry without first receiving a datagram, with no
output drain and with the active record and committed route kept installed.
...
delivers a protected `PATH_CHALLENGE` to that new tuple and routes the matching
...
registered routes and lifecycle timer refresh; the process server sends managed
connection output through the endpoint's committed route tuple instead of the
admission-time peer address. The client endpoint delegates
Initial/Retry/Handshake processing to its `Tls13ClientTransport`; after the
...
1-RTT output. With
`QUICZ_PROCESS_INTEROP_CLIENT_COMPLETION=migrate`, a client switches to a new
UDP socket after handshake, the server queues PATH_CHALLENGE, commits the route
after PATH_RESPONSE validation, and the migrated stream echo completes
(`path_challenge_queued=true`, `route_updated=true`, `migrated=true`). By
default the reproducible command starts two tagged client
...
timer arming up to capacity therefore avoid storage growth. A capacity-aware
admission helper reports a dropped Initial as data while preserving the
caller-owned record and proving no lifecycle route was installed. The distinct client tags prevent Initial DCID and
...
After the certificate-verified handshake, the client opens streams 0 and 4,
sends FIN-terminated `hello` and `world`, and requires each matching echo and
peer FIN. The repository's one-connection `quic-go` v0.59.0 fixture generates
a localhost CA PEM at runtime; against that independent peer the Zig client
reported `external_handshake_done=true certificate_verified=true
alpn=hq-interop echo_streams=2 echo_bytes=10`. This validates external
protected 1-RTT multiplexed STREAM parsing, echo, and terminal send-side close,
...
| --- | --- | --- | --- |
| RFC 8999 version-independent properties | Partial | Version Negotiation packet codec, endpoint unsupported-version Version Negotiation response helper, client-side validation/selection/discard state, reserved-version greasing detection/selection skip, long/short packet envelopes, connection ID length checks including first-client-Initial DCID length enforcement, stateless reset helpers, packet codec/example tests, and socket-backed UDP endpoint routing loopback with protected follow-up Initial emission. `Tls13ClientTransport` now surfaces a validated selection and follow-up configuration only, requiring a fresh connection; a v1-only `quic-go` server externally proves v2 Initial → Version Negotiation → fresh v1 TLS/STREAM echo. | Broader TLS-owned socket routing and version-independent behavior remain incomplete. |
| RFC 9000 transport protocol | Partial | Frame codec with ACK/ACK_ECN range validation, transport parameters, connection state, streams, flow control, connection IDs, Retry/tokens, path validation, close/reset behavior, short-header/fixed-bit stateless reset emission validation, endpoint idle/close route-timer cleanup, endpoint routing helpers, receive-side local UDP datagram size rejection before protected packet decryption/state updates, endpoint installed-key feed and client endpoint active-route stateless reset receive-to-draining handling, server endpoint bounded receive/drain and direct receive-step active reset reporting with terminal record cleanup, server endpoint route lookup directly resolving the owned record for installed-key short-packet receive, server endpoint active connection-handle enumeration plus active-count/limit/capacity signals for bounded admission, server endpoint active-record installed-key output polling/draining with committed route paths, server endpoint multi-record pending-work sweep with closed-record destruction, next-deadline selection, route-bound output draining, and post-drain next-deadline selection, server endpoint route-bound Retry follow-up validation/TLS output, route-bound long-packet Initial/Handshake dispatch, server endpoint routed datagram dispatch across long-header CRYPTO and installed-key short packets, combined server endpoint classify/process and classify/process/bounded-drain dispatch for routed datagrams, bounded server and client receive steps that keep receive results visible while servicing route-bound recovery output and same-step idle/close cleanup, installed-key coalesced Initial/Handshake datagram dispatch with unsupported coalesced long-datagram rejection, and protected close emission with terminal record cleanup, lifecycle-owned caller-keyed protected UDP packet loopback, socket-backed UDP path-validation route-update loopback, socket-backed UDP lifecycle Retry/address-validation loopback, Connection-level TLS-owned path frame-exchange verification for protected close, path validation, stateless reset, flow-control window update, MAX_STREAMS, DATA_BLOCKED/STREAM_DATA_BLOCKED/STREAMS_BLOCKED, NEW_TOKEN, Retry, HANDSHAKE_DONE, and application close, plus a pure-Zig TLS-owned UDP migration that verifies PATH_CHALLENGE/PATH_RESPONSE and commits the server endpoint route only after validation. | Full protected/TLS socket-backed client/server loopback, complete endpoint lifecycle, and external interop. |
| RFC 9001 TLS and packet protection | Partial | QUIC v1 Initial secret derivation, AEAD/header-protection helpers, Retry Integrity Tag, protected packet helpers, mock CRYPTO backend handoff, installed-key tests, client endpoint route-bound Retry follow-up Initial output, backend-confirmed Handshake packet-number-space discard across Application-only crypto-backend ticks, TLS backend receive-state packet-number-space gating, client backend server-parameter/ALPN exposure gated on handshake completion plus Connection-level certification-failure non-commit coverage, Finished verify_data failure non-commit coverage, close-propagating backend receive `CryptoError` mapped to `CRYPTO_ERROR` CONNECTION_CLOSE, server post-handshake CRYPTO rejection and Application-only NewSessionTicket output gating, client/server NewSessionTicket application-secret gating, ACK-gated installed-key key-update initiation, and Connection-level 1-RTT key update (KEY_PHASE flip + next-generation keys) verification on the TLS-owned path. The endpoint lifecycle now schedules retained-key expiry as a `.key_discard` deadline; the pure-Zig TLS-owned UDP event loop proves two live key updates, ACK gating, due-deadline old-key discard, and rejected expired-key replay, the client endpoint directly services no-input key-discard poll and bounded-drain deadlines without route retirement or recovery output, and the server endpoint directly services no-input key-discard route-path and route-less drain deadlines with the active record and route retained. | Real TLS backend transcript integration, TLS-owned traffic-secret production, and broader live key-update policy/interoperability beyond the proved single-connection rotation. |
| RFC 9002 loss detection and congestion control | Partial | Invalid ACK/ACK_ECN range rejection before recovery side effects, largest-acknowledged RTT sampling, connection-level RTT estimate sharing and PTO backoff across packet number spaces with client Initial ACK reset suppression, Initial/Handshake RTT ACK-delay suppression plus Application ACK delay scaling/capping, packet/time-threshold loss, aggregate loss-time-before-PTO timer deadline selection/service with closing/draining disarm, anti-amplification-limited server PTO disarm/rearm plus expired-PTO service when new datagrams unblock sending, and client no-in-flight anti-deadlock PTO, endpoint-owned multi-connection recovery timer scheduling, server endpoint due recovery output paired with the committed route tuple, client endpoint due recovery output polling/draining paired with the committed route tuple plus post-drain deadline selection, cross-space bytes-in-flight congestion admission, peer max_udp_payload_size recovery max_datagram_size/initial-cwnd resync, max_datagram_size growth minimum-window clamp, endpoint connection lifecycle helper tying route retirement to timer disarm plus caller-keyed protected long/Initial-Handshake CRYPTO-space/0-RTT/short-packet, explicit key-phase/key-update short-packet, caller-owned key-phase short-packet, installed-key Handshake/0-RTT long-packet, and installed-key protected short-packet timer refresh, congestion-window bypass for one armed PTO probe, ACK-driven frame-payload STREAM/CRYPTO, protected CRYPTO, protected 0-RTT STREAM/RESET_STREAM/STOP_SENDING retransmission requeue, and ACKed RESET_STREAM obsolete retransmission suppression, NewReno slow-start/congestion-avoidance byte-counted and batched-ACK growth with underutilized-cwnd suppression, recovery-period behavior, new-congestion-event one-packet recovery probe, and minimum-window ssthresh clamp, PTO-backoff-independent persistent congestion duration/response with min-RTT refresh, recovery-period reset/re-entry, and non-contiguous suppression, ACK_ECN CE-driven NewReno recovery response, packet-space PTO PING/new-data/in-flight-CRYPTO/protected-0-RTT-control/protected-0-RTT-STREAM/in-flight-STREAM/cross-space probe hooks with Initial/Handshake max_ack_delay suppression and Application PTO gating until handshake confirmation, ECN validation scoped to each connection path and cleared on connection retirement, socket-backed UDP lifecycle loss/PTO recovery plus lifecycle congestion-recovery/lifecycle STREAM-retransmission loopbacks, Connection-level ACK clears in-flight + RTT sampling + PTO backoff (deadline doubling) verification on the TLS-owned path, a pure-Zig TLS-owned UDP event-loop sparse-ACK packet-threshold loss that reduces NewReno congestion window from 12000 to 6000, and a TLS-owned real-RTT persistent-congestion exchange that reduces it to 2400. | Full TLS-owned socket-owned protected-packet loss/PTO timer lifecycle integration and remaining recovery edge cases with controlled-clock tests. |
...
PTO probe, ALPN selection validation, SNI validation, ClientHello extension validation, duplicate-extension rejection, PSK/early-data validation, key-share/signature-algorithm validation, and recovery-timer service — all without
mock keys or OpenSSL.
...
Endpoint-owned deadline selection, cross-record output polling, and installed-key
receive view construction now apply the same pre-closed record cleanup, so a
...
| Stateless reset | Partial helper + constant-time token match + connection detection + connection-level reset receive-to-draining state + NEW_CONNECTION_ID token uniqueness checks + endpoint inactive-CID reset datagram construction + route/reset/drop receive classification exposed through `EndpointConnectionLifecycle` + socket-backed UDP reset emission loopback + socket-backed close-triggered lifecycle route retirement/reset loopback | Integrate reset emission into socket-owned endpoint lifecycle and connection close/drop policy. | Existing tests cover reset token match, false-positive rejection, short datagram rejection, duplicate-token rejection across CIDs, retired-token ignore, active-token receive moving the connection into draining, pending close output suppression after reset receive, active-route token suppression, retired-route token lookup, inactive-route reset datagram construction, smaller-than-trigger sizing, route/reset/drop receive action classification, lifecycle-owned route/reset/drop after timer-disarming retirement, client endpoint active-reset receive-step visibility with recovery-timer disarm and post-reset close deadline selection, server endpoint route-bound and receive-step active reset reporting with close-deadline record/route cleanup, server endpoint inactive-reset response path pairing, and ambiguous reset-token CID rejection; `run-stateless-reset` demonstrates endpoint inactive-CID reset action; `run-udp-stateless-reset-loopback` demonstrates real UDP active-route suppression before retirement, unknown-CID drop classification, trigger delivery after retirement, reset emission, and client token matching; `run-udp-close-lifecycle-loopback` demonstrates protected close delivery, lifecycle-routed protected receive auto-close, `EndpointConnectionLifecycle` connection-handle route retirement, retained reset token lookup, reset emission, and client token matching; later endpoint tests cover full TLS-owned lifecycle integration. |
...
- 2026-07-22: Connection-level `NEW_TOKEN` validation now has explicit
  ...
  rejects the wire payload without storing a token or queuing ACK state, while
  `processDatagramOrClose()` maps the same frame to `FRAME_ENCODING_ERROR`
  `CONNECTION_CLOSE`.
...
- 2026-07-17: Tightened pure-Zig TLS server ClientHello extension
  ...
  otherwise be ignored, matching the RFC 8446 duplicate-extension boundary.

- 2026-07-17: Tightened local transport-parameter configuration
  validation. `Connection.init()` now rejects local max UDP payload size,
...
- 2026-07-17: Aligned RFC 9002 persistent congestion duration with packet
  ...
  Handshake long-packet-space thresholds, and a separate Application threshold
  test proves the peer `max_ack_delay` term prevents a premature persistent
  congestion reset.
...
- 2026-07-17: Tightened RFC 9000 STREAM duplicate-data handling after final
  ...
  ignored, but late frames that change bytes at an existing stream offset now
  fail with a stream-data protocol violation, including through OrClose close
  classification.

- 2026-07-17: Extended out-of-order STREAM duplicate trimming for pending
  ranges. The receive path now accepts same-byte prefix and suffix overlaps
  against pending stream data, ignores duplicate subranges that are fully
  covered by pending data, and still rejects changed bytes as invalid stream
  data.

- 2026-07-17: Extended pending STREAM duplicate trimming to middle overlaps.
  Incoming STREAM data that covers an existing pending range with identical
  bytes can now be split into the new prefix and suffix gaps. Receive
  flow-control accounting grows only by those new bytes, and changed overlap
  bytes still fail as invalid stream data.

- 2026-07-17: Extended out-of-order CRYPTO duplicate handling for pending
  ranges. CRYPTO frames that cover pending bytes with identical data can now be
  split into the remaining new gaps, while changed overlap bytes still fail as
  invalid payloads with rollback.

- 2026-07-17: Classified conflicting CRYPTO overlap on close-propagating
  receive paths. `processDatagramOrClose()` now queues a `PROTOCOL_VIOLATION`
  CONNECTION_CLOSE for CRYPTO frames that change bytes already present in the
  contiguous CRYPTO buffer or pending ranges, while rollback-only receive APIs
  keep returning `InvalidPacket` without queuing close.
...
- 2026-07-17: Aligned client-side Version Negotiation discard state with
  ...
  Handshake, or Application packet, a later Version Negotiation packet is
  ignored without selecting a follow-up version or mutating existing state.
...
- 2026-07-17: Added route-bound explicit close draining to the client endpoint.
  `Tls13ClientEndpoint.closeWithRoutePathAndDrainDatagrams()` queues a protected
  application CONNECTION_CLOSE, drains bounded committed-route output, and
  returns the post-drain close deadline so socket loops can update their wait
  target without a separate poll.

- 2026-07-17: Added route-bound explicit close draining to the server endpoint.
  `Tls13ServerEndpoint.closeWithRoutePathAndDrainDatagrams()` queues a protected
  CONNECTION_CLOSE for one owned record, drains bounded committed-route output,
...
- 2026-07-20: Extended endpoint registry pre-closed cleanup to deadline
  selection, cross-record output polling, and installed-key receive view
  construction. Closed records are retired before building deadline, poll, or
...
- 2026-07-21: Added scratch-backed server receive-to-pending-work route-bound
  drain. `Tls13ServerEndpoint.feedDatagramWithInstalledKeysAndProcessPendingWorkAndDrainDatagramsWithRoutePathWithScratch()`
...
- 2026-07-21: Added scratch-backed server route-bound installed-key receive
  poll. `Tls13ServerEndpoint.feedInstalledKeyDatagramWithRoutePathWithScratch()`
  preserves the existing path-update, stateless reset, and close-on-error
...
- 2026-07-20: Hardened route-bound explicit close drains with zero output
  capacity. Client and server endpoint close-and-drain helpers now return
  `BufferTooSmall` before mutating connection state when no output slot is
...
- 2026-07-20: Hardened server pending-work and receive-step recovery drains
...
- 2026-07-22: Hardened server ClientHello supported-version vector validation.
  Pure-Zig TLS server parsing now rejects duplicate `supported_versions`
  entries without committing parsed ClientHello state or transcript progress.
...
- 2026-07-17: Tightened STREAM/CRYPTO max-payload budgeting near the QUIC
  end-offset ceiling. The max payload helpers now cap search candidates by the
  remaining valid end-offset space instead of probing unsendable frame lengths.
...
- 2026-07-17: Tightened address-validation token version binding. Token
  issuance and version-specific validation now reject forbidden zero and
  reserved originating QUIC versions.
...
- 2026-07-16: Gated the pure-Zig `CryptoBackend` client 0-RTT write-secret
  ...
  ticket, or with a ticket that lacks QUIC's RFC 9001 sentinel, no longer
  installs client 0-RTT packet keys; tests now cover both denied cases and the
  allowed ticket path.

- 2026-07-16: Added the TLS 1.3 ServerHello `pre_shared_key` selection path
  for resumed handshakes. A server with a verified PSK binder now selects
  identity 0 in ServerHello, the client parses that selection, and the selected
  PSK handshake skips Certificate/CertificateVerify before completing with
  matching application traffic secrets.

- 2026-07-16: Tightened pure-Zig TLS PSK binder handling. A configured server
  now rejects a ClientHello whose PSK binder does not verify against the server
  PSK with `BadFinished`, instead of continuing the handshake with
  `peer_psk_binder_valid=false`. The focused test covers the mismatched-binder
  failure and verifies no 0-RTT receive secret is derived.

- 2026-07-17: Added direct malformed PSK binder-vector coverage. The pure-Zig
...
- 2026-07-19: Tightened the PSK-to-certificate fallback boundary. When a
...
- 2026-07-19: Bound client-side ServerHello `pre_shared_key` selection to the
  current ClientHello offer. The pure-Zig TLS client now rejects PSK selection
  when the current ClientHello did not advertise `pre_shared_key`, even if its
  local ticket/PSK store is populated before ServerHello processing.
...
- 2026-07-16: Added a routed datagram dispatcher to `Tls13ServerEndpoint`.
  ...
  through the existing route-bound processing path. The endpoint now also has a
  combined classify-and-process wrapper that leaves non-routed Initial, Version
  Negotiation, stateless reset, and dropped datagrams visible to callers while
  processing routed long/short packets behind the endpoint owner. A follow-up
  classify/process/bounded-drain wrapper now drains route-bound installed-key
  output after routed short-packet receive, including path-validation output
  and the post-drain recovery deadline. Tests cover routed Initial, Handshake,
  short-packet dispatch, routed short receive plus bounded drain, and non-routed
  Version Negotiation without editing examples.
  `receiveDatagramStepWithRoutePath()` now provides a shorter bounded server
  socket-loop step that runs classify/process/drain, sweeps endpoint-owned
  pending work, drains route-bound recovery output, and returns the next
  endpoint deadline. Tests cover a non-routed Version Negotiation receive while
  an existing record services recovery output in the same step.
  `Tls13ClientEndpoint.receiveDatagramStepWithRoutePath()` now gives the client
  endpoint the matching bounded receive/due-deadline step. Tests prove a
  non-closing route mismatch keeps receive output isolated while the same step
  drains route-bound Application recovery output and returns the next deadline.
...
- 2026-06-18: Added the direct installed-key 1-RTT short
  ...
  Unit coverage proves Application-space CRYPTO is authenticated and received
...
- 2026-06-18: Added the routed installed-key 1-RTT short
  ...
  before packet processing or backend callbacks; successful paths use the
  routed DCID length to process the short packet, apply peer Version
  Information through the compatible-version backend, and write installed-key
  output into the caller-provided bounded drain slice.
- 2026-06-18: Added
...
- 2026-06-18: Moved version-list matching and local RFC 9368
  ...
  authenticated extra-version selection, reserved chosen-version rejection, and
  inconsistent client follow-up config rejection.
...
- 2026-06-18: Added explicit key-update 1-RTT short
  ...
  `processRoutedProtectedShortDatagramWithKeyUpdateOrCloseAndDrainDatagrams()`.
  Unit coverage proves route selection stops mismatched connection handles
  before packet processing, successful routed next-key-phase PING receive polls
...
- 2026-06-18: Added
  ...
  and
  `processRoutedProtectedShortDatagramWithInstalledKeysAndDriveCryptoBackendInSpaceAndPollDatagram()`
  as installed-key 1-RTT receive-to-Application-backend-to-output loop steps.
  Unit coverage proves successful Application CRYPTO receive drives the
...
- 2026-06-18: Added
  ...
  and
  `processRoutedProtectedShortDatagramWithInstalledKeysAndDriveCryptoBackendInSpaceAndDrainDatagrams()`
  as installed-key 1-RTT receive-to-Application-backend-to-bounded-drain loop
  steps. Unit coverage proves successful Application CRYPTO receive drives the
  backend, backend-produced Application CRYPTO and ACK output can be written to
...
- 2026-06-10: Added
  ...
  The poll result now also returns the post-output Initial recovery deadline.
- 2026-06-10: Added
...
- 2026-06-10: Added `EndpointAcceptedInitialCryptoBackendDatagramDrainResult`,
  ...
  complete backend-produced CRYPTO bytes. The bounded-drain result now also
  returns the post-drain Initial recovery deadline.
- 2026-06-10: Added
...
- 2026-06-18: Added
  `EndpointConnectionLifecycle.processPendingWorkAcrossConnectionsAndDriveCryptoBackendsInSpaceAndPollDatagramWithInstalledKeyOptions()`
  and
  ...
  These carry explicit installed-key output through the no-new-datagram
  pending-work/backend tick. Unit coverage proves due accepted 0-RTT recovery
  work is serviced before backend progress and then emitted through the
  caller-selected 0-RTT poll or bounded-drain output path.
- 2026-06-18: Added
  `EndpointConnectionLifecycle.driveCryptoBackendsInSpaceOrCloseAndPollDatagramWithInstalledKeyOptions()`
  and
  `EndpointConnectionLifecycle.driveCryptoBackendsInSpaceOrCloseAndDrainDatagramsWithInstalledKeyOptions()`.
  These carry explicit installed-key output through the close-propagating
...
- 2026-06-18: Added
  ...
  caller-selected 0-RTT output is emitted through poll and bounded-drain forms.
- 2026-06-18: Added
  `EndpointConnectionLifecycle.driveCryptoBackendsInSpaceWithCompatibleVersionOrCloseAndPollDatagramWithInstalledKeyOptions()`
  and
  ...
  These preserve explicit installed-key output through successful
  close-propagating RFC 9368-compatible backend sweeps while retaining the
...
- 2026-06-24: Added the single-connection feed/backend explicit-output
  wrappers:
  ...
  feed one routed installed-key datagram, drive one backend, and still poll or
  drain caller-selected installed-key output views. Unit coverage proves routed
  Handshake input can drive the backend while explicit 0-RTT `RESET_STREAM`
  output comes from separate output views, and dropped datagrams do not drive
  the close/compatible variants.
...
- 2026-06-18: Added the close-propagating explicit-output variants
  ...
  These preserve caller-selected installed-key output space before OrClose
...
- 2026-06-18: Added
  ...
  as the single-connection no-output pending-work/backend/deadline step. Unit
  coverage proves due recovery work is serviced before backend drive and idle
  retirement stops before backend progress on the same connection.
- 2026-06-17: Added
...
- 2026-06-18: Added
  ...
  as the single-connection RFC 9368-compatible no-output pending-work/backend/deadline
  step. Unit coverage proves compatible Version Information is applied after
  due recovery work, and same-connection idle retirement stops before backend
  progress.
- 2026-06-17: Added
...
- 2026-06-24: Added
  ...
  and `EndpointConnectionLifecycle.feedDatagramWithInstalledKeysAndProcessPendingWorkAndDriveCryptoBackendInSpaceWithCompatibleVersionOrCloseAndSelectNextDeadline()`
  as RFC 9368-compatible receive-to-pending-work-to-backend-to-next-deadline
  socket-loop steps. Unit coverage proves routed Handshake input can process
  pending work before applying peer Version Information through the backend,
  selecting the compatible version, applying peer transport parameters, and
  returning the next endpoint deadline.
- 2026-06-24: Added
  ...
  and `EndpointConnectionLifecycle.feedDatagramWithInstalledKeysAndProcessPendingWorkAndDriveCryptoBackendInSpaceOrCloseAndSelectNextDeadline()`
  as close-propagating receive-to-pending-work-to-backend-to-next-deadline
  socket-loop steps. Unit coverage proves routed Handshake input can queue
  CONNECTION_CLOSE for invalid peer transport parameters and return the backend
  error before producing a deadline result.
- 2026-06-24: Added
  ...
  and `EndpointConnectionLifecycle.feedDatagramWithInstalledKeysAndProcessPendingWorkAndDriveCryptoBackendInSpaceOrCloseAndDrainDatagrams()`
  as close-propagating receive-to-pending-work-to-backend-to-output socket-loop
  steps. Unit coverage proves backend peer transport-parameter errors queue
  CONNECTION_CLOSE before output polling, while successful explicit output
  options still poll and drain protected Handshake responses.
- 2026-06-24: Added
...
- 2026-06-24: Added
  ...
  and `EndpointConnectionLifecycle.feedDatagramWithInstalledKeysAndProcessPendingWorkAndDriveCryptoBackendInSpaceAndDrainDatagrams()`
...
- 2026-06-24: Added
  ...
  and `EndpointConnectionLifecycle.feedDatagramWithInstalledKeysAndProcessPendingWorkAndDriveCryptoBackendInSpaceWithCompatibleVersionOrCloseAndDrainDatagrams()`
  as RFC 9368-compatible receive-to-pending-work-to-backend-to-output socket-loop
  steps. Unit coverage proves routed Handshake input can run pending work, apply
  peer Version Information through the backend, preserve explicit installed-key
  output options, and poll/drain protected 0-RTT output; the OrClose drain path
  keeps the same success contract.
- 2026-06-24: Added the single-connection explicit-output forms
  ...
  and the matching compatible-version forms. These wrappers let a simple
  socket loop feed one routed installed-key datagram, process pending work,
  drive one caller-owned backend, and still poll or drain caller-selected
  installed-key output views. Unit coverage proves routed Handshake input
  drives the backend while explicit 0-RTT `RESET_STREAM` output comes from the
  caller-selected output view, and dropped datagrams do not drive any variant.
...
- 2026-07-03: Added cross-space RFC 9368-compatible lifecycle no-output
  ...
  They compose the existing pending-work and due-deadline ownership gates with
  the cross-space compatible backend-drive primitive. Unit coverage proves
  single-connection and cross-connection deadline selection after compatible
  cross-space backend progress, plus before-deadline no-op.
...
- 2026-06-18: Added
  ...
  These carry per-connection installed-key output options through the direct
  routed receive-to-output socket-loop step. Unit coverage proves a routed
...
- 2026-06-18: Added
  ...
  These carry per-connection installed-key output options through the routed
  receive-to-backend loop step. Unit coverage proves a routed Handshake CRYPTO
  datagram still drives backend input while caller-selected 0-RTT output is
  emitted through poll and bounded-drain forms from caller-owned connections.
- 2026-06-18: Added the close-propagating explicit-output variants
  `EndpointConnectionLifecycle.feedDatagramWithInstalledKeysAcrossConnectionsAndDriveCryptoBackendsInSpaceOrCloseAndPollDatagramWithInstalledKeyOptions()`
  and
  ...
  receive-to-backend loop steps while preserving OrClose backend behavior.
...
- 2026-06-04: Added a narrow C-ABI `TlsBackend` adapter that converts
  ...
  remains pending.
- 2026-06-18: Tightened the C-ABI `TlsBackend` adapter status boundary. Pull-style
  callbacks still use `pending` to mean no bytes or secrets are currently
  available, while input callbacks that must consume data in the current drive
  step now reject `pending` as an adapter protocol error. Unit coverage proves
  `receive` and local transport-parameter setup no longer silently succeed on
  `pending`, while CRYPTO output pull keeps the no-output behavior.
- 2026-06-18: Added the matching output-buffer status guard for the C-ABI
  `TlsBackend` adapter. Pull-style callbacks that return `pending` must now
...
- 2026-05-29: Added RFC 9002 max_datagram_size recovery resync from peer
  ...
  cleared. If the peer limit grows after the window was reduced below the new
  minimum, recovery raises the congestion window to the recalculated minimum and
  clears stale congestion-avoidance credit. Tests cover direct recovery decrease/increase behavior and
...
- 2026-07-17: Added client endpoint receive-step active-reset coverage.
  `Tls13ClientEndpoint.receiveDatagramStepWithRoutePath()` now has direct test
  evidence that an authenticated active-route stateless reset remains visible
  in the receive result, disarms endpoint recovery timers, leaves the route
  alive until the drain timeout, and reports the post-reset close deadline.
- 2026-07-17: Added server endpoint receive-step active-reset coverage.
  `Tls13ServerEndpoint.receiveDatagramStepWithRoutePath()` now has direct test
  evidence that an authenticated active-route stateless reset remains visible
  through the socket-loop step, disarms the recovery timer, keeps the record
  routed until close timeout, and returns the post-reset close deadline.
...
- 2026-05-23: Added idempotent receive handling for duplicate STREAM
  ...
  new suffix bytes, exact duplicate pending ranges are ignored, same-byte
  prefix/suffix overlaps against pending ranges are trimmed, same-byte middle
  overlaps are split into the remaining new gaps, and pending-covered subranges
  are ignored without growing receive flow-control accounting.
  Changed bytes still fail as invalid payloads. Tests cover contiguous
  duplicates, suffix trimming, pending duplicate ranges, pending prefix/suffix
  overlap trimming, pending middle-overlap splitting, conflicting overlap
  rejection, and rollback;
  `examples/uni_stream.zig` demonstrates duplicate retransmission discard on a
  client-initiated unidirectional stream.
- 2026-05-23: Added Data Recvd late-STREAM discard. Once all bytes through the
  final size are buffered, later STREAM frames that repeat the same bytes
  inside that final size are ignored without growing flow-control accounting.
  Conflicting late STREAM bytes now fail with a stream-data protocol violation;
  tests cover duplicate discard, conflicting late data after FIN, OrClose close
  classification, and final-size violation preservation.
...
- 2026-05-23: Added out-of-order CRYPTO receive buffering per packet number
  ...
  pending bytes are ignored, same-byte pending overlaps are split into the
  remaining new gaps, and conflicting overlaps still fail as invalid payloads
  with rollback. Tests cover out-of-order delivery, duplicate contiguous and
  pending data, pending middle-overlap splitting, conflict rejection, and
  pending-state rollback;
...
| `tls_backend_adapter` | Runnable C-ABI `TlsBackend` adapter contract check: local/peer transport-parameter byte handoff, inbound/outbound CRYPTO byte delivery, input-callback status rejection, output-buffer pending/byte consistency, Handshake traffic-secret installation, and handshake confirmation through the existing `CryptoBackend` drive path before binding a concrete C TLS library. | Present |
