...
| RFC 9000 transport protocol | Partial | Frame codec with ACK/ACK_ECN range validation, transport parameters, connection state, streams, flow control, connection IDs, Retry/tokens, path validation, close/reset behavior, endpoint idle route/timer cleanup, endpoint routing helpers, receive-side local UDP datagram size rejection before protected packet decryption/state updates, lifecycle-owned caller-keyed protected UDP packet loopback, socket-backed UDP path-validation route-update loopback, socket-backed UDP lifecycle Retry/address-validation loopback, and examples. | Full protected/TLS socket-backed client/server loopback, complete endpoint lifecycle, and external interop. |
