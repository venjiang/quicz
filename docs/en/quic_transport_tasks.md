...
| ACK/loss/PTO recovery | Required. ACK processing, packet/time-threshold loss, PTO, retransmission, and timer service must drive the endpoint loop. | Partial: deterministic recovery model and socket-backed loopbacks exist; a bounded Zig server also completes external PTO recovery, while production lifecycle integration is incomplete. |
