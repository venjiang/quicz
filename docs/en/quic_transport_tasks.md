...
| RFC 9002 loss detection and congestion control | Partial | ACK delay scaling/capping, packet/time-threshold loss, ACK-driven frame-payload STREAM/CRYPTO, protected CRYPTO, and protected 0-RTT STREAM retransmission requeue, NewReno recovery-period behavior, persistent congestion, packet-space PTO PING/new-data/in-flight-CRYPTO/protected-0-RTT-STREAM/in-flight-STREAM probe hooks with Initial/Handshake max_ack_delay suppression, ECN validation tests, and socket-backed UDP loss-recovery/congestion-recovery/PTO-recovery/STREAM-retransmission loopbacks. | Full protected-packet PTO/loss timer scheduling, remaining 0-RTT RESET_STREAM/STOP_SENDING retransmission coverage, and remaining NewReno details with controlled-clock tests. |
...
| RFC 9002 recovery | Partial ACK delay + packet/time-threshold loss + ACK-driven frame-payload STREAM/CRYPTO, protected CRYPTO, and protected 0-RTT STREAM retransmission requeue + NewReno recovery period + persistent congestion + packet-space PTO PING/new-data/in-flight-CRYPTO/protected-0-RTT-STREAM/in-flight-STREAM probe hook with Initial/Handshake max_ack_delay suppression + socket-backed UDP loss-recovery, congestion-recovery, PTO-recovery, and STREAM-retransmission loopbacks | Implement full protected-packet PTO/loss timer scheduling, remaining 0-RTT RESET_STREAM/STOP_SENDING retransmission coverage, and remaining NewReno details. | Existing tests cover ACK, ACK delay exponent scaling, post-confirmation max_ack_delay capping, packet-threshold loss, ACK-driven and timeout-driven time-threshold loss, ACK-driven lost STREAM, CRYPTO, protected CRYPTO, and protected 0-RTT STREAM requeue, invalid-payload rollback after retransmission requeue, NewReno recovery-period suppression, persistent congestion, packet-number-space PTO PING queuing/backoff, Initial/Handshake PTO deadline calculation without max_ack_delay, queued STREAM data probe selection, PTO-driven in-flight CRYPTO/STREAM/protected-0-RTT-STREAM retransmission, and congestion-window arithmetic; `run-crypto-stream` covers frame-payload Handshake CRYPTO loss requeue/retransmission and protected 1-RTT CRYPTO ACK-loss requeue/retransmission; `run-pto-recovery` covers protected 1-RTT CRYPTO PTO probe selection; `run-udp-loss-recovery-loopback` covers protected short PING delivery over UDP, protected ACK-driven packet-threshold loss removal, and timeout-driven time-threshold cleanup; `run-udp-congestion-recovery-loopback` covers protected short PING delivery over UDP, repeated-loss suppression inside the NewReno recovery period, and persistent congestion reduction to the minimum window; `run-udp-pto-recovery-loopback` covers ACK-loss-driven PTO over protected UDP packets, PING fallback probe delivery, queued STREAM data as PTO probe, in-flight STREAM and CRYPTO data as PTO probes, duplicate receive/CRYPTO range discard, and ACK cleanup; `run-udp-stream-retransmission-loopback` covers ACK-driven 1-RTT STREAM retransmission over protected UDP packets and final ACK cleanup; `packet_spaces` covers protected 0-RTT STREAM ACK-loss requeue and retransmission under a new packet number. Remaining controlled-clock tests cover full protected-packet PTO/loss timer scheduling and 0-RTT RESET_STREAM/STOP_SENDING retransmission. |
...
- 2026-05-27: Added protected 0-RTT STREAM retransmission coverage. Successful
  ...
  protected 0-RTT STREAM retransmission. Remaining work is full
  protected-packet PTO/loss timer scheduling, 0-RTT RESET_STREAM/STOP_SENDING
  retransmission, and remaining NewReno details.
- 2026-05-26: Added ACK-driven frame-payload CRYPTO retransmission requeue.
  ...
  retransmission. Later updates covered protected CRYPTO loss/PTO and protected
  0-RTT STREAM retransmission. Remaining work is protected-packet timer
  scheduling and 0-RTT RESET_STREAM/STOP_SENDING retransmission coverage.
- 2026-05-26: Extended the PTO probe hook to reuse in-flight Application
  ...
  protected CRYPTO and protected 0-RTT STREAM retransmission are now covered;
  0-RTT RESET_STREAM/STOP_SENDING retransmission and full protected-packet timer
...
- 2026-05-26: Added ACK-driven STREAM retransmission requeue for lost
  ...
  STREAM retransmission are now covered; 0-RTT RESET_STREAM/STOP_SENDING
  retransmission and full protected-packet timer scheduling remain pending.
- 2026-05-26: Added `examples/udp_congestion_recovery_loopback.zig` and the
  ...
  CRYPTO retransmission and protected 0-RTT STREAM retransmission are now
  covered separately; 0-RTT RESET_STREAM/STOP_SENDING retransmission remains
  pending.
- 2026-05-26: Added `examples/udp_loss_recovery_loopback.zig` and the
  ...
  protected CRYPTO and protected 0-RTT STREAM retransmission are now covered;
  0-RTT RESET_STREAM/STOP_SENDING retransmission remains pending.
- 2026-05-26: Added `examples/udp_pto_recovery_loopback.zig` and the
  ...
  newer queued data. Protected CRYPTO and protected 0-RTT STREAM retransmission
  are now covered; 0-RTT RESET_STREAM/STOP_SENDING retransmission remains pending.
...
- 2026-05-23: Made `checkPtoTimeouts()` prefer already queued ack-eliciting
  ...
  ACK-lost and PTO-probed 1-RTT STREAM data, protected CRYPTO data, and
  protected 0-RTT STREAM data plus ACK-lost frame-payload CRYPTO data. 0-RTT
  RESET_STREAM/STOP_SENDING retransmission remains pending.
...
| `packet_spaces` | Current frame-payload Initial/Handshake/Application ACK/recovery isolation, RFC 9001 Initial discard, Initial/Handshake discard cleanup, 0-RTT packet-type filtering, and caller-keyed protected 0-RTT STREAM delivery plus ACK-loss retransmission. | Present |
...
| `pto_recovery` | Current frame-payload Initial/Handshake/Application PTO hooks, including Initial/Handshake max_ack_delay suppression, PING fallback probes, queued STREAM data probe selection, and in-flight STREAM retransmission probe selection. | Present |
