...
- 2026-06-04: Extended `examples/tls_openssl_backend_adapter.zig` so the
  ...
  transcript secrets. After the echo, the same lifecycle owners deliver a
  protected close and clean up the client/server routes when close/drain
  timeouts expire. Full endpoint-owned live TLS handshake/socket loop remains
  pending.
