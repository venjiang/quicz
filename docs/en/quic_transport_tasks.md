...
| `udp_endpoint_loopback` | Socket-backed loopback UDP exercise for endpoint routing: lifecycle-owned unsupported-version Initial to Version Negotiation response delivery, client-side VN selection, protected follow-up Initial emission, supported Initial accept classification, server-side follow-up Initial CRYPTO receive, client Initial Source CID response routing, accepted server Initial Source CID registration, and short-header registered-CID routing. | Present |
