...
- 2026-06-04: Extended `examples/tls_openssl_backend_adapter.zig` so the
  ...
  inbound CRYPTO through OpenSSL callback boundaries, then verifies the
  adapter-installed client 1-RTT keys can protect a short packet that a peer
  connection decrypts and ACKs with matching transcript secrets. Full
  endpoint-owned live TLS handshake/socket loop remains pending.
...
| `tls_openssl_backend_adapter` | Runnable OpenSSL-backed `TlsBackend` wrapper check: the existing drive path sets quicz local transport parameters on an OpenSSL TLS object, drives `SSL_do_handshake()` to emit the first TLS CRYPTO flight, delivers peer transport parameters, real pair-transcript Handshake/1-RTT secrets, and Handshake CRYPTO bytes through OpenSSL callback boundaries, then verifies adapter-installed client 1-RTT keys can protect a short packet that a peer connection decrypts and ACKs with matching transcript secrets. | Present |
