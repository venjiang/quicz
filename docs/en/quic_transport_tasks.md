...
- 2026-06-04: Extended `examples/tls_openssl_backend_adapter.zig` so the
  ...
  transcript secrets. The Initial, Handshake, Application echo, protected
  close, and close/drain timeout cleanup now share one socket/lifecycle loop
  owner; the example prints `adapter_endpoint_routes=3/4/0/0` after retiring
