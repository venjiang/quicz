...
- 2026-06-05: Added `examples/tls_openssl_pair_transcript.zig` plus a small C
  ...
  helpers and verifies server-side delivery, then installs OpenSSL-produced
  Handshake secrets into quicz and verifies protected Handshake CRYPTO delivery
  in both directions, including loopback UDP delivery through the quicz endpoint
  lifecycle. It also installs OpenSSL-produced 1-RTT secrets and verifies an
  installed-key protected STREAM request/response over short packets, then
  drives a loopback UDP STREAM echo through the same lifecycle with those 1-RTT
  secrets. Full endpoint-owned live TLS handshake/socket loop remains pending.
- 2026-06-04: Extended `examples/tls_openssl_backend_adapter.zig` so the
  ...
  OpenSSL callback boundaries, and intentionally leaves full peer transcript
  driving pending.
