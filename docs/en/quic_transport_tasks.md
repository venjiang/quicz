...
| --- | --- | --- |
| UDP client/server endpoint | Required for first usable milestone. One endpoint owner must drive accept/connect, packet receive/send, timers, route cleanup, and close. | Partial: `Tls13ServerEndpoint` owns bounded TLS server records, UDP ingress classification, lifecycle routing/timers, record-owned TLS backend, Retry/Initial-key state, and output-header CIDs, protected long-header input and TLS driving, owned-record 1-RTT output polling, due-timer service, next-wakeup selection, rollback-safe Initial/Retry record admission, and atomic record retirement; process client/server UDP receive capacity matches each configured local datagram limit. Callers retain UDP I/O and admission policy; production client/server event-loop policy is still missing. |
