...
| --- | --- | --- |
| UDP client/server endpoint | Required for first usable milestone. One endpoint owner must drive accept/connect, packet receive/send, timers, route cleanup, and close. | Partial: `Tls13ServerEndpoint` owns bounded TLS server records, ingress classification, lifecycle routing/timers, TLS backend state, and atomic record retirement; `Tls13ClientEndpoint` owns one client transport, CID route, recovery-timer mirror, and protected close emission/retirement. Process client/server UDP receive capacity matches each configured local datagram limit. Callers retain UDP I/O and admission policy; production multi-connection event-loop policy is still missing. |
...
drives retained coalesced Initial/Handshake input and its Handshake backend
output on the same record boundary; accepted/routed Initial follow-up,
Retry follow-up, and routed Initial plus Handshake input retain bounded
protected-output draining. This is
...
registered routes and lifecycle timer refresh. The client endpoint delegates
