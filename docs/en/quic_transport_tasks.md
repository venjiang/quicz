...
- 2026-06-05: Added `examples/tls_openssl_pair_transcript.zig` plus a small C
  ...
  traffic-secret callbacks on both endpoints. This proves the mature C TLS
  library can drive the transcript outside quicz packetization; wiring those
  level-separated CRYPTO bytes into quicz packet number spaces is still pending.
...
| `tls_openssl_pair_transcript` | Runnable OpenSSL client/server callback-mode transcript check: a fixed example PSK avoids certificate fixture noise, CRYPTO bytes are routed by OpenSSL protection level, both endpoints complete TLS 1.3 without alerts, and peer transport-parameter plus Handshake/1-RTT secret callbacks fire on both endpoints. | Present |
