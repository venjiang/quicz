...
| 0-RTT | Schedule after the first 1-RTT stream echo interop gate; not required for the current milestone. | Partial: explicit accept/reject, mock installed-key 0-RTT paths, NewSessionTicket RFC 9001 sentinel validation, and ticket-gated ClientHello `early_data` emission exist; real TLS replay policy is missing. |
...
when the ClientHello actually offers `early_data`. Full 0-RTT replay and
ticket-age policy remains pending.
