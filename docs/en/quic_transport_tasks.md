...
- 2026-06-05: Migrated the current C TLS example boundary to Zig 0.16's
  ...
  parameters plus Handshake and 1-RTT secrets are available, so
...
- 2026-06-04: Extended `examples/tls_openssl_backend_adapter.zig` so the
  ...
  re-delivering duplicate stream data. After the OpenSSL-backed
  ...
  loopback server side is still explicitly discarded in the example.
