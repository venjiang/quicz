...
| Flow control | Required. Connection, stream, and stream-count flow control must work over protected UDP. | Partial: frame-payload and protected loopback coverage exists; protected 1-RTT STREAM writes re-split against their exact packet budget, including ACK space; a quic-go client completes a 12 KiB echo across the Zig server's 2 KiB stream and 8 KiB connection initial windows. |
