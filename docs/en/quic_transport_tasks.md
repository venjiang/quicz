...
The client endpoint now also has direct no-input due-deadline coverage for
`.key_discard`: a socket loop can service the retained-key expiry without first
receiving a datagram, without emitting recovery output, and without retiring
the committed route.
