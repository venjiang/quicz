...
| RFC 9002 loss detection and congestion control | Partial | ACK delay scaling/capping, packet/time-threshold loss, NewReno recovery-period behavior, persistent congestion, packet-space PTO PING/new-data probe hooks with Initial/Handshake max_ack_delay suppression, ECN validation tests, and socket-backed UDP PTO recovery loopback. | Full protected-packet PTO/loss timer scheduling and retransmission behavior with controlled-clock tests. |
