...
| `loss_recovery` | Current frame-payload packet-threshold loss, time-threshold loss, aggregate loss-time timer service, NewReno slow-start/congestion-avoidance growth, recovery period, new-congestion-event one-packet STREAM recovery probe, minimum-window ssthresh clamp, PTO-backoff-independent persistent congestion, and ACK-delay handling. | Present |
