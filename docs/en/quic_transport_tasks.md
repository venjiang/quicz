...
- 2026-06-05: Added `examples/tls_openssl_pair_transcript.zig` plus a small C
  ...
  traffic-secret callbacks on both endpoints. The Zig side now copies those
  generated CRYPTO bytes into quicz Initial/Handshake/Application frame-payload
  CRYPTO queues and reads them back by packet number space. It also packetizes
...
- 2026-06-04: Extended `examples/tls_openssl_backend_adapter.zig` so the
  ...
  transcript secrets. After modeled handshake confirmation, it explicitly
  discards both Handshake packet-number spaces and Handshake keys. The Initial,
  Handshake, Application echo, protected close, and close/drain timeout cleanup
  now share one socket/lifecycle loop owner; the example prints
