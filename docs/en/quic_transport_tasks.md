...
| Spin bit | Configurable single-path short-header spin-bit state + protected spin-bit peek + socket-backed UDP spin-bit loopback | Keep default-disabled behavior and later bind reset/update ownership to real endpoint path lifecycle. | Existing tests cover enabled/disabled spin-bit updates and invalid-packet preservation; `run-udp-spin-bit-loopback` proves first false spin, second true spin, ACK cleanup, and path reset over real loopback UDP sockets. |
...
| ECN validation | Partial frame-payload ACK_ECN validation + ACK_ECN CE-driven NewReno recovery response + endpoint UDP-path ECN state policy + socket-backed UDP ACK_ECN validation/CE response loopback | Bind ECN validation to real IP ECN marking once socket packetization exposes packet ECN marks. | Existing tests cover ECT(0) success, CE-counter congestion response, repeated CE suppression inside the NewReno recovery period, missing ACK_ECN failure, insufficient counters, counter totals exceeding sent ECT packets, reordered ACK handling, rollback, and endpoint path-identity state isolation; `run-udp-ecn-validation-loopback` covers modeled ECT(0) protected PING delivery, protected ACK_ECN success, ECN-CE ACK_ECN congestion response, endpoint ECN state update for the current UDP tuple, and migrated-path isolation without claiming real IP-header ECN marking. |
...
- 2026-05-26: Added `examples/udp_ecn_validation_loopback.zig` and the
  ...
  validates the client's ECN state as `capable`, writes that state into
  `endpoint.EcnPathPolicy` for the active UDP tuple, and proves a migrated
...
- 2026-05-26: Added `examples/udp_spin_bit_loopback.zig` and the
  ...
  single-path spin-bit policy on both endpoints, sends two protected short
  PING/ACK exchanges over loopback UDP, verifies the first round stays
  `spin=false`, verifies the second round carries `spin=true`, and proves path
  reset clears the next outgoing spin bit.
...
| `udp_spin_bit_loopback` | Socket-backed loopback UDP spin-bit exercise: enabled single-path spin-bit signaling, protected short PING/ACK routing, first false spin round, second true spin round, final ACK cleanup, and path reset. | Present |
| `udp_ecn_validation_loopback` | Socket-backed loopback UDP ECN validation exercise: modeled ECT(0) protected short PING routing, protected ACK_ECN success, ACK_ECN CE-driven NewReno recovery response, endpoint ECN state update for the active UDP tuple, and migrated-path ECN isolation without claiming real IP-header ECN marking. | Present |
