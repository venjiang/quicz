...
| TLS 1.3 integration | Required. Pure-Zig TLS 1.3 (`src/quic/tls13.zig` + `src/quic/tls13_backend.zig`) exposed through the `CryptoBackend` interface; the OpenSSL C adapter is the deprecated path. | Partial: `Tls13ClientTransport` owns protected client I/O and selects/services recovery, idle, close, and key-discard deadlines; each concurrent server record owns `Tls13ServerTransport` (Connection + TLS backend) and uses its stream boundary. The `quic-go` proof below completes a real TLS-owned handshake and two FIN-terminated 1-RTT STREAM echoes without mock keys or OpenSSL; a production endpoint-owned event loop remains missing. |
