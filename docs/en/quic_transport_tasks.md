...
| RFC 9002 loss detection and congestion control | Partial | ACK delay scaling/capping, packet/time-threshold loss, aggregate loss-time-before-PTO timer deadline selection, ACK-driven frame-payload STREAM/CRYPTO, protected CRYPTO, and protected 0-RTT STREAM/RESET_STREAM/STOP_SENDING retransmission requeue, NewReno recovery-period behavior, persistent congestion, packet-space PTO PING/new-data/in-flight-CRYPTO/protected-0-RTT-control/protected-0-RTT-STREAM/in-flight-STREAM probe hooks with Initial/Handshake max_ack_delay suppression, ECN validation tests, and socket-backed UDP loss-recovery/congestion-recovery/PTO-recovery/STREAM-retransmission loopbacks. | Endpoint-owned protected-packet loss/PTO timer integration and remaining NewReno details with controlled-clock tests. |
...
- 2026-05-27: Added aggregate RFC 9002 loss detection timer deadline selection.
  `lossDetectionTimerDeadlineMillis()` now reports the earliest timer cause
  across packet number spaces, gives pending loss-time deadlines precedence over
  PTO, and otherwise selects the earliest PTO deadline. Tests cover loss-time
  precedence over PTO, earliest-space PTO selection, and protected short CRYPTO
  retransmission when the selected loss-time deadline expires. `loss_recovery`
  demonstrates aggregate loss-time selection, and `pto_recovery` demonstrates
  aggregate PTO selection. Remaining work is endpoint-owned protected-packet
  loss/PTO timer integration and remaining NewReno details.
...
- 2026-05-26: Added `examples/udp_loss_recovery_loopback.zig` and the
  ...
  and timeout-driven time-threshold loss cleanup through the real UDP routing
  path. ACK-driven and PTO-driven 1-RTT STREAM retransmission plus ACK-driven
...
- 2026-05-26: Added `examples/udp_pto_recovery_loopback.zig` and the
  ...
  protected PING over loopback UDP, withholds the ACK, drives the controlled PTO
  deadline to send and deliver a protected PING fallback probe, then ACKs both
  packets. It also queues new STREAM data after an unacked STREAM packet and
  proves the PTO hook sends that queued data as the protected probe before ACK
...
| `udp_loss_recovery_loopback` | Socket-backed loopback UDP loss-recovery exercise: protected short PING routing, protected ACK-driven packet-threshold loss, and timeout-driven time-threshold cleanup. | Present |
...
| `udp_pto_recovery_loopback` | Socket-backed loopback UDP PTO recovery exercise: ACK-loss PTO deadline, protected PING fallback probe delivery, queued STREAM data as a protected PTO probe, in-flight STREAM data as a protected PTO probe, duplicate receive discard, and final ACK cleanup. | Present |
...
| `loss_recovery` | Current frame-payload packet-threshold loss, time-threshold loss, aggregate loss-time timer deadline selection, NewReno recovery period, persistent congestion, and ACK-delay handling. | Present |
| `pto_recovery` | Current frame-payload Initial/Handshake/Application PTO hooks, including aggregate PTO timer deadline selection, Initial/Handshake max_ack_delay suppression, PING fallback probes, queued STREAM data probe selection, in-flight STREAM retransmission probe selection, and protected 1-RTT CRYPTO PTO probe selection. | Present |
