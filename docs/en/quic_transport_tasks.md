...
| TLS 1.3 integration | Required. Pure-Zig TLS 1.3 (`src/quic/tls13.zig` + `src/quic/tls13_backend.zig`) exposed through the `CryptoBackend` interface; the OpenSSL C adapter is the deprecated path. | Partial: `Tls13ClientTransport` owns protected ClientHello, Retry retry-Initial, Initial/Handshake receive, peer Finished, bidirectional stream I/O, 1-RTT output, and PTO service for the certificate-verified external client. The `quic-go` proof below completes a real TLS-owned handshake and two FIN-terminated 1-RTT STREAM echoes without mock keys or OpenSSL; a production endpoint-owned event loop remains missing. |
...
| Streams | Required. Bidirectional and unidirectional stream open, read, write, FIN, reset, STOP_SENDING, and stream limits must work over protected UDP. | Partial: `Tls13ClientTransport` exposes bidirectional protected-UDP open/write/read/FIN and the independent `quic-go` echo proves two streams. Unidirectional, reset, STOP_SENDING, and stream-limit API coverage remains incomplete. |
