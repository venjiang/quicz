...
| RFC 9002 loss detection and congestion control | Partial | ACK delay scaling/capping, packet/time-threshold loss, NewReno recovery-period behavior, persistent congestion, packet-space PTO PING/new-data probe hooks with Initial/Handshake max_ack_delay suppression, ECN validation tests, and socket-backed UDP loss-recovery/congestion-recovery/PTO-recovery loopbacks. | Full protected-packet PTO/loss timer scheduling and retransmission behavior with controlled-clock tests. |
...
- 2026-05-26: Added ACK-driven STREAM retransmission requeue for lost
  ...
  receive discard, and final ACK cleanup. 0-RTT/CRYPTO and PTO-driven protected
  data retransmission remain pending.
- 2026-05-26: Added `examples/udp_congestion_recovery_loopback.zig` and the
  ...
  1-RTT STREAM retransmission is now covered separately; 0-RTT/CRYPTO and
  PTO-driven protected data retransmission remain pending.
- 2026-05-26: Added `examples/udp_loss_recovery_loopback.zig` and the
  ...
  path. ACK-driven 1-RTT STREAM retransmission is now covered separately;
  0-RTT/CRYPTO and PTO-driven protected data retransmission remain pending.
- 2026-05-26: Added `examples/udp_pto_recovery_loopback.zig` and the
  ...
  cleanup. ACK-driven 1-RTT STREAM retransmission is now covered separately;
  0-RTT/CRYPTO and PTO-driven protected data retransmission remain pending.
...
- 2026-05-22: Added `ptoDeadlineMillis()` and `checkPtoTimeouts()` for
  ...
  applies per-space PTO backoff. PTO-driven protected-packet data
...
- 2026-05-23: Made `checkPtoTimeouts()` prefer already queued ack-eliciting
  ...
  probe selection without adding a PING; cloning of ACK-lost 1-RTT STREAM data
  is now covered, while 0-RTT/CRYPTO and PTO-driven protected data
  retransmission remain pending.
...
| `udp_pto_recovery_loopback` | Socket-backed loopback UDP PTO recovery exercise: ACK-loss PTO deadline, protected PING fallback probe delivery, queued STREAM data as a protected PTO probe, and final ACK cleanup. | Present |
