...
| --- | --- | --- |
| UDP client/server endpoint | Required for first usable milestone. One endpoint owner must drive accept/connect, packet receive/send, timers, route cleanup, and close. | Partial: `Tls13ServerEndpoint` now owns bounded TLS server records plus lifecycle routing/timers while callers retain UDP I/O and application policy. Socket-backed loopback examples and core socket-loop entrypoints cover receive, accepted Initial backend response/close propagation, bounded long-header CRYPTO output draining, pending-work, due-deadline, TLS-backend drive, explicit per-connection installed-key output polling/draining, close propagation, compatible-version backend drive, cross-space backend drive, and wakeup selection; production client/server event loop policy is still missing. |
