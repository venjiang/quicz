...
| `stream_reset` | Current local RESET_STREAM emission, public stream-state snapshot evidence including reset-read observation, final-size observability, unsent STREAM drop behavior, ignored MAX_STREAM_DATA after reset, and late STREAM ignore after reset. | Present |
