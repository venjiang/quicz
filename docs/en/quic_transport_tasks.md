...
| --- | --- | --- |
| UDP client/server endpoint | Required for first usable milestone. One endpoint owner must drive accept/connect, packet receive/send, timers, route cleanup, and close. | Partial: `Tls13ServerEndpoint` now owns bounded TLS server records plus lifecycle routing/timers, and directly provides protected receive, due-timer service, and next-wakeup selection while callers retain UDP I/O and application policy. Socket-backed loopback examples cover accepted Initial backend response/close propagation, bounded long-header CRYPTO output draining, pending-work, TLS-backend drive, close propagation, compatible-version and cross-space work; production client/server event loop policy is still missing. |
