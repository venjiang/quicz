...
| RFC 9002 loss detection and congestion control | Partial | ACK delay scaling/capping, packet/time-threshold loss, ACK-driven frame-payload STREAM/CRYPTO retransmission requeue, NewReno recovery-period behavior, persistent congestion, packet-space PTO PING/new-data/in-flight-STREAM probe hooks with Initial/Handshake max_ack_delay suppression, ECN validation tests, and socket-backed UDP loss-recovery/congestion-recovery/PTO-recovery/STREAM-retransmission loopbacks. | Full protected-packet PTO/loss timer scheduling, remaining 0-RTT/protected-CRYPTO retransmission coverage, and remaining NewReno details with controlled-clock tests. |
...
| RFC 9002 recovery | Partial ACK delay + packet/time-threshold loss + ACK-driven frame-payload STREAM/CRYPTO retransmission requeue + NewReno recovery period + persistent congestion + packet-space PTO PING/new-data/in-flight-STREAM probe hook with Initial/Handshake max_ack_delay suppression + socket-backed UDP loss-recovery, congestion-recovery, PTO-recovery, and STREAM-retransmission loopbacks | Implement full protected-packet PTO/loss timer scheduling, remaining 0-RTT/protected-CRYPTO retransmission coverage, and remaining NewReno details. | Existing tests cover ACK, ACK delay exponent scaling, post-confirmation max_ack_delay capping, packet-threshold loss, ACK-driven and timeout-driven time-threshold loss, ACK-driven lost STREAM and CRYPTO requeue, invalid-payload rollback after retransmission requeue, NewReno recovery-period suppression, persistent congestion, packet-number-space PTO PING queuing/backoff, Initial/Handshake PTO deadline calculation without max_ack_delay, queued STREAM data probe selection, PTO-driven in-flight STREAM retransmission, and congestion-window arithmetic; `run-crypto-stream` covers frame-payload Handshake CRYPTO loss requeue and retransmission; `run-udp-loss-recovery-loopback` covers protected short PING delivery over UDP, protected ACK-driven packet-threshold loss removal, and timeout-driven time-threshold cleanup; `run-udp-congestion-recovery-loopback` covers protected short PING delivery over UDP, repeated-loss suppression inside the NewReno recovery period, and persistent congestion reduction to the minimum window; `run-udp-pto-recovery-loopback` covers ACK-loss-driven PTO over protected UDP packets, PING fallback probe delivery, queued STREAM data as PTO probe, in-flight STREAM data as PTO probe, and ACK cleanup; `run-udp-stream-retransmission-loopback` covers ACK-driven 1-RTT STREAM retransmission over protected UDP packets and final ACK cleanup. Remaining controlled-clock tests cover full protected-packet PTO/loss timer scheduling and 0-RTT/protected-CRYPTO retransmission. |
...
- 2026-05-26: Added ACK-driven frame-payload CRYPTO retransmission requeue.
  ...
  retransmission. Remaining work is protected-packet CRYPTO loss/PTO timer
  scheduling and 0-RTT retransmission coverage.
- 2026-05-26: Extended the PTO probe hook to reuse in-flight Application
  ...
  0-RTT/protected-CRYPTO retransmission and full protected-packet timer scheduling remain
  pending.
- 2026-05-26: Added ACK-driven STREAM retransmission requeue for lost
  ...
  receive discard, and final ACK cleanup. 0-RTT/protected-CRYPTO retransmission and full
  protected-packet timer scheduling remain pending.
- 2026-05-26: Added `examples/udp_congestion_recovery_loopback.zig` and the
  ...
  CRYPTO retransmission are now covered separately; 0-RTT/protected-CRYPTO
  retransmission remains pending.
- 2026-05-26: Added `examples/udp_loss_recovery_loopback.zig` and the
  ...
  0-RTT/protected-CRYPTO retransmission remains pending.
- 2026-05-26: Added `examples/udp_pto_recovery_loopback.zig` and the
  ...
  newer queued data. 0-RTT/protected-CRYPTO retransmission remains pending.
...
- 2026-05-23: Made `checkPtoTimeouts()` prefer already queued ack-eliciting
  ...
  ACK-lost and PTO-probed 1-RTT STREAM data plus ACK-lost frame-payload CRYPTO
  data. 0-RTT/protected-CRYPTO retransmission remains pending.
