...
| Flow control | Required. Connection, stream, and stream-count flow control must work over protected UDP. | Partial: frame-payload and protected loopback coverage exists; a quic-go client now completes a 12 KiB stream echo across the Zig server's 2 KiB stream and 8 KiB connection initial windows. |
