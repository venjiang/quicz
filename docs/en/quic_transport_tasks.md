...
| --- | --- | --- |
| UDP client/server endpoint | Required for first usable milestone. One endpoint owner must drive accept/connect, packet receive/send, timers, route cleanup, and close. | Partial: socket-backed loopback examples, endpoint lifecycle helpers, and core socket-loop entrypoints for receive, accepted Initial backend response/close propagation, bounded long-header CRYPTO output draining, pending-work, due-deadline, TLS-backend drive, explicit per-connection installed-key output polling/draining, close propagation, compatible-version backend drive, single-call cross-space backend drive that services ordered Initial/Handshake spaces in one lifecycle pass for one or several connections, feed/pending-work/cross-space-drive steps that advance ordered spaces after one routed datagram, pending-work-only cross-space backend drive with wakeup selection or immediate poll/drain output, due-deadline cross-space backend drive with wakeup selection or immediate poll/drain output, including symmetric per-connection installed-key output options, with both strict and close-on-error peer transport-parameter handling, and wakeup selection exist; production client/server event loop is missing. |
