...
| TLS 1.3 integration | Required. Pure-Zig TLS 1.3 (`src/quic/tls13.zig` + `src/quic/tls13_backend.zig`) exposed through the `CryptoBackend` interface; the OpenSSL C adapter is the deprecated path. | Partial: `Tls13ClientTransport` now owns protected ClientHello, Retry retry-Initial, Initial/Handshake receive, peer Finished, and 1-RTT tail dispatch, and drives the certificate-verified external client. The `quic-go` proof below completes a real TLS-owned handshake and two FIN-terminated 1-RTT STREAM echoes without mock keys or OpenSSL; a production endpoint-owned event loop remains missing. |
