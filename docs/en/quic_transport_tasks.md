...
The server endpoint has the matching direct no-input due-deadline coverage:
the socket owner can service a retained-key expiry without first receiving a
datagram, with no output drain and with the active record and committed route
kept installed.
