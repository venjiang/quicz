...
| RFC 9002 loss detection and congestion control | Partial | ACK delay scaling/capping, packet/time-threshold loss, aggregate loss-time-before-PTO timer deadline selection/service, endpoint-owned multi-connection recovery timer scheduling, endpoint connection lifecycle helper tying route retirement to timer disarm plus caller-keyed protected long/Initial-Handshake CRYPTO-space/0-RTT/short-packet, explicit key-phase/key-update short-packet, caller-owned key-phase short-packet, installed-key Handshake/0-RTT long-packet, and installed-key protected short-packet timer refresh, congestion-window bypass for one armed PTO probe, ACK-driven frame-payload STREAM/CRYPTO, protected CRYPTO, and protected 0-RTT STREAM/RESET_STREAM/STOP_SENDING retransmission requeue, NewReno slow-start/congestion-avoidance growth, recovery-period behavior, and minimum-window ssthresh clamp, PTO-backoff-independent persistent congestion duration/response, ACK_ECN CE-driven NewReno recovery response, packet-space PTO PING/new-data/in-flight-CRYPTO/protected-0-RTT-control/protected-0-RTT-STREAM/in-flight-STREAM/cross-space probe hooks with Initial/Handshake max_ack_delay suppression, ECN validation and lifecycle-owned UDP-path mirroring tests, and socket-backed UDP endpoint-timer-driven loss/PTO recovery plus congestion-recovery/STREAM-retransmission loopbacks. | Full TLS-owned socket-owned protected-packet loss/PTO timer lifecycle integration and remaining NewReno edge cases with controlled-clock tests. |
...
- 2026-05-27: Added cross-space PTO probe queuing. When any packet-number
  ...
  packet number spaces that still have in-flight packets without advancing
  those spaces' PTO backoff until their own deadlines expire. Tests cover
  Initial-triggered Handshake peer-space probes, duplicate suppression, and
  delayed Handshake backoff; `pto_recovery` now prints the peer-space probe
  evidence.
...
- 2026-05-27: Wired socket-backed UDP loss/PTO recovery examples through
  `EndpointLossDetectionTimers`. Protected-packet PTO and loss-time tests now
  prove endpoint arm/service/disarm around protected short packets; the UDP
  loss example services time-threshold cleanup through the endpoint timer
  owner and prints `time_timers=0`, while the UDP PTO example drives PING,
  queued STREAM, in-flight STREAM, and CRYPTO probes through the same timer
  owner and prints `timers_remaining=0`. Remaining work is full socket-owned
...
- 2026-05-26: Added ACK-driven STREAM retransmission requeue for lost
  ...
  plus `run-udp-stream-retransmission-loopback` prove protected UDP delivery,
  sparse ACK loss classification, protected STREAM retransmission, duplicate
  receive discard, and final ACK cleanup. Protected CRYPTO and protected 0-RTT
  STREAM/RESET_STREAM/STOP_SENDING retransmission are now covered; full
  protected-packet timer scheduling remains pending.
- 2026-05-26: Added `examples/udp_congestion_recovery_loopback.zig` and the
  ...
  protected short PING packets over loopback UDP, returns protected ACK frames
  that trigger NewReno recovery-period behavior, proves a repeated loss does
  not reduce the congestion window again inside recovery, and proves persistent
  congestion reduces the congestion window to the minimum window. ACK-driven
  and PTO-driven 1-RTT STREAM retransmission plus ACK-driven frame-payload
  CRYPTO retransmission and protected 0-RTT STREAM/RESET_STREAM/STOP_SENDING
  retransmission are now covered separately.
- 2026-05-26: Added `examples/udp_loss_recovery_loopback.zig` and the
  ...
  acknowledge only the largest packet, and proves packet-threshold loss removal
  and aggregate-service-driven time-threshold loss cleanup through the real UDP
  routing path. ACK-driven and PTO-driven 1-RTT STREAM retransmission plus ACK-driven
...
- 2026-05-26: Added `examples/udp_pto_recovery_loopback.zig` and the
  ...
  protected PING over loopback UDP, withholds the ACK, drives the aggregate PTO
  ...
  proves the aggregate service sends that queued data as the protected probe before ACK
  cleanup. It also proves PTO reuses an in-flight STREAM copy when there is no
  newer queued data. Protected CRYPTO and protected 0-RTT
  STREAM/RESET_STREAM/STOP_SENDING retransmission are now covered.
...
- 2026-05-26: Added `examples/udp_flow_control_loopback.zig` and the
  ...
  STREAM_DATA_BLOCKED through the endpoint router, delivers receive-side
...
- 2026-05-22: Added `ptoDeadlineMillis()` and `checkPtoTimeouts()` for
  ...
  applies per-space PTO backoff. Later updates make Application PTO reuse
  queued or in-flight STREAM data before falling back to PING.
- 2026-05-23: Made `checkPtoTimeouts()` prefer already queued ack-eliciting
  ...
  probe selection without adding a PING; later updates also cover cloning
...
- 2026-05-22: Added `examples/pto_recovery.zig` and
  ...
  queued STREAM data and in-flight STREAM data serving as PTO probes, and
...
| `udp_flow_control_loopback` | Socket-backed loopback UDP flow-control exercise: protected STREAM delivery to the receive limit, protected STREAM_DATA_BLOCKED routing, receive-side MAX_DATA/MAX_STREAM_DATA credit refresh delivery, resumed STREAM data with FIN, and final ACK cleanup. | Present |
...
| `udp_loss_recovery_loopback` | Socket-backed loopback UDP loss-recovery exercise: protected short PING routing, protected ACK-driven packet-threshold loss, endpoint-timer-driven time-threshold cleanup, and final timer disarm. | Present |
| `udp_congestion_recovery_loopback` | Socket-backed loopback UDP congestion-recovery exercise: protected short PING routing, NewReno recovery-period repeated-loss suppression, and persistent congestion reduction to the minimum congestion window. | Present |
| `udp_pto_recovery_loopback` | Socket-backed loopback UDP PTO recovery exercise: endpoint-timer-driven ACK-loss PTO deadline, protected PING fallback probe delivery, queued STREAM data as a protected PTO probe, in-flight STREAM/CRYPTO data as protected PTO probes, duplicate receive/CRYPTO range discard, ACK cleanup, and final timer disarm. | Present |
| `udp_stream_retransmission_loopback` | Socket-backed loopback UDP STREAM retransmission exercise: sparse protected ACK marks a 1-RTT STREAM packet lost, the sender emits a new protected STREAM retransmission packet, the receiver discards the duplicate stream range idempotently, and a final ACK clears bytes in flight. | Present |
