...
| 0-RTT | Schedule after the first 1-RTT stream echo interop gate; not required for the current milestone. | Partial: explicit accept/reject, mock installed-key 0-RTT paths, NewSessionTicket RFC 9001 sentinel validation, ticket-gated ClientHello `early_data` emission, connection-policy-bound EncryptedExtensions acceptance, client rejection-driven key discard, server-side ticket-age policy, and bounded local PSK identity replay filtering with snapshot/restore exist; production/distributed replay policy is still missing. |
