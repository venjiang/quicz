...
- Current `pollTx` / `processDatagram` behavior moves unencrypted QUIC frame payload bytes. `pollTx` may emit ACK-only payloads, queued CRYPTO, PATH_RESPONSE, RESET_STREAM, or STREAM payloads, or coalesce a pending ACK when space allows, but it does not yet produce or consume protected QUIC packets over UDP.
