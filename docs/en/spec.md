...
- Locally initiated bidirectional streams must be created with `openStream()` before `sendOnStream()`. `sendOnStream()` can still create send state for peer-initiated bidirectional streams so the in-memory echo examples can reply on the peer's stream.
