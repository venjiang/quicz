...
- Basic stream support, send-side STREAM fragmentation, inbound RESET_STREAM handling, flow-control and stream-count limits, strict stream direction validation, and close-state handling
...
- `QuicConnection` implements an in-memory stream send/receive skeleton with send-side STREAM fragmentation, contiguous receive buffers, inbound RESET_STREAM handling, basic connection and stream flow control, bidirectional and unidirectional stream-count limits, close-state handling for CONNECTION_CLOSE/APPLICATION_CLOSE, automatic ACK generation for ACK-eliciting payloads, ACK-only emission, ACK and STREAM coalescing when space allows, ACK-driven sent-packet tracking, and a simplified recovery/congestion state object.
...
- Invalid multi-frame payloads roll back state changed earlier in that payload, including receive buffers, RESET_STREAM state, MAX_DATA/MAX_STREAMS_BIDI/UNI updates, pending ACK state, sent-packet recovery state, and close-state changes.
...
- Not implemented yet: TLS 1.3 integration, packet protection, distinct packet number spaces, full RFC 9002 loss timers and packet-threshold loss detection, UDP 4-tuple connection ownership, out-of-order stream reassembly, QUIC v2 behavior, path migration, and stateless reset.
