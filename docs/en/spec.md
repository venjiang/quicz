...
- Locally initiated bidirectional streams must be created with `openStream()` before `sendOnStream()`. `sendOnStream()` can still create send state for observed peer-initiated bidirectional streams so the in-memory echo examples can reply on the peer's stream.
- The connection skeleton currently accepts only bidirectional STREAM/RESET_STREAM traffic. Unidirectional stream state is not modeled yet, though the frame codec can still encode and decode the relevant frame types.
