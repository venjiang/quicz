...
- 每个 UDP 四元组对应一个连接
- 基础流支持、发送侧 STREAM 分片、入站 RESET_STREAM 与 STOP_SENDING 处理、最小 PATH_CHALLENGE/PATH_RESPONSE 处理、客户端侧 NEW_TOKEN/HANDSHAKE_DONE 接收校验、流量控制和 stream-count 限制、严格 stream 方向校验与关闭状态处理
