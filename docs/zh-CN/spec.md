...
- 已实现：QUIC varint 工具、最小 long/short header codec、基础 frame codec（STREAM、CRYPTO、PADDING、PING、ACK 多区间、RESET_STREAM、STOP_SENDING、MAX_DATA、MAX_STREAM_DATA、MAX_STREAMS_BIDI/UNI 与 connection-close 变体）、内存态 `QuicConnection` stream 发送/接收骨架、基础 connection/stream 流量控制、CONNECTION_CLOSE/APPLICATION_CLOSE 关闭状态处理、针对 ACK-eliciting payload 的自动 ACK 生成、ACK 驱动的 sent-packet tracking，以及简化 recovery / congestion 状态对象。
