...
| P0-A2 | 本地 endpoint 1-RTT close loop | RFC 9000 1-RTT packet processing、stream data、close；RFC 9001 key discard | Endpoint-owned client/server loop | 同一个本地 loop 完成 protected STREAM echo、due timer service、protected close、key discard 和 route/record 退役。 | Active |
...
| 2 | P0-A2 本地 endpoint STREAM/close loop | Active | 扩展同一个本地 loop，覆盖 protected STREAM echo、due timer service、close、key discard 和 route cleanup。 |
