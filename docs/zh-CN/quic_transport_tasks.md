...
是否仍排队。
P0-1b 已完成：server receive-step API 现在通过有界 result shape 返回 admission、
routed output、pending work、idle/close/key-discard due cleanup、route error、
capacity error 和 step 后 deadline selection。P0-1c 现在 active，必须只使用本地
Zig endpoint API；mock-only 或 installed-key-only 证据不能关闭本地 endpoint-loop
门禁。
