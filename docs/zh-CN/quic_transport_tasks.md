...
| RFC 9002 loss detection 与 congestion control | Partial | ACK delay scaling/capping、packet/time-threshold loss、NewReno recovery-period 行为、persistent congestion、带 Initial/Handshake max_ack_delay suppression 的 packet-space PTO PING/new-data probe hook、ECN validation 测试和 socket-backed UDP loss-recovery/congestion-recovery/PTO-recovery loopback。 | 仍需 protected-packet PTO/loss timer 完整调度，以及基于 controlled-clock 的重传行为测试。 |
...
- 2026-05-26：新增 ACK 驱动的 STREAM retransmission requeue。已发送的
  ...
  幂等丢弃和 final ACK cleanup。0-RTT/CRYPTO 与 PTO 驱动的 protected data
  retransmission 仍待实现。
- 2026-05-26：新增 `examples/udp_congestion_recovery_loopback.zig` 和
  ...
  window。ACK 驱动的 1-RTT STREAM retransmission 已单独覆盖；0-RTT/CRYPTO
  与 PTO 驱动的 protected data retransmission 仍待实现。
- 2026-05-26：新增 `examples/udp_loss_recovery_loopback.zig` 和
  ...
  cleanup 都能走真实 UDP route。ACK 驱动的 1-RTT STREAM retransmission 已单独
  覆盖；0-RTT/CRYPTO 与 PTO 驱动的 protected data retransmission 仍待实现。
- 2026-05-26：新增 `examples/udp_pto_recovery_loopback.zig` 和
  ...
  作为 protected probe 发送，最后完成 ACK cleanup。ACK 驱动的 1-RTT STREAM
  retransmission 已单独覆盖；0-RTT/CRYPTO 与 PTO 驱动的 protected data
  retransmission 仍待实现。
...
- 2026-05-22：新增 `ptoDeadlineMillis()` 与 `checkPtoTimeouts()`，作为
  ...
  backoff。PTO 驱动的 protected-packet data retransmission 仍待实现。
- 2026-05-23：让 `checkPtoTimeouts()` 在新增 PING probe 前优先使用已经排队的
  ...
  ACK-lost 1-RTT STREAM data 的克隆已覆盖；0-RTT/CRYPTO 与 PTO 驱动的
  protected data retransmission 仍待实现。
...
- 2026-05-22：新增 `examples/pto_recovery.zig` 和
  ...
  PTO probe，以及通过 `pollTxInSpace()` 发出 Initial/Handshake PING。
...
| `udp_pto_recovery_loopback` | Socket-backed loopback UDP PTO recovery：ACK-loss PTO deadline、protected PING fallback probe delivery、queued STREAM data 作为 protected PTO probe，以及 final ACK cleanup。 | 已存在 |
...
| `pto_recovery` | 当前 frame-payload Initial/Handshake/Application PTO hook，包含 Initial/Handshake max_ack_delay suppression、PING fallback probe 和 queued STREAM data probe selection。 | 已存在 |
