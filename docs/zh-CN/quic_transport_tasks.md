...
| TLS 1.3 集成 | 必须具备。使用支持 QUIC transport-parameter 和 traffic-secret hook 的 C TLS 库，通过很小的 Zig `TlsBackend` adapter 接入；不在仓库内自研 TLS。 | 部分完成：已有 mock `CryptoBackend`、installed-key handoff、带测试的 C-ABI `TlsBackend` adapter、C-object adapter probe、OpenSSL QUIC TLS API/link probe、按 protection level 分离 CRYPTO handoff 并映射到 quicz CRYPTO 队列，覆盖 protected Initial long-packet 投递和 socket-backed Initial 投递，使用 OpenSSL 产出的 Handshake secrets 完成双向 installed-key protected Handshake 投递和 socket-backed Handshake 投递，并使用 OpenSSL 产出的 1-RTT secrets 完成 installed-key protected STREAM request/response 和 socket-backed STREAM echo 的 OpenSSL client/server callback-mode transcript 检查，以及可产出第一段 TLS CRYPTO flight，并让 peer transport parameters、真实 pair-transcript Handshake/1-RTT secrets 和入站 CRYPTO 经 OpenSSL callback 边界进入连接层、且验证 adapter 安装的 client 1-RTT keys 能保护 short packet、对端 connection 可用匹配 transcript secrets 解密并 ACK 的 OpenSSL-backed adapter wrapper；缺少完整 endpoint-owned live TLS handshake/socket loop。 |
...
- 2026-06-04：扩展 `examples/tls_openssl_backend_adapter.zig`，让
  ...
  连接层；当前这些 Handshake/1-RTT secrets 与 Handshake CRYPTO bytes 已复用真实
  pair transcript，并会验证 adapter 安装的 client 1-RTT keys 能保护 short packet，
  且对端 connection 可用匹配 transcript secrets 解密并 ACK。完整 endpoint-owned live
...
| `tls_openssl_backend_adapter` | 可运行的 OpenSSL-backed `TlsBackend` wrapper 检查：现有 drive 路径会把 quicz 本端 transport parameters 设置到 OpenSSL TLS object，调用 `SSL_do_handshake()` 产出第一段 TLS CRYPTO flight，并让 peer transport parameters、真实 pair-transcript Handshake/1-RTT secrets 和 Handshake CRYPTO bytes 经 OpenSSL callback 边界进入连接层，随后验证 adapter 安装的 client 1-RTT keys 能保护 short packet，且对端 connection 可用匹配 transcript secrets 解密并 ACK。 | 已存在 |
