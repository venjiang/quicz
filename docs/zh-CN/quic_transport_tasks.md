...
| TLS 1.3 集成 | 必须具备。纯 Zig TLS 1.3（`src/quic/tls13.zig` + `src/quic/tls13_backend.zig`）通过 `CryptoBackend` 接口接入；OpenSSL C adapter 为废弃路径。 | 部分完成：`Tls13ClientTransport` 现持有 protected ClientHello、Retry retry-Initial、Initial/Handshake 接收、peer Finished 和 1-RTT 尾包分发状态，并驱动证书校验的外部 client。下方 `quic-go` 证据完成真实 TLS-owned handshake 与两条带 FIN 的 1-RTT STREAM echo，不使用 mock key 或 OpenSSL；仍缺少生产级 endpoint-owned event loop。 |
