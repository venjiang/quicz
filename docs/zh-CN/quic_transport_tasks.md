...
| RFC 9000 传输协议 | Partial | Frame codec、transport parameter、连接状态、stream、flow control、connection ID、Retry/token、path validation、close/reset 行为、endpoint routing helper、caller-keyed protected UDP packet loopback 和示例。 | 仍需完整 protected/TLS socket-backed client/server loopback、完整 endpoint 生命周期和外部互通。 |
