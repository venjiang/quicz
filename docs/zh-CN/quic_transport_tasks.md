...
| TLS 1.3 集成 | 必须具备。使用支持 QUIC transport-parameter 和 traffic-secret hook 的 C TLS 库，通过很小的 Zig `TlsBackend` adapter 接入；不在仓库内自研 TLS。 | 部分完成：已有 mock `CryptoBackend`、installed-key handoff、带测试的 C-ABI `TlsBackend` adapter、C-object adapter probe、OpenSSL QUIC TLS API/link probe、按 protection level 分离 CRYPTO handoff 并映射到 quicz CRYPTO 队列，复制并解析 peer transport-parameter bytes，记录 keylog callback 次数和字节数但不打印 key material，覆盖 protected Initial long-packet 投递和 socket-backed Initial 投递，使用 OpenSSL 产出的 Handshake secrets 完成双向 installed-key protected Handshake 投递和 socket-backed Handshake 投递，并使用 OpenSSL 产出的 1-RTT secrets 完成 installed-key protected STREAM request/response 和 socket-backed STREAM echo 的 OpenSSL client/server callback-mode transcript 检查，以及可产出第一段 TLS CRYPTO flight、把 adapter 产出的 Initial CRYPTO flight 和真实 pair-transcript Handshake CRYPTO 作为 protected Initial/Handshake datagram 通过 loopback UDP 投递，并让 peer transport parameters、真实 pair-transcript Handshake/1-RTT secrets 和入站 CRYPTO 经 OpenSSL callback 边界进入连接层，且使用 adapter 安装的 client keys 和匹配 peer transcript secrets 通过同一个 socket/lifecycle loop owner 驱动 loopback UDP 1-RTT STREAM echo、Application PTO service、protected close 投递和 route cleanup 的 OpenSSL-backed adapter wrapper；缺少完整 endpoint-owned live TLS handshake/socket loop。 |
...
| ACK/loss/PTO recovery | 必须具备。ACK processing、packet/time-threshold loss、PTO、retransmission 和 timer service 必须驱动 endpoint loop。 | 部分完成：已有确定性 recovery model 和 socket-backed loopback；socket-owned protected-packet lifecycle 集成未完成。 |
| Congestion control | 至少需要 NewReno-style 基线。CUBIC 或可配置 controller 是后续性能工作。 | 部分完成：已有简化 NewReno-style 行为；缺少生产调优和可配置 controller。 |
| Connection ID 和 stateless reset | 必须具备。Routing、CID issue/retire、reset-token handling、close cleanup 和 inactive-CID reset emission 必须接入 endpoint lifecycle。 | 部分完成：已有 endpoint router、lifecycle helper 和 socket-backed loopback；缺少完整 TLS-owned lifecycle 集成。 |
...
面向 socket 的目标 API 形态是 `feedDatagram`、`processPendingWork`、`pollDatagram`
和 `nextDeadline`，由同一个 lifecycle owner 驱动 timer、route cleanup、close 和
key discard。第一版互通入口只承接 `handshake` 和 `transfer`；真实 TLS-owned
二进制不存在时必须返回明确 blocker，不得伪装成功。真实 TLS 和互通路径还需要同步
输出可排障证据，包括 keylog，以及 handshake、transport parameter、traffic-secret
installation、packet number space、ACK/loss/PTO、key discard、close 和 route cleanup
事件。
...
- 2026-06-05：新增 `examples/tls_openssl_pair_transcript.zig` 和一个小 C
  ...
  Handshake/1-RTT traffic-secret callback；Zig 侧现在会复制并解析这些 peer
  ...
  loopback UDP 投递；也会安装 OpenSSL 产出的 1-RTT secrets，并验证 short packet 上的
  installed-key protected STREAM request/response；随后用同一组 1-RTT secrets 通过同一个
  lifecycle 驱动 loopback UDP STREAM echo。完整 endpoint-owned live TLS handshake/socket
  loop 仍待实现。
- 2026-06-04：扩展 `examples/tls_openssl_backend_adapter.zig`，让
  ...
  与 OpenSSL full QUIC mode 不同，也能让 peer transport parameters、Handshake
  secrets、1-RTT secrets 和 Handshake CRYPTO bytes 经 OpenSSL callback 边界进入
  ...
  server，且不会重复交付重复 STREAM data；建模 handshake confirmation 后会显式丢弃双端
  Handshake packet-number space 和 Handshake keys；Initial、Handshake、Application echo、
  Application PTO、protected close 和 close/drain timeout cleanup 现在共用同一个
  socket/lifecycle loop owner；示例会打印 `transcript_tp=6` 证明 pair transcript
  收到的 peer transport-parameter bytes，也会打印 `transcript_keylog=5/5/773/773`
...
| `tls_backend_adapter` | 可运行的 C-ABI `TlsBackend` adapter 契约检查：在绑定具体 C TLS 库之前，先通过现有 `CryptoBackend` drive 路径验证本端/对端 transport-parameter byte handoff、入站/出站 CRYPTO byte 投递、Handshake traffic-secret 安装和 handshake confirmation。 | 已存在 |
...
| `tls_openssl_pair_transcript` | 可运行的 OpenSSL client/server callback-mode transcript 检查：固定示例 PSK 避免证书 fixture 噪声，CRYPTO bytes 按 OpenSSL protection level 路由，双端无 alert 完成 TLS 1.3，双端都会触发 peer transport-parameter callback 与 Handshake/1-RTT secret callback，peer transport-parameter bytes 会被复制并解析，并记录 keylog callback 次数和字节数但不打印 key material；生成的 CRYPTO bytes 会投递到 quicz Initial/Handshake/Application CRYPTO 队列，同时会把 client Initial CRYPTO bytes 经 quicz protected Initial long-packet helper 组包并投递到 server connection，也会通过 quicz endpoint lifecycle 在 loopback UDP 上投递双向 Initial flight，用 OpenSSL 产出的 Handshake secrets 驱动双向 installed-key protected Handshake CRYPTO 投递，包括通过同一个 lifecycle 路由的 loopback UDP 投递，并用 OpenSSL 产出的 1-RTT secrets 驱动 short packet 上的 installed-key protected STREAM request/response，以及通过同一个 lifecycle 路由的 loopback UDP STREAM echo。 | 已存在 |
| `tls_openssl_backend_adapter` | 可运行的 OpenSSL-backed `TlsBackend` wrapper 检查：现有 drive 路径会把 quicz 本端 transport parameters 设置到 OpenSSL TLS object，调用 `SSL_do_handshake()` 产出第一段 TLS CRYPTO flight，把该 adapter 产出的 Initial CRYPTO flight 作为 protected Initial datagram 通过 loopback UDP 投递，并让 peer transport parameters、真实 pair-transcript Handshake/1-RTT secrets 经 OpenSSL callback 边界进入连接层，随后把真实 pair-transcript Handshake CRYPTO 作为 protected Handshake datagram 通过 loopback UDP 投递后再喂回 adapter，最后用 adapter 安装的 client keys 和匹配 peer transcript secrets 驱动 loopback UDP 1-RTT STREAM echo，通过同一个 lifecycle owner 服务 Application PTO 并路由 protected probe，输出 transcript peer transport-parameter byte 证据、transcript keylog 证据和当前 wrapper keylog 边界，建模 handshake confirmation 后显式丢弃双端 Handshake packet-number space 和 Handshake keys，并通过同一个 socket/lifecycle loop owner 完成 protected close 投递和 route cleanup。 | 已存在 |
