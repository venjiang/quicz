...
上方客户端专用路径仍只是外部握手证据。独立的 Go 与 Rust 示例证明了外部客户端到 Zig
server 的 STREAM echo；Retry、loss/recovery、version negotiation、更广泛的服务端行为和
应用层协议互通仍需验证，里程碑不能据此视为完成。
