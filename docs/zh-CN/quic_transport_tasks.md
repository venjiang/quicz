...
| `udp_congestion_recovery_loopback` | Socket-backed loopback UDP lifecycle congestion recovery：lifecycle-routed protected short PING/ACK receive path、显式 NewReno recovery-period 重复 loss 抑制证据，以及 persistent congestion 降到 minimum congestion window 的显式证据。 | 已存在 |
