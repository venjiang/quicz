...
- 2026-06-04：扩展 `examples/tls_openssl_backend_adapter.zig`，让
  ...
  datagram 通过 loopback UDP 投递。当前这些 Handshake/1-RTT secrets 与 Handshake
  CRYPTO bytes 已复用真实 pair transcript，并会验证 adapter 安装的 client 1-RTT keys
  能保护 short packet，且对端 connection 可用匹配 transcript secrets 解密并 ACK。
