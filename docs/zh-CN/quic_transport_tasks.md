...
| 0-RTT | 第一轮 1-RTT stream echo 互通之后推进；不阻塞当前里程碑。 | 部分完成：已有显式 accept/reject、mock installed-key 0-RTT 路径、NewSessionTicket RFC 9001 sentinel 校验，以及按 ticket permission gate 的 ClientHello `early_data` 发送；缺少真实 TLS replay policy。 |
