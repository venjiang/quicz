...
- 2026-06-04：扩展 `examples/tls_openssl_backend_adapter.zig`，让
  ...
  Initial、Handshake、Application echo、protected close 和 close/drain timeout cleanup
  现在共用同一个 socket/lifecycle loop owner；示例会在退役全部 client/server route 后
  打印 `adapter_endpoint_routes=3/4/0/0`。
