...
`processDatagram` 只流转未加密 QUIC frame payload 字节，尚不能生成或消费
带 packet protection 的 QUIC-over-UDP packet。
...
| Packet number spaces | 部分 frame-payload ACK/recovery + CRYPTO 隔离 + frame-type filtering + discard cleanup | 维护独立 Initial、Handshake、Application packet number space，并在后续把 protected packet 路由到匹配空间且遵守真实 TLS key discard 规则。 | 现有 ACK/recovery、CRYPTO 隔离、forbidden-frame 与 discard 测试证明空间隔离和清理；后续 protected-packet 测试证明路由正确。 |
| 真实 datagram API | 仅有 Initial protected long-packet helper | 在当前 frame-payload 骨架之上增加受保护 QUIC datagram 收发 API。 | 现有 helper 测试覆盖一个受保护 Initial packet；后续本地 client/server loopback 必须可交换受保护 packet。 |
...
- 2026-05-22：新增 `retryIntegrityTag()` 与 `verifyRetryIntegrityTag()`，
  覆盖 RFC 9001 Retry Packet Integrity。该 helper 会基于 Original
  Destination Connection ID 和去掉最终 tag 后的 transmitted Retry bytes
  构造 Retry pseudo-packet，再用固定 key 的 AEAD_AES_128_GCM 计算 tag。
  测试覆盖 RFC 9001 Appendix A.4 Retry 向量、篡改拒绝、非法 Original DCID
  长度和过短 Retry datagram。加密 Retry token 生成、过期和地址绑定仍属于
  后续 endpoint policy。
...
- 2026-05-22：新增按 packet number space 隔离的 CRYPTO 收发 byte stream，
  ...
  与 `zig build run-crypto-stream` 已演示建模的 TLS bridge flow；真实 TLS
  backend 与 packet protection 仍待实现。
...
| `crypto_stream` | 当前 frame-payload Initial/Handshake CRYPTO bridge flow 与建模 handshake confirmation。 | 已存在 |
...
| `retry_token` | 当前 Retry packet integrity-tag 生成/校验、token loopback、一次性 token 消费与 address-validation unblocking。 | 已存在 |
