...
| 功能 | 实用目标 | quicz 状态 |
| --- | --- | --- |
| UDP client/server endpoint | 第一轮可用里程碑必须具备。由同一个 endpoint owner 驱动 accept/connect、packet receive/send、timer、route cleanup 和 close。 | 部分完成：已有 socket-backed loopback 示例、endpoint lifecycle helper，以及覆盖 receive、accepted Initial backend response/close propagation、bounded long-header CRYPTO output draining、pending-work、due-deadline、TLS-backend drive、显式 per-connection installed-key output poll/drain、close propagation、compatible-version backend drive、在一次 lifecycle pass 里为单个或多个连接按 Initial/Handshake 有序 space 一次性驱动的跨 space backend drive、在一个 datagram 路由后按有序 space 推进、pending-work-only 跨 space backend drive 可选择下一次 wakeup 或立即 poll/drain 输出、due-deadline 跨 space backend drive 可选择下一次 wakeup 或立即 poll/drain 输出、包含对称的 per-connection installed-key output options，并同时支持 strict 和 close-on-error peer transport-parameter 处理的 feed/pending-work/跨 space drive 接收步骤 和 wakeup selection 的核心 socket-loop 入口；缺少生产级 client/server event loop。 |
| TLS 1.3 集成 | 必须具备。纯 Zig TLS 1.3（`src/quic/tls13.zig` + `src/quic/tls13_backend.zig`）通过 `CryptoBackend` 接口接入；OpenSSL C adapter 为废弃路径。 | 部分完成：已有 mock `CryptoBackend`、installed-key handoff、带测试的 C-ABI `TlsBackend` adapter、C-object adapter probe、OpenSSL QUIC TLS API/link probe、按 protection level 分离 CRYPTO handoff 并映射到 quicz CRYPTO 队列，复制并解析按角色区分的 peer transport-parameter bytes，记录 keylog callback 次数和字节数但不打印 key material，覆盖 protected Initial long-packet 投递、socket-backed Initial 投递、手动 OpenSSL Initial/Handshake transcript、同一 context 的 1-RTT STREAM echo、Handshake key discard、protected close 和 route cleanup 通过同一 socket/lifecycle 边界路由，使用 OpenSSL 产出的 Handshake secrets 完成双向 installed-key protected Handshake 投递和 socket-backed Handshake 投递，并使用 OpenSSL 产出的 1-RTT secrets 完成 installed-key protected STREAM request/response 和 socket-backed STREAM echo 的 OpenSSL client/server callback-mode transcript 检查，以及可产出第一段 TLS CRYPTO flight、把 adapter 产出的 Initial CRYPTO flight 和真实 pair-transcript Handshake CRYPTO 作为 protected Initial/Handshake datagram 通过 loopback UDP 投递，并消费 pair-transcript server transport parameters，让真实 pair-transcript Handshake/1-RTT secrets 和入站 CRYPTO 经 OpenSSL callback 边界进入连接层，且使用 adapter 安装的 client keys 和匹配 peer transcript secrets 通过同一个 socket/lifecycle loop owner 驱动 loopback UDP 1-RTT STREAM echo、Application PTO service、protected close 投递和 route cleanup，并证明 direct server probe 消费 Handshake CRYPTO 后确认、server connection 通过 backend 拉取 1-RTT、确认握手、丢弃 Handshake space/key，以及 paired loopback server 通过 backend 消费 Handshake CRYPTO、拉取 peer transport parameters 和 Handshake/1-RTT secrets 后确认并清理 Handshake keys 的 OpenSSL-backed adapter wrapper，以及在单次调用里按 Initial/Handshake 有序 packet number space 投喂入站 CRYPTO 并拉取 backend 产出 CRYPTO 的跨 space backend 驱动；缺少完整 endpoint-owned live TLS handshake/socket loop。 |
...
| 外部互通 | 声称第一轮可用 transport 里程碑前必须具备。 | 部分完成：客户端专用二进制已与两个来自不同实现家族的本机独立服务端完成证书校验的 QUIC/TLS 握手；Go 与 Rust 客户端已和本地 Zig server 完成证书校验的 STREAM echo。更广泛的服务端、Retry、recovery 和应用层场景尚未验证。 |
...
SNI，保持证书校验开启，协商 `hq-interop`，在 stream 0 发送 `hello`，并且只有收到五字节
echo 后才报告成功。本仓库的本地开发命令如下：
...
使用真实墙钟和 SNI，保持证书校验开启，并通过真实 UDP socket 驱动 TLS 持有的
Initial、Handshake 和 1-RTT key。一个 UDP datagram 中共包的 long-header packet 会先被
...
| Stateless reset | 部分 helper + constant-time token match + 连接层检测 + 连接层 reset receive-to-draining 状态 + NEW_CONNECTION_ID token uniqueness checks + endpoint inactive-CID reset datagram construction + 通过 `EndpointConnectionLifecycle` 暴露的 route/reset/drop receive classification + socket-backed UDP reset emission loopback + socket-backed close-triggered lifecycle route retirement/reset loopback | 把 reset emission 接入 socket 拥有的 endpoint lifecycle 和 connection close/drop policy。 | 现有测试覆盖 reset token 命中、误判拒绝、短 datagram 拒绝、跨 CID duplicate-token 拒绝、retired token 忽略、active token receive 使连接进入 draining、reset receive 后抑制 pending close output、active route token 抑制、retired route token lookup、inactive-route reset datagram construction、smaller-than-trigger sizing、route/reset/drop receive action classification、lifecycle-owned route/reset/drop after timer-disarming retirement 和 ambiguous reset-token CID 拒绝；`run-stateless-reset` 演示 endpoint inactive-CID reset action；`run-udp-stateless-reset-loopback` 演示真实 UDP active-route suppression、unknown-CID drop classification、退役后的 trigger 投递、reset 发出和客户端 token 匹配；`run-udp-close-lifecycle-loopback` 演示 protected close delivery、lifecycle-routed protected receive auto-close、`EndpointConnectionLifecycle` connection-handle route retirement、保留 reset token lookup、reset emission 和 client token matching；后续 endpoint 测试覆盖完整 TLS-owned lifecycle 集成。 |
