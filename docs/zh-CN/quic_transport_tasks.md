...
接收侧 Initial protected long packet bridge，但尚不能完整生成或消费
...
| Packet number spaces | 部分 frame-payload ACK/recovery + CRYPTO 隔离 + Initial protected receive bridge + frame-type filtering + discard cleanup | 维护独立 Initial、Handshake、Application packet number space，并在后续把 protected packet 路由到匹配空间且遵守真实 TLS key discard 规则。 | 现有 ACK/recovery、CRYPTO 隔离、Initial protected receive、forbidden-frame 与 discard 测试证明空间隔离和清理；后续 protected transmit/coalesced 测试证明完整路由正确。 |
| 真实 datagram API | Initial protected receive bridge + protected long-packet helper | 在当前 frame-payload 骨架之上增加受保护 QUIC datagram 收发 API。 | 现有 helper 测试覆盖一个受保护 Initial packet；连接层测试覆盖受保护 Initial 解密并投递 CRYPTO；后续本地 client/server loopback 必须可交换受保护 packet。 |
...
- 2026-05-22：新增按 packet number space 隔离的 CRYPTO 收发 byte stream，
  ...
  flight 会经过 protected Initial receive bridge；真实 TLS backend、protected
  transmit 和后续 encryption level 仍待实现。
