...
| RFC 9000 frame codec | frame 集合已覆盖 + frame type 最短 varint 校验 + ACK range 校验 + 未知类型拒绝 + 部分 packet-type 校验 | 覆盖所有 RFC 9000 transport frame，并统一严格校验和错误映射。 | 每类 frame 的合法、截断、非法、未知输入编解码测试，以及会计算出负 packet number 的 ACK range；connection-level ACK 处理会在 recovery 副作用前拒绝同类 invalid range。 |
...
- 2026-05-22：新增 `examples/codec_roundtrip.zig` 和 `zig build run-codec`。
  ...
  transport error helper 的 roundtrip（含 frame codec error 分类）。
...
| `codec_roundtrip` | 演示 varint、packet header、RFC 9369 QUIC v2 long-header type-bit 映射、short-header spin-bit 保留、long/short-packet envelope、header packet number 截断/重建、packet number 编码、RFC 8999 Version Negotiation packet codec、reserved-version skip、client-side VN selection 和 RFC 9368 downgrade-check state、frame、包含 RFC 9368 `version_information` 的 transport parameter、连接层参数暴露（含 TLS extension bytes 和 server preferred_address）与包含 `VERSION_NEGOTIATION_ERROR`、frame decode `FRAME_ENCODING_ERROR` 分类的 transport error codec。 | 已存在 |
