...
不替代下方 transport 任务矩阵：优先推进 TLS-owned socket-backed echo，再推进可嵌入
socket API、最小互通入口和 TLS/互通可观测性。
...
- 2026-06-05：新增 `examples/tls_openssl_pair_transcript.zig` 和一个小 C
  ...
  request/response。真实 socket-owned echo 仍待实现。
