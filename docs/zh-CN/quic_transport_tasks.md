...
server endpoint 也有对应的无输入 due-deadline 直接覆盖：socket owner 可以在没有先收到
datagram 的情况下服务 retained-key expiry，不做 output drain，并保留 active record
与已提交 route。
