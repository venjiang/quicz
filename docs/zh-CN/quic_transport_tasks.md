...
| P0-A1 | 本地 endpoint TLS handshake | RFC 9001 handshake；RFC 9000 Initial/Handshake packet | Endpoint-owned client/server loop | Zig client/server 只通过 endpoint API 完成证书校验 handshake；output 保持 route-bound；不依赖 mock key、OpenSSL、examples 或调用方 per-case glue。 | Done |
| P0-A2 | 本地 endpoint 1-RTT close loop | RFC 9000 1-RTT packet processing、stream data、close；RFC 9001 key discard | Endpoint-owned client/server loop | 同一个本地 loop 完成 protected STREAM echo、due timer service、protected close、key discard 和 route/record 退役。 | Done |
| P0-B1 | Protected bidi/uni stream data | RFC 9000 Sections 2、4、19.8 | Endpoint 后面的 `Connection` stream state | Bidi 和 uni data plus FIN 能跑在 protected endpoint packet 上。 | Done |
...
| P0-D1 | Failure-before-mutation 不变量 | RFC 9000 packet/frame validation | Packet decoder、frame handler、endpoint admission | 畸形 packet/frame 在 packet-number、ACK、stream、CID、token、route 或 timer mutation 前失败。 | Done |
| P0-D2 | Retry 与 address validation | RFC 9000 Retry、token、address validation | Server admission 与 token policy | Retry/token 成功与失败路径只提交已认证状态，并暴露 endpoint-visible error。 | Done |
| P0-D3 | CID routing 与 stateless reset | RFC 9000 connection ID 与 stateless reset | Endpoint router 与 connection CID state | CID issue/retire、missing route、inactive CID reset、active reset 和终态 cleanup 保持 route 不变量。 | Done |
| P0-D4 | Path validation 与单路径 migration | RFC 9000 path validation 与 migration | Endpoint route update seam | PATH_CHALLENGE/PATH_RESPONSE 与验证后的 route migration 通过 endpoint state 工作；验证失败不提交。 | Done |
| P0-E1 | TLS rollback closure | RFC 9001 与 QUIC 依赖的 TLS 1.3 handshake 输入 | `tls13.zig`、`tls13_backend.zig` | Certificate、SNI、ALPN、transport-parameter、key-share、transcript、Finished、ticket 和 backend failure 不暴露半提交 QUIC state。 | Done |
| P0-E2 | QUIC key phase handling | RFC 9001 1-RTT key update 与 key discard | TLS backend 与 packet protection | Key update、old-key retention、packet-number protection 和 discard deadline 通过 protected endpoint packet 覆盖。 | Done |
| P0-F1 | 第一轮外部互通门禁 | QUIC v1 对一个成熟栈互通 | Process client/server 与 endpoint loop | Clean checkout 通过 `quic-go` 证书校验 handshake、Retry、stream echo/control、close 和一次可控 loss/PTO case。 | Done |
...
| 顺序 | 任务 | 状态 | 提交范围 |
| --- | --- | --- | --- |
| 1 | P0-A1 本地 endpoint TLS handshake | Done | 已增加最小核心 Zig endpoint 测试，证明证书校验的本地 handshake。 |
| 2 | P0-A2 本地 endpoint STREAM/close loop | Done | 扩展同一个本地 loop，覆盖 protected STREAM echo、due timer service、close、Initial key discard 和 route/record cleanup。 |
| 3 | P0-B1 protected bidi/uni stream data | Done | 增加 endpoint-level bidi/uni data 与 FIN 覆盖；只修这条证据需要的 stream/endpoint 代码。 |
| 4 | P0-B2 stream control 与 flow-control credit | Done | 增加 endpoint-level reset/stop、stream-count release 和 flow-control unblock 覆盖。 |
| 5 | P0-B3 stream retransmission requeue | Done | 证明丢失的 stream/control frame 通过 recovery requeue，且不重复提交 stream state。 |
| 6 | P0-C1 ACK/RTT accounting | Done | 在 endpoint seam 关闭 ACK cleanup、RTT、outstanding packet 和 bytes-in-flight 证据。 |
| 7 | P0-C2 loss/PTO/NewReno baseline | Done | 关闭 route-bound CRYPTO/STREAM PTO、loss detection、PTO backoff 与 congestion-window 证据。 |
| 8 | P0-C3 terminal timer disarm | Done | 证明 close/drain/idle/key-discard 终态路径不会留下 stale deadline。 |
| 9 | P0-D1 failure-before-mutation | Done | 为会修改共享状态的畸形 packet/frame 类别补 rollback test。 |
| 10 | P0-D2 Retry/address validation | Done | 关闭认证 Retry/token admission 与 rejection 语义。 |
| 11 | P0-D3 CID/stateless reset lifecycle | Done | 关闭 route/CID/reset cleanup 与 failure-result 语义。 |
| 12 | P0-D4 path validation/migration | Done | 关闭验证后单路径 migration 的 endpoint route-update 行为。 |
| 13 | P0-E1 TLS rollback closure | Done | 关闭剩余会破坏 P0 gate 的 TLS/backend rollback 类别。 |
| 14 | P0-E2 QUIC key phase handling | Done | 通过 endpoint packet 关闭 1-RTT key update/discard 行为。 |
| 15 | P0-F1 quic-go gate | Done | 运行并记录 clean-checkout 第一轮外部互通命令集。 |
...
P0-A1 已完成：`Tls13 endpoints complete certificate-verified local handshake
through endpoint routes` 会解码本地 trust-anchor certificate，把 client 配成
`skip_cert_verify = false`，通过 `Tls13ServerEndpoint` 接纳
`Tls13ServerTransport` record，只转发 route-bound endpoint datagram，再把 client
Finished flight 通过 routed server receive step 送回服务端，并确认 client/server
...
| P6-B | QUIC v2 / RFC 9368 | 版本协商、v2 salt/labels、v2 全流程 | v2 握手 + 传输通过；版本协商回退正常 | Done |
| P6-C | 高级拥塞控制 | BBR 算法；CUBIC 生产加固；算法切换 API | BBR/CUBIC 连接启动时可选；均通过丢包/恢复测试 | Done |
| P6-D | Interop-Runner 完整覆盖 | retry/multiplexing/chacha20/keyupdate/v2 | Docker 中全部标准 testcase 通过 | Done |
| P6-E | QUIC Datagram 扩展 | RFC 9221 DATAGRAM 帧；不可靠数据 API | Datagram 收发正常；丢包不阻塞 stream | Done |
| P6-F | 性能优化 | GSO/GRO 批量 UDP；零拷贝发送；连接池 | 批量收发测试；连接池复用测试 | Done |
| P6-G | 可观测性生产化 | qlog 完整事件；连接指标；错误追踪 | qlog 输出握手+传输；指标可查询 | Done |
| P6-H | WebTransport over HTTP/3 | 双向流 + datagram over H3 | WT client 开双向流；echo 正常；WT datagram 往返 | Done |
## 实用 Transport 基线

| 功能 | 实用目标 | quicz 状态 |
| --- | --- | --- |
| UDP client/server endpoint | 第一轮可用里程碑必须具备。由同一个 endpoint owner 驱动 accept/connect、packet receive/send、timer、route cleanup 和 close。 | 部分完成：`Tls13ServerEndpoint` 持有动态分配或有界的 TLS server record、active connection-handle 枚举、active-connection count/limit/capacity 接纳信号、active record installed-key output polling/draining 与已提交 route path、route-bound application stream send/cancellation polling、多 record pending-work sweep（会上报单连接 deadline planning、单连接 output-poll/drain、跨连接 output-poll/drain、单连接/跨连接显式 output-poll/drain 与 sweep key-discard、销毁已关闭 record、选择下一 deadline、执行 route-bound output drain，并返回 drain 后下一 deadline）、ingress 分类、route-bound Version Negotiation/stateless-reset response pairing、lifecycle routing/timer、route-bound accepted/routed Initial 和 Handshake TLS backend output drain，以及 accepted Initial poll/drain 后 output deadline 选择、capacity-aware accepted-Initial admission drop（保留 caller-owned record 且不留下半安装 route）、endpoint-owned long-packet Initial/Handshake dispatch 与 route-bound output、long-header CRYPTO 与 installed-key short packet 的 routed datagram dispatch、针对 routed datagram 的 classify/process 和 classify/process/bounded-drain 组合 dispatch、一个会同轮清理 idle/close record 的有界 server receive/drain/pending-work socket-loop step、installed-key coalesced Initial/Handshake datagram dispatch，以及 unsupported coalesced long-datagram rejection、route-bound Retry follow-up Initial 验证与 TLS output drain、原子 record 退役、route-owned installed-key short-packet receive、route-bound installed-key feed/output polling、route-bound due recovery output polling/draining 与 drain 后下一 deadline 选择、route-bound protected close 发送、route-bound 显式 close drain 与 drain 后 close deadline 选择及 zero-capacity non-commit guard、通过已提交 route 发出 close-on-frame-error、due idle/close 的 record 与 route 退役、process server 基于已提交 route tuple 的 datagram send selection，以及 process server 验证后 route migration；`Tls13ClientEndpoint` 持有一个 client transport、CID route、调用方选择的 route migration、route-bound Initial begin output、Version Negotiation follow-up route 注册和 endpoint-owned restart Initial 发送、route-bound receive-generated Initial/Handshake output、route-bound application output polling/draining 与 stream send/cancellation polling、route-bound close-on-frame-error output polling/draining 与 drain 后下一 deadline 选择、一个会同轮处理 idle、key-discard、终态 receive 与 close cleanup 的有界 client receive/drain/due-deadline socket-loop step、route-bound due recovery output polling/draining 与 drain 后下一 deadline 选择、recovery-timer mirror、route-bound protected close 发送、route-bound 显式 close drain 与 drain 后 close deadline 选择及 zero-capacity non-commit guard，以及 due idle/close 的 route 与 timer 退役。process client/server 的 UDP receive capacity 已各自与配置的 local datagram limit 对齐。调用方仍持有 UDP I/O 与接纳策略；仍缺少生产级多连接 event-loop policy。 |
| TLS 1.3 集成 | 必须具备。纯 Zig TLS 1.3（`src/quic/tls13.zig` + `src/quic/tls13_backend.zig`）通过 `CryptoBackend` 接口接入；OpenSSL C adapter 为废弃路径。 | 部分完成：client 与 server TLS transport 持有 protected I/O，并统一选择和处理 recovery、idle、close 与 key-discard deadline；每个并发 server record 持有 `Tls13ServerTransport`（Connection、TLS backend，以及本端/对端/Original DCID 的固定副本）。服务端 Certificate 消息保留配置的 leaf-first DER certificate chain，并仅在 server connected 后发出 NewSessionTicket；配置 bundle 的 client 会在信任锚前逐跳校验收到的 leaf-to-issuer chain。纯 Zig TLS client 会校验本地 SNI、ALPN 与 PSK ClientHello 输出边界，会拒绝 unsolicited ServerHello 与 EncryptedExtensions extension，同时拒绝重复 extension type，拒绝缺失、未 offer 或 unsolicited server ALPN selection，拒绝当前不支持的 HelloRetryRequest sentinel 输入，拒绝畸形 ServerHello 基础字段、supported_versions 长度和非法 X25519 key_share，拒绝超出本地上限的本端或对端 transport-parameter extension bytes，拒绝 extension vector 后 trailing bytes，拒绝非空 server Certificate request context，拒绝畸形 CertificateVerify signature vector，拒绝畸形 NewSessionTicket length/vector/ticket 边界、重复的 NewSessionTicket extension type（包括已知 `early_data` 和未知 extension 重复）、zero-lifetime NewSessionTicket、超出上限的 NewSessionTicket lifetime、非法 NewSessionTicket `early_data` 值和不支持的 post-handshake message；resumed ClientHello 和 `CryptoBackend` 0-RTT write-secret hook 只在已存 ticket 带 RFC 9001 `0xffffffff` sentinel 时启用，被拒绝的 0-RTT hook poll 不会 build ClientHello 或修改 outbound CRYPTO，PSK identity 的 `obfuscated_ticket_age` 会保留服务端提供的 `ticket_age_add`。纯 Zig TLS server 会校验 EncryptedExtensions 输出边界，发出 CSPRNG-backed NewSessionTicket `ticket_age_add` 和 RFC 9001 `early_data` sentinel；配置了名称时会校验 ClientHello SNI，拒绝重复 ALPN protocol name，拒绝超长 SNI host_name，拒绝畸形 ClientHello 基础字段与 extension vector length，拒绝非空 ClientHello legacy_session_id，拒绝 ClientHello extensions 后 trailing bytes，拒绝重复的 ClientHello extension type（包括未知 extension 重复），要求 supported_groups，拒绝重复 supported_groups entry，拒绝空 key_share entry、重复的 key_share group、不在 supported_groups 中的 key_share group、key_share 与 supported_groups 顺序不一致和非法 X25519 key_share，拒绝超出本地上限的本端或对端 transport-parameter extension bytes，要求 early_data 为空，要求 pre_shared_key 位于最后，要求兼容的 PSK key-exchange mode，拒绝畸形 pre_shared_key identity/binder vector（包括过小 binder vector、非 SHA-256 binder 长度与 binder 校验不匹配），拒绝 client Finished verify_data 不匹配，发送并解析 ServerHello `pre_shared_key` identity selection，selected-PSK 路径会跳过 Certificate/CertificateVerify，并在 server flight 前校验 signature-algorithm 支持。下方 `quic-go` 证据完成真实 TLS-owned handshake 与两条带 FIN 的 1-RTT STREAM echo，不使用 mock key 或 OpenSSL；仍缺少生产级 endpoint-owned event loop。 |
...
| Stream | 必须具备。Bidirectional/unidirectional stream 的 open、read、write、FIN、reset、STOP_SENDING 和 stream limit 必须能跑在 protected UDP 上。 | 部分完成：TLS-owned client/server transport 已暴露 bidirectional 和 unidirectional 的 open/write/read/FIN，以及 RESET_STREAM 和 STOP_SENDING；`Tls13ClientEndpoint` 和 per-record `Tls13ServerEndpoint` API 现在通过 endpoint-owned 边界暴露 stream open/read/write/FIN 以及 RESET_STREAM/STOP_SENDING 控制。TLS-backed endpoint UDP loopback 已验证 RESET_STREAM 与 FIN 驱动的 MAX_STREAMS_BIDI 额度释放；独立 `quic-go` client 已验证 stream 0 到 4 到 8 到 12 的连续 stream-count 额度释放、RESET_STREAM(41)、STOP_SENDING(42) 后对端回送 RESET_STREAM(42)，以及 client 单向 stream 2 / server 单向 stream 3 的 FIN 交换。更广泛的 stream-limit 互通仍不完整。 |
...
| Connection ID 和 stateless reset | 必须具备。Routing、CID issue/retire、reset-token handling、close cleanup 和 inactive-CID reset emission 必须接入 endpoint lifecycle。 | 部分完成：已有 endpoint router、连接层 reset receive-to-draining 状态、endpoint installed-key feed 的 active-route reset 处理与 recovery timer disarm、client endpoint active reset receive-to-draining 处理以及 receive-step close-deadline 选择、server endpoint bounded receive/drain 与直接 receive-step active reset 上报及 close-deadline record 退役、server endpoint inactive-reset response path pairing、本地生成 stateless reset datagram 的 short-header/fixed-bit 校验、lifecycle helper 和 socket-backed loopback。TLS client/server transport 会为发包选择最新未退役的 peer `NEW_CONNECTION_ID`，并在没有该值时回退到已认证 Initial SCID；zero-length CID 下的 `NEW_CONNECTION_ID` 和 `RETIRE_CONNECTION_ID` protocol-violation guard 会在 CID 或 ACK 状态变化前执行；server 现在只会在已知非空 Initial Source Connection ID 后广告 stateless reset token。并发 TLS process server 现为每个接纳的连接生成独立的 8 字节 CSPRNG Initial SCID。完整 TLS-owned lifecycle 集成仍未完成。 |
...
P0-1a 已完成：`receiveDatagramStepWithRoutePath()` 现在通过一个有界 client result
shape 暴露 receive output、due recovery output、idle/close/key-discard transition、
route error、capacity error 和 step 后 deadline selection。
Server routed installed-key receive/drain 现在会在处理 protected Application-space
datagram 且 ACK output 待发送时，把零 installed-key output 容量报告为
`drain.first_error = BufferTooSmall`。排队的 ACK 会保留到后续 route-bound poll，
server socket loop 不再需要额外 post-receive probe 来判断 receive-generated output
是否仍排队。
P0-A0 已完成：server receive-step API 现在通过有界 result shape 返回 admission、
routed output、pending work、idle/close/key-discard due cleanup、route error、
capacity error 和 step 后 deadline selection。P0-A1 已完成，P0-A2 现在 active；
mock-only 或 installed-key-only 证据不能关闭 protected endpoint-loop 任务。
...
`retire_prior_to > sequence_number`。后续新收到且 sequence 低于最大保留阈值的
CID 会被立即排队退休，不会进入 active CID 池；后续 payload 无效时也有 rollback
覆盖。close-propagating 接收路径会把该畸形 overflow frame 映射为
`FRAME_ENCODING_ERROR`，与 frame decoder 和 RFC 9000 Section 19.15 保持一致。

Endpoint-owned TLS client 和 server close API 现在除了 transport
`CONNECTION_CLOSE`，也会暴露 route-bound `APPLICATION_CLOSE` output。缺失 route
和零容量 drain 会在修改 close state 前被拒绝；client/server route-bound drain 会在已提交
route 上解密到 `APPLICATION_CLOSE`，并选择 close deadline。
...
纯 Zig TLS client 的 `CryptoBackend` hook 现在只会在 TLS 握手完成后暴露
server transport parameters 和 negotiated ALPN。证书校验失败时，即使 TLS 状态机
已经解析 EncryptedExtensions，面向 connection 的 hook 仍不会返回 server 参数、
ALPN、1-RTT secret 或 handshake confirmation。
Connection 级 CRYPTO-frame 驱动现在也有同类回归覆盖：在合法 ServerHello 与
EncryptedExtensions 之后遇到坏 server certificate 时，不会应用已解析的 server
transport parameters，不会安装 1-RTT key，也不会确认握手。
...
certificate、transcript state 和 client handshake state 不变；tag 或 length
畸形的 DER envelope 会在进入 certificate parser 前被拒绝。CertificateVerify
解析现在也会在可选证书链校验策略前
...
identity 以及同下标 binder，而不是固定校验第一个 binder；后续 ServerHello
`pre_shared_key.selected_identity` 也会返回同一个下标。
...
`signature_algorithms` 条目、重复的 `psk_key_exchange_modes` 条目、空证书条目、
过大的本地证书链或编码后的 Certificate flight，以及缺失、过短或其它非法的本地私钥配置。
...
Connection crypto-backend drive 现在会在 backend 已确认握手、且本轮只驱动
Application space、没有待发送 Handshake CRYPTO 时丢弃 Handshake packet-number-space
...
server-side connected TLS backend 现在会在 input buffering 或 NewSessionTicket
副作用前拒绝 peer post-handshake CRYPTO；client-side Application CRYPTO 仍会处理
server NewSessionTicket。
server NewSessionTicket 输出现在只允许由 Application-space pull 触发，late Initial
或 Handshake polling 不会跨空间排队 post-handshake Application CRYPTO。
NewSessionTicket 构造和 client-side 处理现在还要求已派生 Application traffic
secrets，避免 TLS state 被错误标记为 connected 时从未初始化 master secret 派生
resumption PSK。
...
只返回 `InvalidPacket`。
close-propagating connection backend drive 现在会把 peer CRYPTO 输入触发的
TLS backend `CryptoError` 映射为 QUIC `CRYPTO_ERROR` CONNECTION_CLOSE，并使用
CRYPTO frame type，因此 lifecycle poll/drain wrapper 可以发出 close output，而不是
只向调用方暴露裸 backend error。
...
NewSessionTicket 解析现在会先拒绝重复的 extension type，包括已知
`early_data` extension 和重复的未知 extension，再为后续 ClientHello
派生或存储 resumption PSK。
...
ServerHello `pre_shared_key` selection 现在也绑定到当前 ClientHello 的实际
offer state，避免 stale 或外部变化的 ticket store 让 client 接受当前 ClientHello
没有广告过的 PSK selection。
...
Connection-ID 处理现在会执行 RFC 9000 zero-length CID 边界：正在发送
zero-length destination CID 的连接会在 active CID 或 ACK 状态变化前拒绝对端
`NEW_CONNECTION_ID`；已提供 zero-length Initial Source CID 的连接会拒绝本端
`NEW_CONNECTION_ID` 签发，并在本地 CID retirement 前拒绝对端
`RETIRE_CONNECTION_ID`。close-propagating receive path 会把已认证违规映射为
`PROTOCOL_VIOLATION`。
...
Server Retry follow-up 的 route-bound 处理现在会在 token validation 前预检 follow-up
Initial DCID route。缺失 route 会返回 `UnknownConnectionId`，不会消费一次性 Retry
token、不会把 record 标记为 retry-validated，也不会驱动 backend output。
...
调用方持 key 的 1-RTT short OrClose poll/drain helper 现在也对齐同一
Application close 边界：认证后的 frame 错误进入 `closing` 时，会在同一步直接返回或
drain 选中连接上的 protected short close output；未进入 closing 的 invalid packet 仍在
输出前失败。direct bounded-drain helper 在没有输出槽位可承载该 close 时返回
`BufferTooSmall`，并保留 pending close 供后续 poll 发出。
显式 key-update 与 caller-owned key-phase 1-RTT OrClose helper 也采用同一边界；
认证后的 frame 错误排队 close output 时，caller-owned key-phase state 仍会在
key-phase advancement 前保持不变。
...
accepted Initial backend OrClose poll 和 bounded-drain helper 现在也对齐输出型
边界：同一 backend 错误会在触发步骤返回或 drain protected Initial
CONNECTION_CLOSE，同时不会拉取普通 backend CRYPTO output。
...
`Tls13ClientEndpoint` 现在通过 endpoint-owned API 转发单向 stream open、
RESET_STREAM 和 STOP_SENDING；聚焦测试证明这些控制会进入同一个 owned connection
队列，调用方不需要绕过 endpoint 边界。
`Tls13ServerEndpoint` 现在通过 per-record endpoint-owned API 转发 stream
open/read/write/FIN、RESET_STREAM 和 STOP_SENDING。bounded record 测试证明
server-initiated stream ID 与取消队列都留在选中的 owned record 上，未知 handle
返回 `UnknownConnectionId`。
`Tls13ServerEndpoint.sendStreamWithRoutePath()` 现在会先解析已提交 route，再
queue application data，并把 protected stream datagram 与该 route 一起返回。
聚焦测试证明缺失 handle 不会绕过 endpoint 边界，stream output 会绑定到选中
record 的 route。
...
route 时不会先 queue STREAM data、pending RESET_STREAM 或 STOP_SENDING 状态。
...
connection 仍保持 active。server 显式 close drain 现在只 drain 被选中 record 的
close output，其他 active record 的 pending output 会保留给各自的 poll step。
...
重放包（`stale_rejected=true`）。单连接/跨连接 key-discard 的 due-deadline
poll/bounded-drain 路径也已有覆盖：它们只清理保留 key，不消费仍应由正常 poll 持有的
ACK/output。
client endpoint 现在也有无输入 due-deadline poll 和 bounded-drain 的 `.key_discard`
直接覆盖：socket loop 可以在没有先收到 datagram 的情况下服务 retained-key expiry，
不产生 recovery output，也不退役已提交 route。
server endpoint 也有对应的无输入 due-deadline 直接覆盖，涵盖 route-path 与非
route-path bounded-drain 入口：socket owner 可以在没有先收到 datagram 的情况下服务
retained-key expiry，不做 output drain，并保留 active record 与已提交 route。
...
recovery timer，以及 lifecycle 的 deadline、due-recovery poll view 与 installed-key receive view。因此容量以内的 record 接纳、CID route 建立与 timer arm 都不会扩容。capacity-aware admission helper 会把容量满的 Initial drop 作为数据返回，同时保留 caller-owned record，并证明没有安装 lifecycle route。不同 tag 使 Initial DCID 和 SCID 不会冲突。每个 FIN-terminated echo 匹配后，客户端都会发送受保护的
...
routing 状态。
endpoint-owned deadline selection、跨 record output polling 和 installed-key receive
...
这些 telemetry helper 仍可通过 const endpoint view 调用，因此只读 socket-loop
capacity policy 不需要持有 mutable endpoint reference 也能检查 active capacity。
...
| Streams | 部分接收重组与重复重传丢弃 + pending range 重复前缀/后缀/中间重叠裁剪 + final data 后 late duplicate STREAM 丢弃 + conflicting late STREAM 拒绝/close 分类 + FIN completion + 本地 reset/stop 可观测 + 公开 `StreamState` 快照（含 Data Read/Reset Read 接收状态和 Data Acked/Reset Acked 发送状态）+ 隐式低编号接收 stream 创建 + pre-STREAM peer-bidirectional STOP_SENDING 处理 | 在当前内存态重组骨架之外继续完成 stream 状态机、FIN/reset 规则和读写行为。 | 双向、单向、FIN、reset、STOP_SENDING、stream 状态快照、乱序、重复重传、冲突重叠、回滚、final-size 测试。 |
...
| Stateless reset | 部分 helper + constant-time token match + 连接层检测 + 连接层 reset receive-to-draining 状态 + NEW_CONNECTION_ID token uniqueness checks + endpoint inactive-CID reset datagram construction + 通过 `EndpointConnectionLifecycle` 暴露的 route/reset/drop receive classification + socket-backed UDP reset emission loopback + socket-backed close-triggered lifecycle route retirement/reset loopback | 把 reset emission 接入 socket 拥有的 endpoint lifecycle 和 connection close/drop policy。 | 现有测试覆盖 reset token 命中、误判拒绝、短 datagram 拒绝、跨 CID duplicate-token 拒绝、retired token 忽略、active token receive 使连接进入 draining、reset receive 后抑制 pending close output、active route token 抑制、retired route token lookup、inactive-route reset datagram construction、smaller-than-trigger sizing、route/reset/drop receive action classification、lifecycle-owned route/reset/drop after timer-disarming retirement、client endpoint active-reset receive-step 可见性、recovery timer disarm 和 reset 后 close deadline 选择、server endpoint route-bound 与 receive-step active reset 上报及 close-deadline record/route cleanup、server endpoint inactive-reset response path pairing，以及 ambiguous reset-token CID 拒绝；`run-stateless-reset` 演示 endpoint inactive-CID reset action；`run-udp-stateless-reset-loopback` 演示真实 UDP active-route suppression、unknown-CID drop classification、退役后的 trigger 投递、reset 发出和客户端 token 匹配；`run-udp-close-lifecycle-loopback` 演示 protected close delivery、lifecycle-routed protected receive auto-close、`EndpointConnectionLifecycle` connection-handle route retirement、保留 reset token lookup、reset emission 和 client token matching；后续 endpoint 测试覆盖完整 TLS-owned lifecycle 集成。 |
...
- 2026-07-22：Connection-level `NEW_TOKEN` 校验现在补充了畸形空 token
  ...
  queue ACK；`processDatagramOrClose()` 会把同一 frame 映射为
  `FRAME_ENCODING_ERROR` `CONNECTION_CLOSE`。

- 2026-07-22：Endpoint-owned TLS client/server API 现在暴露 protected
  `APPLICATION_CLOSE` polling 和 route-bound drain helper。聚焦测试覆盖
  missing-route 与 zero-capacity non-commit、已提交 route 上的 `APPLICATION_CLOSE`
...
- 2026-07-17：收紧纯 Zig TLS server 的 ClientHello extension 校验。
  ...
  type ID，即使重复的是本来会被忽略的未知 extension，也会按 RFC 8446
  duplicate-extension 边界返回错误。

- 2026-07-17：收紧本地 transport-parameter 配置校验。
  `Connection.init()` 现在会在 TLS extension 导出前拒绝不能编码为合法
  RFC 9000 transport parameter 的本地 max UDP payload size、idle timeout、
  initial data limit 和 active connection ID limit。

- 2026-07-19：收紧纯 Zig TLS backend 的本地 transport-parameter 交接。
  `Tls13Backend.set_local_transport_parameters` 现在会在编码后的字节超过 TLS
  handshake 固定本地 transport-parameter buffer 时返回 `CryptoError`，
  不再在 ClientHello 或 EncryptedExtensions transcript 构造前截断 extension bytes。

- 2026-07-19：收紧纯 Zig TLS backend pull hook。peer transport-parameter 和
  negotiated-ALPN callback 现在会在调用方 scratch buffer 无法容纳完整值时返回
  `BufferTooSmall`，并保持 one-shot sent flag 不变，让后续更大的 buffer 仍能拉取
  精确的 TLS 结果。

- 2026-07-17：对齐 RFC 9002 ACK processing 的 no-newly-acked 边界。
  对当前 in-flight packet 没有任何新确认的重复或陈旧 ACK frame，现在会在
...
- 2026-07-17：按 packet number space 对齐 RFC 9002 persistent congestion
  ...
  并用单独的 Application 阈值测试证明 peer `max_ack_delay` 项会阻止过早
...
- 2026-07-17：收紧 RFC 9000 STREAM final data 后的重复数据处理。late
  ...
  的字节，现在会按 stream-data protocol violation 失败，并覆盖 OrClose close
  分类路径。

- 2026-07-17：扩展 out-of-order STREAM 对 pending range 的重复数据裁剪。
  接收路径现在会接受与 pending stream data 字节相同的前缀/后缀重叠，
  完全落在 pending data 内的重复 subrange 会被忽略；改写字节仍按 invalid
  stream data 拒绝。

- 2026-07-17：扩展 pending STREAM 重复数据裁剪到中间重叠。incoming
  STREAM data 如果用相同字节覆盖已有 pending range，现在会被拆成剩余的新
  前缀和新后缀 gap；接收侧流控只按这些新字节增长，改写重叠字节仍按
  invalid stream data 拒绝。

- 2026-07-17：扩展 out-of-order CRYPTO 对 pending range 的重复数据处理。
  CRYPTO frame 如果用相同字节覆盖已有 pending bytes，现在会被拆成剩余的新
  gap；改写重叠字节仍作为 invalid payload 失败并回滚。

- 2026-07-17：为 close-propagating receive path 增加冲突 CRYPTO overlap
  分类。`processDatagramOrClose()` 现在会为改写连续 CRYPTO buffer 或 pending
  range 已有字节的 CRYPTO frame 排队 `PROTOCOL_VIOLATION`
  CONNECTION_CLOSE；只回滚的旧 receive API 仍保持返回 `InvalidPacket` 且不排队 close。
...
- 2026-07-17：对齐 client-side Version Negotiation discard state 与
  ...
  后续 Version Negotiation packet 也会被忽略，不会选择 follow-up version，
  也不会改写既有状态。
...
- 2026-07-17：为 client endpoint 增加 route-bound 显式 close drain。
  `Tls13ClientEndpoint.closeWithRoutePathAndDrainDatagrams()` 会排队 protected
  application CONNECTION_CLOSE，按已提交 route 做有界 drain，并返回 drain 后的 close
  deadline，socket loop 不需要再额外 poll 一次才能更新等待目标。

- 2026-07-17：为 server endpoint 增加 route-bound 显式 close drain。
  `Tls13ServerEndpoint.closeWithRoutePathAndDrainDatagrams()` 会为一个
...
- 2026-07-20：把 endpoint registry 的 pre-closed cleanup 扩展到 deadline
  selection、跨 record output polling 和 installed-key receive view construction。
...
- 2026-07-20：保留 server endpoint active telemetry 的只读调用边界。
  `activeConnectionCount()`、`hasConnectionCapacity()` 和
...
- 2026-07-21：新增 server installed-key receive wakeup 的 scratch-backed
  入口。`Tls13ServerEndpoint.feedDatagramWithInstalledKeysAndSelectNextDeadlineWithScratch()`
...
- 2026-07-21：新增 server receive-to-pending-work route-bound drain 的
  scratch-backed 入口。
  `Tls13ServerEndpoint.feedDatagramWithInstalledKeysAndProcessPendingWorkAndDrainDatagramsWithRoutePathWithScratch()`
  在 receive 前预检查 receive、deadline 与 poll scratch，pending work 后复用现有
  route-bound bounded drain。

- 2026-07-21：新增 server receive-to-pending-work route-bound poll 的
  scratch-backed 入口。
  `Tls13ServerEndpoint.feedDatagramWithInstalledKeysAndProcessPendingWorkAndPollDatagramWithRoutePathWithScratch()`
...
- 2026-07-21：新增 server route-bound installed-key receive drain 的
  scratch-backed 入口。
  `Tls13ServerEndpoint.feedInstalledKeyDatagramAndDrainWithRoutePathWithScratch()`
...
- 2026-07-21：新增 server classified datagram dispatch 的 scratch-backed
  入口。`Tls13ServerEndpoint.processDatagramWithRoutePathWithScratch()`
...
- 2026-07-20：加固 route-bound 显式 close drain 的零输出容量路径。
  client 与 server endpoint 的 close-and-drain helper 现在会在没有 output slot
...
- 2026-07-22：加固 server ClientHello supported-version vector validation。
  纯 Zig TLS server parsing 现在会拒绝重复的 `supported_versions` 条目，并保持
  parsed ClientHello state 与 transcript progress 未提交。
...
- 2026-07-17：收紧 protected long-header length-field budget。protected long
  datagram length prediction 现在会在编码后的 QUIC Length 字段超过 varint 上限时返回
  `BufferTooSmall`。
...
- 2026-07-17：收紧 NEW_CONNECTION_ID wire-length 校验。pending local-CID
  ...
  sequence number、不可编码的 `retire_prior_to`、非法 retire ordering 和非法本端
  CID 长度。
...
- 2026-07-16：`CryptoBackend` client 0-RTT write-secret hook 现在受已存
  ...
  RFC 9001 sentinel 时，不再安装 client 0-RTT packet key；测试覆盖这两个拒绝路径
  和允许 ticket 路径。

- 2026-07-16：补齐 TLS 1.3 ServerHello `pre_shared_key` selection 路径。
  服务端在 PSK binder 校验通过后会在 ServerHello 中选择 identity 0，客户端会解析该
  selection；selected-PSK handshake 会跳过 Certificate/CertificateVerify，并完成
  application traffic secret 匹配的握手。

- 2026-07-16：收紧纯 Zig TLS PSK binder 处理。配置了服务端 PSK 时，如果
  ClientHello 的 PSK binder 与服务端 PSK 校验不匹配，server 现在直接返回
  `BadFinished`，不再带着 `peer_psk_binder_valid=false` 继续 handshake。聚焦测试
  覆盖 binder 不匹配失败，并确认不会派生 0-RTT 接收 secret。

- 2026-07-17：新增 malformed PSK binder vector 直接覆盖。纯 Zig TLS server
  现在有聚焦测试证明：过小的 ClientHello `pre_shared_key` binders vector 会在
  binder 校验或 PSK ticket selection 前以 `DecodeError` 拒绝。
...
- 2026-07-19：将 client-side ServerHello `pre_shared_key` selection 绑定到当前
  ClientHello offer。纯 Zig TLS client 现在会拒绝当前 ClientHello 未广告
  `pre_shared_key` 时收到的 PSK selection，即使 ServerHello 处理前本地 ticket/PSK
  store 已被填充。
...
- 2026-07-19：收紧纯 Zig TLS client ServerHello 失败状态覆盖。
...
- 2026-07-16：`Tls13ServerEndpoint` 新增 routed datagram dispatcher。socket
  ...
  现在也有 classify-and-process 组合 wrapper：非 routed Initial、Version Negotiation、
  stateless reset 和 dropped datagram 仍返回给调用方，routed long/short packet 则留在
  endpoint owner 内处理。后续 classify/process/bounded-drain wrapper 可在 routed
  short-packet receive 后继续 drain route-bound installed-key output，包含 path-validation
  output 和 drain 后 recovery deadline。测试覆盖 routed Initial、Handshake、
  short-packet dispatch、routed short receive 加 bounded drain，以及非 routed
  Version Negotiation；未修改 examples。
  `receiveDatagramStepWithRoutePath()` 现在提供更短的有界 server socket-loop step：
  一次完成 classify/process/drain、endpoint-owned pending work sweep、route-bound
  recovery output drain，并返回下一 endpoint deadline。测试覆盖接收非 routed
  Version Negotiation 的同时，为已有 record 服务 recovery output。
  `Tls13ClientEndpoint.receiveDatagramStepWithRoutePath()` 现在给 client endpoint
  提供匹配的有界 receive/due-deadline step。测试证明非 closing route mismatch
  会隔离 receive output，同时同一步 drain route-bound Application recovery output
  并返回下一 deadline。

- 2026-07-16：`EndpointRoutedDatagramResult` 和
...
- 2026-06-10：新增 `EndpointAcceptedInitialCryptoBackendDatagramResult` 和
  ...
  且 client 能解开 backend 产出的 response。poll result 现在也会返回 output 后的
  Initial recovery deadline。
- 2026-06-10：新增
...
- 2026-06-10：新增 `EndpointAcceptedInitialCryptoBackendDatagramDrainResult`、
  ...
  bytes。bounded-drain result 现在也会返回 drain 后的 Initial recovery deadline。
- 2026-06-10：新增
...
- 2026-06-10：新增 `EndpointPendingWorkDatagramDrainResult` 和
  ...
  deadline 前 no-op 和 Application PTO 到期后 1-RTT PING probe 路径。result 现在还会在
  no-op 与 due-drain 路径后暴露下一 endpoint-visible deadline，与 due-deadline
  bounded-drain helper 对齐。
...
- 2026-06-18：把 QUIC wire-length 预算 helper 拆到 `src/quic/wire_len.zig`，
  ...
  切片预算。新增单元测试覆盖 varint 边界、非法 packet-number length、long/short-packet
  payload overflow 拒绝、Initial/short datagram 最小长度扩展、跨 varint 长度扩展点的
  STREAM/CRYPTO payload sizing，以及 oversized remaining-byte budget，证明该拆分不改变发送路径的长度预算语义。
...
- 2026-05-29：新增 RFC 9002 max_datagram_size recovery resync from peer
  ...
  initial congestion window，并清空 congestion-avoidance credit；如果 peer
  limit 增大且当前 window 低于新的 minimum window，则 recovery 会把 congestion
  window 抬到新的 minimum 并清空旧的 congestion-avoidance credit。测试覆盖
...
- 2026-05-29：新增 RFC 9002 largest-acknowledged RTT sampling。ACK frame
  ...
  connection-level ACK 路径现在也直接覆盖 peer ACK-delay exponent 解码和
  `max_ack_delay` 截断之后的 Application 样本。
...
- 2026-07-17：新增 client endpoint receive-step active-reset 覆盖。
  `Tls13ClientEndpoint.receiveDatagramStepWithRoutePath()` 现在有直接测试证明：
  认证后的 active-route stateless reset 会保留在 receive result 中可见，
  disarm endpoint recovery timer，在 drain timeout 前保留 route，并返回 reset
  后的 close deadline。
- 2026-07-17：新增 server endpoint receive-step active-reset 覆盖。
  `Tls13ServerEndpoint.receiveDatagramStepWithRoutePath()` 现在有直接测试证明：
  认证后的 active-route stateless reset 会通过 socket-loop step 可见，disarm
  recovery timer，在 close timeout 前保留 record route，并返回 reset 后的
  close deadline。
...
- 2026-05-23：新增重复 STREAM 重传的幂等接收处理。已经存在于连续接收
  ...
  字节，完全相同的 pending range 会被忽略；与 pending range 字节相同的
  前缀/后缀重叠会被裁剪，相同字节的中间重叠会被拆成剩余的新 gap，
  完全落在 pending data 内的 subrange 会被忽略，
  且不会重复增长接收侧流控记账。改写字节仍按无效 payload 失败。测试覆盖连续重复、
  后缀裁剪、pending range 重复、pending 前缀/后缀 overlap 裁剪、pending
  中间 overlap 拆分、冲突重叠拒绝和回滚；
  `examples/uni_stream.zig` 演示 client 发起单向 stream 的重复重传丢弃。
- 2026-05-23：新增 Data Recvd 后 late STREAM 丢弃。所有 final size 之前的
  字节都已经进入连续 buffer 后，重复相同字节的 late STREAM frame 会被忽略，
  且不会增加流控记账。改写已有 offset 字节的 conflicting late STREAM 现在会按
  stream-data protocol violation 失败。测试覆盖重复丢弃、FIN 后冲突 late data、
  OrClose close 分类，以及 final-size 违规错误保留。
...
- 2026-05-23：新增按 packet number space 隔离的乱序 CRYPTO 接收缓冲。
  ...
  重传会被忽略，相同字节的 pending overlap 会拆成剩余的新 gap，
  冲突重叠仍作为无效 payload 失败并回滚。测试覆盖乱序投递、
  连续和 pending 重复数据、pending 中间 overlap 拆分、冲突拒绝以及 pending 状态回滚；
...
| `loss_recovery` | 当前 frame-payload invalid ACK range rejection、duplicate/no-newly-acked ACK recovery side effect suppression、largest-acknowledged RTT sampling、跨 packet number space bytes-in-flight 拥塞发送准入、packet-threshold loss、time-threshold loss、aggregate loss-time timer service、NewReno underutilized-cwnd suppression、slow-start/congestion-avoidance 字节计数与 batched-ACK 增长、recovery period、recovery-period ACK accounting without congestion growth、loss/CE-driven 新拥塞事件后一次性 STREAM recovery probe、minimum-window ssthresh clamp、不受 PTO backoff 放大的 persistent congestion、persistent-congestion min-RTT refresh、persistent-congestion recovery-period 清理/重新进入、非连续 persistent-congestion 抑制与 ACK-delay 处理。 | 已存在 |
