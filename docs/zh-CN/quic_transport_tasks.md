...
| `tls_backend_adapter` | 可运行的 C-ABI `TlsBackend` adapter 契约检查：在绑定具体 C TLS 库之前，先通过现有 `CryptoBackend` drive 路径验证本端/对端 transport-parameter byte handoff、入站/出站 CRYPTO byte 投递、输入 callback status 拒绝、Handshake traffic-secret 安装和 handshake confirmation。 | 已存在 |
