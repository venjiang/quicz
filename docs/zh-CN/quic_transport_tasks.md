...
| Flow control | 必须具备。Connection、stream 和 stream-count flow control 必须能跑在 protected UDP 上。 | 部分完成：已有 frame-payload 和 protected loopback 覆盖；quic-go client 已跨 Zig server 2 KiB 初始 stream window 完成 4 KiB stream echo。 |
