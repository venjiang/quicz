...
- 2026-06-05：新增 `examples/tls_openssl_pair_transcript.zig` 和一个小 C
  ...
  Handshake/1-RTT traffic-secret callback；示例会记录 keylog callback 次数和字节数，
  但不打印 key material。Zig 侧现在会复制这些生成的 CRYPTO bytes，
  以 frame-payload 形式投递到 quicz Initial/Handshake/Application CRYPTO 队列，并按
...
- 2026-06-04：扩展 `examples/tls_openssl_backend_adapter.zig`，让
  ...
  socket/lifecycle loop owner；示例会打印 `transcript_keylog=5/5/773/773` 证明完整
  OpenSSL pair transcript 的 keylog callback/bytes，也会打印 `adapter_keylog=0/0`
