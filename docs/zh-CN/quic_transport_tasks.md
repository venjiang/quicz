...
| ECN validation | 部分 frame-payload ACK_ECN 校验 + endpoint UDP-path ECN state policy | 等 socket packetization 存在后把 ECN validation 绑定到真实 IP ECN 标记。 | 现有测试覆盖 ECT(0) 成功、缺少 ACK_ECN 失败、counter 不足、counter 总量超过已发送 ECT packet、reordered ACK 处理、回滚，以及 endpoint path-identity state isolation；后续 endpoint 测试覆盖真实 IP-header marking。 |
