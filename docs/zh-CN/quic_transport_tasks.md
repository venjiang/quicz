...
纯 Zig TLS client-side ServerHello 和 EncryptedExtensions 解析现在会忽略未知
extension type，同时继续拒绝重复 extension type；ServerHello `pre_shared_key`
selection 不再要求必须位于最后，后续未知 extension 仍可被忽略。
