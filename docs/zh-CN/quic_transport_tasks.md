...
- 2026-05-26：新增 `examples/udp_congestion_recovery_loopback.zig` 和
  ...
  window。完整 protected-packet data retransmission 仍待实现。
- 2026-05-26：新增 `examples/udp_loss_recovery_loopback.zig` 和
  ...
  cleanup 都能走真实 UDP route。完整 protected-packet data retransmission
  仍待实现。
- 2026-05-26：新增 `examples/udp_pto_recovery_loopback.zig` 和
  ...
  作为 protected probe 发送，最后完成 ACK cleanup。完整 protected-packet data
  retransmission 仍待实现。
...
- 2026-05-23：让 `checkPtoTimeouts()` 在新增 PING probe 前优先使用已经排队的
  ...
  probe packet。测试覆盖 queued STREAM probe selection 且不会额外排队 PING；完整克隆
  已发送 protected data 做重传仍待实现。
