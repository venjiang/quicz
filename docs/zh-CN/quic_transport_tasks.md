...
- 2026-06-05：把当前 C TLS 示例边界迁移到 Zig 0.16 的 `addTranslateC`
  ...
  `handshake_confirmed` callback，在 peer transport parameters、Handshake
  secrets 和 1-RTT secrets 均可用后报告确认，因此
...
- 2026-06-04：扩展 `examples/tls_openssl_backend_adapter.zig`，让
  ...
  server，且不会重复交付重复 STREAM data；OpenSSL-backed `handshake_confirmed`
  callback 确认 client 后，通过 no-output Handshake drive 丢弃 client
  Handshake packet-number space 和 keys；配对 loopback server 侧仍在示例中显式
  丢弃；Initial、Handshake、Application echo、
