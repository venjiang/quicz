...
- 2026-05-29：新增 socket-backed UDP address-validation loopback 覆盖。
  ...
  snapshot restore rejection，以及后续 server address-validation unblocking。
...
| `udp_address_validation_loopback` | Socket-backed loopback UDP address-validation：lifecycle-routed protected HANDSHAKE_DONE/NEW_TOKEN delivery、NEW_TOKEN path/version binding、replay snapshot restore rejection，以及后续 server address-validation unblocking。 | 已存在 |
