...
| TLS 1.3 集成 | 必须具备。使用支持 QUIC transport-parameter 和 traffic-secret hook 的 C TLS 库，通过很小的 Zig `TlsBackend` adapter 接入；不在仓库内自研 TLS。 | 部分完成：已有 mock `CryptoBackend`、installed-key handoff、带测试的 C-ABI `TlsBackend` adapter、C-object adapter probe、OpenSSL QUIC TLS API/link probe、按 protection level 分离 CRYPTO handoff 并映射到 quicz CRYPTO 队列，覆盖 client Initial flight protected Initial long-packet 投递，使用 OpenSSL 产出的 Handshake secrets 完成双向 installed-key protected Handshake 投递和 socket-backed Handshake 投递，并使用 OpenSSL 产出的 1-RTT secrets 完成 installed-key protected STREAM request/response 和 socket-backed STREAM echo 的 OpenSSL client/server callback-mode transcript 检查，以及可产出第一段 TLS CRYPTO flight，并让 peer transport parameters、Handshake/1-RTT secrets 和入站 CRYPTO 经 OpenSSL callback 边界进入连接层的 OpenSSL-backed adapter wrapper；缺少完整 endpoint-owned live TLS handshake/socket loop。 |
...
- 2026-06-05：新增 `examples/tls_openssl_pair_transcript.zig` 和一个小 C
  ...
  Initial long-packet helper 组包，并验证 server 侧投递；随后会把 OpenSSL 产出的
  Handshake secrets 安装进 quicz，验证双向 protected Handshake CRYPTO 投递，包括通过
  quicz endpoint lifecycle 的 loopback UDP 投递；也会安装 OpenSSL 产出的 1-RTT
  secrets，并验证 short packet 上的 installed-key protected STREAM request/response；
  随后用同一组 1-RTT secrets 通过同一个 lifecycle 驱动 loopback UDP STREAM echo。完整
