...
| 外部互通 | 声称第一轮可用 transport 里程碑前必须具备。 | 部分完成：客户端专用二进制已与本机独立实现的服务端完成证书校验的 QUIC/TLS 握手；外部 STREAM transfer 和更广泛场景尚未验证。 |
...
依次处理，再处理其后的 1-RTT short-header packet。针对使用 ECDSA P-256 证书的本机独立
服务端，命令输出
