...
| RFC 9002 loss detection 与 congestion control | Partial | ACK delay scaling/capping、packet/time-threshold loss、ACK 驱动的 frame-payload STREAM/CRYPTO 和 protected CRYPTO retransmission requeue、NewReno recovery-period 行为、persistent congestion、带 Initial/Handshake max_ack_delay suppression 的 packet-space PTO PING/new-data/in-flight-CRYPTO/in-flight-STREAM probe hook、ECN validation 测试和 socket-backed UDP loss-recovery/congestion-recovery/PTO-recovery/STREAM-retransmission loopback。 | 仍需 protected-packet PTO/loss timer 完整调度、剩余 0-RTT retransmission 覆盖和剩余 NewReno 细节。 |
...
| RFC 9002 recovery | 部分 ACK delay + packet/time-threshold loss + ACK 驱动的 frame-payload STREAM/CRYPTO 和 protected CRYPTO retransmission requeue + NewReno recovery period + persistent congestion + 带 Initial/Handshake max_ack_delay suppression 的 packet-space PTO PING/new-data/in-flight-CRYPTO/in-flight-STREAM probe hook + socket-backed UDP loss-recovery、congestion-recovery、PTO-recovery 和 STREAM-retransmission loopback | 实现完整 protected-packet PTO/loss timer 调度、剩余 0-RTT retransmission 覆盖和剩余 NewReno 细节。 | 现有测试覆盖 ACK、ACK delay exponent scaling、handshake confirmed 后 max_ack_delay 截断、packet-threshold loss、ACK 驱动和 timeout 驱动的 time-threshold loss、ACK 驱动 lost STREAM 和 frame-payload/protected CRYPTO requeue、retransmission requeue 后 invalid-payload rollback、NewReno recovery-period 抑制、persistent congestion、packet-number-space PTO PING 排队/backoff、Initial/Handshake PTO deadline 不计 max_ack_delay、queued STREAM data probe selection、PTO 驱动 in-flight CRYPTO/STREAM retransmission 和拥塞窗口算术；`run-crypto-stream` 覆盖 frame-payload Handshake CRYPTO loss requeue/retransmission 和 protected 1-RTT CRYPTO ACK-loss requeue/retransmission；`run-pto-recovery` 覆盖 protected 1-RTT CRYPTO PTO probe selection；`run-udp-loss-recovery-loopback` 覆盖 UDP 上的 protected short PING delivery、protected ACK 驱动的 packet-threshold loss removal 和 timeout 驱动的 time-threshold cleanup；`run-udp-congestion-recovery-loopback` 覆盖 UDP 上的 protected short PING delivery、NewReno recovery-period 内重复 loss reduction 抑制，以及 persistent congestion 降到 minimum window；`run-udp-pto-recovery-loopback` 覆盖 ACK loss 触发的 protected UDP packet PTO、PING fallback probe delivery、queued STREAM data 作为 PTO probe、in-flight STREAM 和 CRYPTO data 作为 PTO probe、重复 receive range/CRYPTO range 丢弃和 ACK cleanup；`run-udp-stream-retransmission-loopback` 覆盖 protected UDP packet 上 ACK 驱动的 1-RTT STREAM retransmission 和 final ACK cleanup。剩余可控时钟测试覆盖完整 protected-packet PTO/loss timer 调度和 0-RTT retransmission。 |
...
- 2026-05-27：新增 PTO 驱动的 protected CRYPTO retransmission probe。
  ...
  剩余工作是完整 protected-packet PTO/loss timer scheduling、0-RTT retransmission
  和剩余 NewReno 细节。
- 2026-05-26：新增 ACK 驱动的 protected CRYPTO retransmission requeue。
  ...
  PTO probe；剩余工作是完整 protected-packet PTO/loss timer scheduling、
  0-RTT retransmission 和剩余 NewReno 细节。
- 2026-05-26：新增 ACK 驱动的 frame-payload CRYPTO retransmission requeue。
  ...
  CRYPTO PTO probe；剩余工作是 protected-packet PTO/loss timer scheduling 和
...
- 2026-05-26：扩展 PTO probe hook，让 Application space 在没有更新的
  ...
  丢弃和 final ACK cleanup。后续更新已覆盖 protected CRYPTO PTO probe；0-RTT
...
- 2026-05-26：新增 ACK 驱动的 STREAM retransmission requeue。已发送的
  ...
  幂等丢弃和 final ACK cleanup。后续更新已覆盖 protected CRYPTO PTO probe；
  0-RTT retransmission 与完整 protected-packet timer scheduling 仍待实现。
- 2026-05-26：新增 `examples/udp_congestion_recovery_loopback.zig` 和
  ...
  已单独覆盖；0-RTT retransmission 仍待实现。
- 2026-05-26：新增 `examples/udp_loss_recovery_loopback.zig` 和
  ...
  与 protected CRYPTO PTO probe 已单独覆盖；0-RTT retransmission 仍待实现。
- 2026-05-26：新增 `examples/udp_pto_recovery_loopback.zig` 和
  ...
  PTO probe；0-RTT retransmission 仍待实现。
...
- 2026-05-23：让 `checkPtoTimeouts()` 在新增 PING probe 前优先使用已经排队的
  ...
  protected CRYPTO data、ACK-lost frame-payload CRYPTO data 和 ACK-lost
  protected CRYPTO data 的克隆；0-RTT retransmission 仍待实现。
