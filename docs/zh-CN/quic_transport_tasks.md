...
| `codec_roundtrip` | 演示 varint、packet header、RFC 9369 QUIC v2 long-header type-bit 映射、short-header spin-bit 保留、long/short-packet envelope、header packet number 截断/重建、packet number 编码、RFC 8999 Version Negotiation packet codec、reserved-version skip、client-side VN selection 和 RFC 9368 downgrade-check state、frame、包含 RFC 9368 `version_information` 的 transport parameter、连接层参数暴露（含 TLS extension bytes 和 server preferred_address）与包含 `VERSION_NEGOTIATION_ERROR` 的 transport error codec。 | 已存在 |
