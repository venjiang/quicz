...
| `codec_roundtrip` | 演示 varint、packet header、RFC 9369 QUIC v2 long-header type-bit 映射、short-header spin-bit 保留、long/short-packet envelope、header packet number 截断/重建、packet number 编码、RFC 8999 Version Negotiation packet codec、reserved-version skip、client-side VN selection、follow-up config propagation、RFC 9368 downgrade-check state 和 `VERSION_NEGOTIATION_ERROR` close-code 证据、frame、包含 RFC 9368 `version_information` 的 transport parameter、连接层参数暴露（含 TLS extension bytes 和 server preferred_address）与包含 `VERSION_NEGOTIATION_ERROR`、transport-parameter `TRANSPORT_PARAMETER_ERROR`、frame decode `FRAME_ENCODING_ERROR`、packet-type `PROTOCOL_VIOLATION` 分类的 transport error codec。 | 已存在 |
...
| `loss_recovery` | 当前 frame-payload invalid ACK range rejection、largest-acknowledged RTT sampling、跨 packet number space bytes-in-flight 拥塞发送准入、packet-threshold loss、time-threshold loss、aggregate loss-time timer service、NewReno underutilized-cwnd suppression、slow-start/congestion-avoidance 字节计数与 batched-ACK 增长、recovery period、recovery-period ACK accounting without congestion growth、新拥塞事件后一次性 STREAM recovery probe、minimum-window ssthresh clamp、不受 PTO backoff 放大的 persistent congestion、persistent-congestion min-RTT refresh、非连续 persistent-congestion 抑制与 ACK-delay 处理。 | 已存在 |
