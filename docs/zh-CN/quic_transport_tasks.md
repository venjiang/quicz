...
| Streams | 部分接收重组与重复重传丢弃 + pending range 重复前缀/后缀裁剪 + final data 后 late duplicate STREAM 丢弃 + conflicting late STREAM 拒绝/close 分类 + FIN completion + 本地 reset/stop 可观测 + 公开 `StreamState` 快照（含 Data Read/Reset Read 接收状态和 Data Acked/Reset Acked 发送状态）+ 隐式低编号接收 stream 创建 + pre-STREAM peer-bidirectional STOP_SENDING 处理 | 在当前内存态重组骨架之外继续完成 stream 状态机、FIN/reset 规则和读写行为。 | 双向、单向、FIN、reset、STOP_SENDING、stream 状态快照、乱序、重复重传、冲突重叠、回滚、final-size 测试。 |
...
- 2026-05-23：新增重复 STREAM 重传的幂等接收处理。已经存在于连续接收
  ...
  前缀/后缀重叠会被裁剪，完全落在 pending data 内的 subrange 会被忽略，
  且不会重复增长接收侧流控记账。改写字节仍按无效 payload 失败。测试覆盖连续重复、
  后缀裁剪、pending range 重复、pending 前缀/后缀 overlap 裁剪、冲突重叠拒绝和回滚；
