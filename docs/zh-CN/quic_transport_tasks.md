...
| TLS 1.3 集成 | 必须具备。使用支持 QUIC transport-parameter 和 traffic-secret hook 的 C TLS 库，通过很小的 Zig `TlsBackend` adapter 接入；不在仓库内自研 TLS。 | 部分完成：已有 mock `CryptoBackend`、installed-key handoff、带测试的 C-ABI `TlsBackend` adapter、C-object adapter probe、OpenSSL QUIC TLS API/link probe，以及可产出第一段 TLS CRYPTO flight，并让 peer transport parameters、Handshake secrets 和入站 CRYPTO 经 OpenSSL callback 边界进入连接层的 OpenSSL-backed adapter wrapper；缺少完整对端 transcript 和 1-RTT traffic-secret yield。 |
...
- 2026-06-04：扩展 `examples/tls_openssl_backend_adapter.zig`，让
  ...
  secrets 和 Handshake CRYPTO bytes 经 OpenSSL callback 边界进入连接层；完整对端
  transcript 驱动和 1-RTT traffic-secret yield 仍待实现。
...
| `tls_openssl_backend_adapter` | 可运行的 OpenSSL-backed `TlsBackend` wrapper 检查：现有 drive 路径会把 quicz 本端 transport parameters 设置到 OpenSSL TLS object，调用 `SSL_do_handshake()` 产出第一段 TLS CRYPTO flight，并让 peer transport parameters、Handshake secrets 和 Handshake CRYPTO bytes 经 OpenSSL callback 边界进入连接层；完整对端 transcript 驱动和 1-RTT traffic-secret yield 仍待实现。 | 已存在 |
