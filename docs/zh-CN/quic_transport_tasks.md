...
- 2026-06-04：扩展 `examples/tls_openssl_backend_adapter.zig`，让
  ...
  建模 handshake confirmation 后会显式丢弃双端 Handshake packet-number space 和
  Handshake keys；Initial、Handshake、Application echo、protected close 和
  close/drain timeout cleanup 现在共用同一个 socket/lifecycle loop owner；示例会打印
