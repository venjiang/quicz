...
| `udp_zero_rtt_loopback` | Socket-backed loopback UDP 0-RTT：lifecycle-routed installed-key 0-RTT STREAM delivery、explicit accept-before-process enforcement、rejection-driven peer key discard、accepted early ACK 证据、routed 1-RTT ACK cleanup，以及 1-RTT 边界上的 client/server 0-RTT key discard 证据。 | 已存在 |
