...
- 2026-06-02：新增 socket-backed UDP installed-key echo loopback 覆盖。
  ...
  request/echo payload equality、bytes-in-flight cleanup 和 timer cleanup 证据。
- 2026-06-02：扩展 socket-backed UDP CryptoBackend 1-RTT loopback echo 覆盖。
  ...
  final ACK routing，以及 bytes-in-flight/timer cleanup 证据组合到同一闭环。
...
- 2026-05-29：新增 socket-backed UDP address-validation loopback 覆盖。
  ...
  rejection，以及后续 server address-validation block/unblock 输出。
...
| `udp_zero_rtt_loopback` | Socket-backed loopback UDP 0-RTT：lifecycle-routed installed-key 0-RTT STREAM delivery、explicit accept-before-process enforcement、accepted early ACK 证据、routed 1-RTT ACK cleanup，以及 1-RTT 边界上的 client/server 0-RTT key discard 证据。 | 已存在 |
...
| `udp_echo_loopback` | Socket-backed loopback UDP installed-key 1-RTT echo：lifecycle-routed client STREAM delivery、server bidirectional STREAM echo、request/echo payload equality、final ACK cleanup，以及 bytes-in-flight/timer cleanup 证据。 | 已存在 |
| `udp_crypto_backend_loopback` | Socket-backed loopback UDP CryptoBackend：mock `CryptoBackend` 1-RTT traffic-secret handoff、modeled handshake confirmation、lifecycle-routed installed-key STREAM echo、final ACK cleanup，以及 bytes-in-flight/timer cleanup 证据。 | 已存在 |
| `udp_handshake_done_loopback` | Socket-backed loopback UDP HANDSHAKE_DONE：lifecycle-routed installed-key HANDSHAKE_DONE confirmation、server/client Handshake key discard 证据，以及 routed ACK pending/cleanup 证据。 | 已存在 |
...
| `udp_address_validation_loopback` | Socket-backed loopback UDP address-validation：lifecycle-routed protected HANDSHAKE_DONE/NEW_TOKEN delivery、带显式 changed-path rejection 的 NEW_TOKEN path/version binding、secret rotation、replay snapshot restore rejection，以及后续 server address-validation block/unblock 证据。 | 已存在 |
