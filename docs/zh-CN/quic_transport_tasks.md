...
| RFC 9002 loss detection 与 congestion control | Partial | ACK delay scaling/capping、packet/time-threshold loss、NewReno recovery-period 行为、persistent congestion、带 Initial/Handshake max_ack_delay suppression 的 packet-space PTO PING/new-data probe hook、ECN validation 测试和 socket-backed UDP loss-recovery/PTO-recovery loopback。 | 仍需 protected-packet PTO/loss timer 完整调度，以及基于 controlled-clock 的重传行为测试。 |
