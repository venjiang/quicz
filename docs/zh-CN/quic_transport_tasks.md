...
- 2026-06-05：新增 `examples/tls_openssl_pair_transcript.zig` 和一个小 C
  ...
  Handshake/1-RTT traffic-secret callback。Zig 侧现在会复制这些生成的 CRYPTO bytes，
...
- 2026-06-04：扩展 `examples/tls_openssl_backend_adapter.zig`，让
  ...
  socket/lifecycle loop owner；示例会打印 `adapter_pto=62/53/511/1`，依次证明 PTO
...
- 2026-06-04：新增 `examples/tls_openssl_probe.zig` 和通过 `pkg-config` 链接
  ...
  callbacks 后 `SSL_is_quic()` 仍为 false。真实 TLS transcript 驱动仍待实现。
...
- 2026-06-04：新增很小的 C-ABI `TlsBackend` adapter，把 C TLS callback 的
  ...
  bytes、Handshake traffic-secret 安装和 handshake confirmation。具体 C TLS 库绑定仍待实现。
