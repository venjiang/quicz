...
| `udp_close_lifecycle_loopback` | Socket-backed loopback UDP close lifecycle：protected CONNECTION_CLOSE delivery、`EndpointConnectionLifecycle` connection-handle route retirement、保留的 inactive-CID stateless reset token lookup、reset emission 和 client token match。 | 已存在 |
