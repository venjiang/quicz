...
| `graceful_close` | 当前内存态、调用方 key protected long Initial/Handshake CONNECTION_CLOSE、调用方 key protected short CONNECTION_CLOSE/APPLICATION_CLOSE 收发、包含 flow-control、未匹配 PATH_RESPONSE、active connection-ID limit 和 role-specific NEW_TOKEN/HANDSHAKE_DONE 的语义 frame 错误 auto-close、protected receive auto-close、lifecycle-routed protected receive auto-close、peer close 诊断、默认/space/packet-type 非法 frame-payload auto-close、重发与 closing/draining 状态行为。 | 已存在 |
