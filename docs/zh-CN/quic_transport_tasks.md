...
| RFC 9000 传输协议 | Partial | 含 ACK/ACK_ECN range 校验的 frame codec、transport parameter、连接状态、stream、flow control、connection ID、Retry/token、path validation、close/reset 行为、endpoint idle route/timer 清理、endpoint routing helper、受保护 packet 解密和状态更新前的本地 UDP datagram 大小拒绝、lifecycle-owned caller-keyed protected UDP packet loopback、socket-backed UDP path-validation route-update loopback、socket-backed UDP lifecycle Retry/address-validation loopback 和示例。 | 仍需完整 protected/TLS socket-backed client/server loopback、完整 endpoint 生命周期和外部互通。 |
