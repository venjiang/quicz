...
| P0-F1 | 第一轮外部互通门禁 | QUIC v1 对一个成熟栈互通 | Process client/server 与 endpoint loop | Clean checkout 通过 `quic-go` 证书校验 handshake、Retry、stream echo/control、close 和一次可控 loss/PTO case。 | Active |
...
| 15 | P0-F1 quic-go gate | Active | 运行并记录 clean-checkout 第一轮外部互通命令集。 |
