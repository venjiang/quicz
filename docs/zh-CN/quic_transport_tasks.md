...
- 2026-06-04：新增 `examples/tls_openssl_backend_adapter.zig` 和
  OpenSSL-backed C wrapper for `TlsBackend`。该示例证明现有 crypto drive 路径可以把
  quicz 本端 transport parameters 传入 `SSL_set_quic_tls_transport_params()`，并把
  Handshake CRYPTO bytes 投递给 OpenSSL wrapper。peer transport parameters、
  traffic-secret yield 和完整 TLS transcript 驱动仍待实现。
...
| `tls_openssl_backend_adapter` | 可运行的 OpenSSL-backed `TlsBackend` wrapper 检查：现有 drive 路径会把 quicz 本端 transport parameters 设置到 OpenSSL TLS object，并把 Handshake CRYPTO bytes 投递给 wrapper；transcript output、peer parameters 和 traffic-secret yield 仍待实现。 | 已存在 |
