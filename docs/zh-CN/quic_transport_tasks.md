...
- 2026-05-28：新增 RFC 9002 NewReno underutilized-cwnd 增长抑制。
  ...
  underutilized cwnd，再展示满窗口 slow-start 与 congestion-avoidance 增长。
...
| `loss_recovery` | 当前 frame-payload packet-threshold loss、time-threshold loss、aggregate loss-time timer service、NewReno underutilized-cwnd suppression、slow-start/congestion-avoidance 增长、recovery period、新拥塞事件后一次性 STREAM recovery probe、minimum-window ssthresh clamp、不受 PTO backoff 放大的 persistent congestion 与 ACK-delay 处理。 | 已存在 |
