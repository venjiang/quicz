...
- [CRYPTO streams](examples/crypto_stream.zig)：按 packet number space 隔离的 CRYPTO buffering、接收缓冲超限 auto-close、mock backend handoff、protected backend transport-parameter auto-close、backend-confirmed Handshake key discard 和 protected CRYPTO flow。
...
- [Loss recovery](examples/loss_recovery.zig)：ACK 驱动的 loss、RTT sampling、NewReno、persistent-congestion min-RTT refresh 和非连续 persistent-congestion 抑制。
...
- [UDP 0-RTT loopback](examples/udp_zero_rtt_loopback.zig)：socket-backed lifecycle-routed installed-key 0-RTT STREAM delivery、accept-before-process enforcement、1-RTT ACK cleanup 和 server-side 0-RTT key discard。
