...
- [Graceful close](examples/graceful_close.zig)：本端/对端关闭、protected long/short close、非法 ACK range auto-close、包含非法 ACK、冲突 STREAM data 和非法 stream control frame 的语义 frame 错误 auto-close、protected receive auto-close、lifecycle-routed protected auto-close、draining 行为和关闭触发校验。
