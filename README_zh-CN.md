...
- [CRYPTO streams](examples/crypto_stream.zig)：按 packet number space 隔离的 CRYPTO buffering、接收缓冲超限 auto-close、mock backend handoff、protected backend transport-parameter auto-close、compatible backend Version Information handoff、backend-confirmed Handshake key discard 和 protected CRYPTO flow。
