...
- [TLS OpenSSL pair transcript](examples/tls_openssl_pair_transcript.zig)：使用固定
  ...
  并验证双向 protected Handshake CRYPTO 投递。运行：
...
- TLS 状态：已有 mock `CryptoBackend` handoff 和很小的 C-ABI `TlsBackend` adapter；
  ...
  使用 OpenSSL Handshake secrets 的 installed-key protected Handshake 投递；
  ...
  和入站 CRYPTO 经 callback 边界进入连接层；protected Application packetization 和真实
  TLS-owned socket echo 仍待实现。
