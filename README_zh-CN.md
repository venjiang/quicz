...
相同的显式输出选择；single-connection due-deadline-to-backend poll 和 drain wrapper，
包括 cross-space backend-drive 形态，也会在 backend drive 前保留显式 0-RTT recovery output；
cross-connection due-deadline-to-backend poll/drain 的 `WithInstalledKeyOptions` 变体会在
backend sweep 前保留同样的显式选择；生产级
