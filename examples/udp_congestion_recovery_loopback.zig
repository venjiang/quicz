...
fn runRecoveryPeriodPhase(allocator: std.mem.Allocator, io: std.Io) !RecoveryPeriodResult {
    ...
    while (packet_number < 8) : (packet_number += 1) {
        ...
        _ = try sendClientPing(allocator, io, &client_socket, &server_socket, &server_router, &client, &server, now_millis, &server_receive_buf, secrets.client);
    }
    try require(client.sentPacketCount(.application) == 8);
    try require(server.pendingAckLargest(.application) == 7);

    _ = try sendServerAck(allocator, io, &server_socket, &client_socket, &client_router, &client, 100, 0, .{
    ...
    _ = try sendServerAck(allocator, io, &server_socket, &client_socket, &client_router, &client, 120, 1, .{
...
fn runPersistentCongestionPhase(allocator: std.mem.Allocator, io: std.Io) !PersistentCongestionResult {
    ...
    _ = try sendClientPing(allocator, io, &client_socket, &server_socket, &server_router, &client, &server, 0, &server_receive_buf, secrets.client);
    _ = try sendServerAck(allocator, io, &server_socket, &client_socket, &client_router, &client, 100, 0, .{
    ...
    _ = try sendClientPing(allocator, io, &client_socket, &server_socket, &server_router, &client, &server, 10, &server_receive_buf, secrets.client);
    _ = try sendClientPing(allocator, io, &client_socket, &server_socket, &server_router, &client, &server, 1000, &server_receive_buf, secrets.client);
    _ = try sendClientPing(allocator, io, &client_socket, &server_socket, &server_router, &client, &server, 1100, &server_receive_buf, secrets.client);
    _ = try sendClientPing(allocator, io, &client_socket, &server_socket, &server_router, &client, &server, 1200, &server_receive_buf, secrets.client);
    try require(client.sentPacketCount(.application) == 4);

    _ = try sendServerAck(allocator, io, &server_socket, &client_socket, &client_router, &client, 1300, 1, .{
