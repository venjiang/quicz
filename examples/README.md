...
| Go client | `interop/go_echo_client/main.go` | quic-go FIN echo client; `-expect-stream-limit`, `-expect-reset`, `-expect-stop-sending`, `-expect-uni`, and `-expect-server-pto` validate matching concurrent server modes, including a bounded server-PTO recovery run. |
