...
| Go client | `interop/go_echo_client/main.go` | quic-go FIN echo client; `-expect-stream-limit`, `-expect-reset`, `-expect-stop-sending`, and `-expect-uni` validate matching concurrent server modes. |
