...
- [x] RFC 9000/RFC 9368 transport error code helpers, including fixed codes, VERSION_NEGOTIATION_ERROR, CRYPTO_ERROR TLS alert mapping, frame codec error classification to FRAME_ENCODING_ERROR, and frame packet-type violation classification to PROTOCOL_VIOLATION
...
- [x] Simplified loss recovery and congestion-control state with automatic ACK generation, ACK range validation/handling, unsent-packet ACK rejection, ACK-driven sent-packet tracking, ACK-driven STREAM, protected 0-RTT STREAM/RESET_STREAM/STOP_SENDING, and frame-payload/protected CRYPTO retransmission requeue, largest-acknowledged RTT sampling with connection-level RTT estimate sharing across packet number spaces and Initial/Handshake RTT ACK-delay suppression, Application ACK delay exponent / post-confirmation max_ack_delay handling, packet/time-threshold loss detection, deterministic loss-timeout hook, aggregate loss-time-before-PTO timer deadline selection and service helper with closing/draining disarm, endpoint-owned multi-connection recovery timer scheduling, cross-space bytes-in-flight congestion admission, peer `max_udp_payload_size` recovery max_datagram_size/initial-cwnd resync, NewReno slow-start/congestion-avoidance byte-counted and batched-ACK growth with underutilized-cwnd suppression, recovery period, new-congestion-event one-packet recovery probe, minimum-window clamp with unclamped ssthresh, PTO-backoff-independent persistent congestion duration/response, ACK_ECN CE congestion response, client Initial ACK PTO-backoff reset suppression, packet-number-space PTO hook that uses connection-level PTO backoff, disarms server PTO while anti-amplification-limited with no send credit, omits max_ack_delay for Initial/Handshake spaces, gates Application PTO until handshake confirmation, prefers queued ack-eliciting data, in-flight CRYPTO, protected 0-RTT STREAM/control frames, or in-flight STREAM before PING probes, and also queues cross-space peer probes for other in-flight packet number spaces, plus socket-backed UDP endpoint-timer-driven loss/PTO and congestion/STREAM-retransmission recovery loopbacks
- [x] Experimental Initial/Handshake/Application packet number space model for frame-payload ACK/recovery isolation, RFC 9000 Initial/Handshake/0-RTT frame-type filtering, modeled RFC 9001 Initial discard, and Initial/Handshake discard cleanup including ECN state
- [x] Frame-payload ACK_ECN counter validation for modeled ECT(0)/ECT(1) sent packets, ACK_ECN CE-driven NewReno recovery response, plus in-memory endpoint ECN state scoped by UDP path identity and socket-backed UDP ECN validation loopback
...
pub fn main() !void {
    ...
    // CRYPTO, ACK, loss, PTO, ECN state, and installed Handshake keys after key
    ...
    // window to the RFC 9002 minimum using a duration that ignores PTO backoff;
    // lossDetectionTimerDeadlineMillis() exposes the aggregate loss-time-before-PTO
    // recovery timer; serviceLossDetectionTimer() handles a due aggregate
    // timer by applying due loss-time deadlines or PTO probing;
    // EndpointLossDetectionTimers schedules those aggregate timers across
    // caller-owned connection handles. Direct checkLossDetectionTimeouts() and
    // checkPtoTimeouts() remain available for tests that need to service those
    // paths separately; ACK_ECN uses the ACK
...
[`examples/udp_stream_retransmission_loopback.zig`](examples/udp_stream_retransmission_loopback.zig),
...
loss-recovery including invalid ACK range rejection, largest-acknowledged RTT sampling, shared RTT estimates across packet number spaces, ACK-delay, cross-space bytes-in-flight congestion admission, peer max_udp_payload_size recovery resync, NewReno underutilized-cwnd suppression, slow-start/congestion-avoidance byte-counted and batched-ACK growth, endpoint-timer-driven loss/PTO service with closing/draining disarm, recovery-period, minimum-window ssthresh clamp, ACK_ECN CE response, and persistent congestion handling,
PTO-recovery including Application PTO gating until handshake confirmation, client Initial ACK PTO-backoff reset suppression, anti-amplification-limited server PTO disarm, connection-level PTO backoff, and cross-space peer probes, path-validation, address-validation including token version binding, Retry-token processing and v1/v2 integrity-tag,
...
socket-backed UDP endpoint routing with client-side Version Negotiation selection, socket-backed UDP zero-length CID tuple routing, socket-backed UDP preferred-address route migration, socket-backed UDP replacement-CID route retirement, socket-backed UDP connection-ID NEW/RETIRE exchange, caller-keyed protected UDP packet loopbacks, socket-backed UDP flow-control credit refresh, socket-backed UDP spin-bit signaling, socket-backed UDP ECN validation, socket-backed UDP loss recovery, socket-backed UDP congestion recovery, socket-backed UDP PTO recovery, socket-backed UDP STREAM retransmission, socket-backed UDP installed-key key update, socket-backed UDP path-validation route updates, socket-backed UDP Retry/address-validation routing, socket-backed close-triggered route retirement, and socket-backed UDP stateless-reset emission; they are not yet interoperable
