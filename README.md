...
- [x] Frame-payload ACK_ECN counter validation for modeled ECT(0)/ECT(1) sent packets plus in-memory endpoint ECN state scoped by UDP path identity and socket-backed UDP ECN validation loopback
...
loss-recovery including ACK-delay, NewReno slow-start/congestion-avoidance growth, endpoint-timer-driven loss/PTO service, recovery-period, and persistent congestion handling,
