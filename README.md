...
- [TLS OpenSSL pair transcript](examples/tls_openssl_pair_transcript.zig):
  ...
  protected Handshake CRYPTO delivery in both directions. Run with
...
- TLS status: mock `CryptoBackend` handoff and a narrow C-ABI `TlsBackend`
  ...
  callback boundaries. Protected Application packetization and real TLS-owned
  socket echo are still pending.
