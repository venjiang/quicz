...
- [x] Simplified loss recovery and congestion-control state with automatic ACK generation, ACK range handling, unsent-packet ACK rejection, ACK-driven sent-packet tracking, ACK delay exponent / post-confirmation max_ack_delay handling, packet/time-threshold loss detection, deterministic loss-timeout hook, NewReno-style recovery period, persistent congestion response, packet-number-space PTO hook that omits max_ack_delay for Initial/Handshake spaces and prefers queued ack-eliciting data before PING probes, and socket-backed UDP loss/congestion/PTO recovery loopbacks
