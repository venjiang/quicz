...
- `run-tls-openssl-pair-transcript`: OpenSSL client/server callback-mode TLS
  ...
  back by the server connection, and the OpenSSL Handshake secrets drive
  installed-key protected Handshake CRYPTO delivery in both directions. The
  OpenSSL 1-RTT secrets also protect a quicz STREAM request/response over
  installed-key short packets.
...
- [TLS OpenSSL pair transcript](examples/tls_openssl_pair_transcript.zig):
  ...
  long-packet helpers, installs OpenSSL-produced Handshake secrets, and verifies
  protected Handshake CRYPTO delivery in both directions. OpenSSL-produced
  1-RTT secrets also drive an installed-key protected STREAM request/response.
...
- TLS status: mock `CryptoBackend` handoff and a narrow C-ABI `TlsBackend`
  ...
  verifies protected Initial long-packet delivery for the client Initial flight
  plus installed-key protected Handshake delivery using OpenSSL Handshake secrets
  and installed-key protected STREAM request/response using OpenSSL 1-RTT secrets;
  ...
  callback boundaries. Real TLS-owned socket echo is still pending.
