...
- [UDP 0-RTT loopback](examples/udp_zero_rtt_loopback.zig):
  ...
  accept-before-process enforcement, accepted early ACK evidence, 1-RTT ACK
  cleanup, and client/server 0-RTT key discard evidence. Run with
...
- [UDP echo loopback](examples/udp_echo_loopback.zig):
  ...
  payload equality, final ACK cleanup, and bytes-in-flight/timer cleanup
...
- [UDP CryptoBackend loopback](examples/udp_crypto_backend_loopback.zig):
  ...
  lifecycle-routed installed-key STREAM echo, final ACK cleanup, and
  bytes-in-flight/timer cleanup evidence. Run with
...
- [UDP key update loopback](examples/udp_key_update_loopback.zig):
  ...
  gating with observable ACK threshold and generation-count evidence. Run with
