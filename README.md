...
- [x] Simplified loss recovery and congestion-control state with automatic ACK generation, ACK range handling, unsent-packet ACK rejection, ACK-driven sent-packet tracking, ACK-driven STREAM and frame-payload/protected CRYPTO retransmission requeue, ACK delay exponent / post-confirmation max_ack_delay handling, packet/time-threshold loss detection, deterministic loss-timeout hook, NewReno-style recovery period, persistent congestion response, packet-number-space PTO hook that omits max_ack_delay for Initial/Handshake spaces and prefers queued ack-eliciting data, in-flight CRYPTO, or in-flight STREAM before PING probes, and socket-backed UDP loss/congestion/PTO/STREAM-retransmission recovery loopbacks
...
- [ ] Full RFC 9002 loss detection & congestion control with complete protected-packet loss/PTO timer scheduling, remaining 0-RTT retransmission coverage, and remaining NewReno details
