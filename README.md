...
- [TLS OpenSSL pair transcript](examples/tls_openssl_pair_transcript.zig):
  ...
  long-packet helpers and verifies server-side delivery. Run with
...
- TLS status: mock `CryptoBackend` handoff and a narrow C-ABI `TlsBackend`
  ...
  callback boundaries. Protected Handshake/Application packetization and real
  TLS-owned socket echo are still pending.
