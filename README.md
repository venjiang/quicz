...
- [x] Minimal in-memory connection and stream queue/receive flow with send-side STREAM fragmentation, inbound RESET_STREAM and STOP_SENDING handling, PATH_CHALLENGE response queuing, client-side HANDSHAKE_DONE handling, basic connection/stream/stream-count flow control, strict stream direction validation, and close-state handling
...
pub fn main() !void {
    ...
    // matching PATH_RESPONSE; HANDSHAKE_DONE is accepted only by client
