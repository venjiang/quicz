...
- `run-tls-openssl-pair-transcript`: OpenSSL client/server callback-mode TLS
  ...
  check also routes the live Initial TLS CRYPTO bytes through that same
  socket/lifecycle boundary. The OpenSSL Handshake secrets drive
...
- [TLS OpenSSL pair transcript](examples/tls_openssl_pair_transcript.zig):
  ...
  and verifies a manual OpenSSL Initial transcript over the same socket path,
...
- TLS status: mock `CryptoBackend` handoff and a narrow C-ABI `TlsBackend`
  ...
  delivery for both Initial flights, and manual OpenSSL Initial transcript
  routing over the same socket/lifecycle boundary, installed-key protected Handshake delivery,
