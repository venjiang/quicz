...
- `run-tls-openssl-pair-transcript`: OpenSSL client/server callback-mode TLS
  ...
  Initial/Handshake/Application CRYPTO queues.
...
- [TLS OpenSSL pair transcript](examples/tls_openssl_pair_transcript.zig):
  ...
  the generated CRYPTO bytes into quicz packet-number-space CRYPTO queues. Run
  with `zig build run-tls-openssl-pair-transcript`.
...
- TLS status: mock `CryptoBackend` handoff and a narrow C-ABI `TlsBackend`
  ...
  CRYPTO handoff and maps the generated bytes into quicz CRYPTO queues, and
  ...
  callback boundaries. Protected packetization and real TLS-owned socket echo
  are still pending.
