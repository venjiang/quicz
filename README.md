...
- [TLS OpenSSL pair transcript](examples/tls_openssl_pair_transcript.zig):
  ...
  protected Handshake CRYPTO delivery in both directions, including loopback UDP
  delivery through the quicz endpoint lifecycle. OpenSSL-produced 1-RTT secrets
  also drive an installed-key protected STREAM request/response and a loopback
  UDP STREAM echo through the same lifecycle.
...
- TLS status: mock `CryptoBackend` handoff and a narrow C-ABI `TlsBackend`
  ...
  plus installed-key protected Handshake delivery, including socket-backed
  delivery, using OpenSSL Handshake secrets
