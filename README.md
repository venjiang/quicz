...
`EndpointConnectionLifecycle` provides the internal building blocks for routed
datagram receive, TLS-backend driving, timer selection, and bounded output
draining. These low-level lifecycle variants are intentionally not listed here;
application code should start from the runnable examples, while protocol details
remain in the transport task matrix and architecture documentation.
